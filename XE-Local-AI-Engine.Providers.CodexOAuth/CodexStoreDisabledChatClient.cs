...
internal sealed class CodexStoreDisabledChatClient : DelegatingChatClient
{
    ...
    /// </para>
    /// </summary>
    private (IEnumerable<ChatMessage> Messages, ChatOptions Options) PrepareCodexRequest(
        IEnumerable<ChatMessage> messages,
        ChatOptions? options)
    {
        ...
        result.Instructions = string.Join(
            "\n\n",
            new[] { result.Instructions }.Concat(systemTexts).Where(text => !string.IsNullOrWhiteSpace(text)));
