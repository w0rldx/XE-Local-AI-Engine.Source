...
internal sealed class DevelopmentTaskConfiguration : IEntityTypeConfiguration<DevelopmentTask>
{
    public void Configure(EntityTypeBuilder<DevelopmentTask> builder)
    {
        ...
        // A project carries N tasks: workflow decomposition materializes one implementation task per child node in the project the parent run already trusts. The
        // index stays — every project-scoped read leans on it — but it is no longer unique, and it is no longer named as if it were.
