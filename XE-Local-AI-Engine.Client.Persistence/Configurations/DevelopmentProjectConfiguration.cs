...
internal sealed class DevelopmentProjectConfiguration : IEntityTypeConfiguration<DevelopmentProject>
{
    public void Configure(EntityTypeBuilder<DevelopmentProject> builder)
    {
        ...
        // Deliberately PLAINTEXT, unlike DevelopmentTask/DevelopmentArtifact's encrypted byte[] columns: an encrypted column cannot be indexed, filtered or digest-compared, and the command
        // profile is non-secret operator-confirmed configuration, not credentials. Never add it to NodeEncryptionSaveChangesInterceptor or NodeEncryptionMaterializationInterceptor.
