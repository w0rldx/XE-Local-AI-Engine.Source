...
internal sealed class ConversationUploadedFileConfiguration : IEntityTypeConfiguration<ConversationUploadedFile>
{
    public void Configure(EntityTypeBuilder<ConversationUploadedFile> builder)
    {
        ...
        // Cascade from the owning conversation, configured without a navigation on the principal so the NodeConversation entity stays untouched. The cascade is
        // enforced; the raw-SQL purge still names this table so the whole footprint is listed in one place. Disk-resident bytes/extracted text: the file store.
