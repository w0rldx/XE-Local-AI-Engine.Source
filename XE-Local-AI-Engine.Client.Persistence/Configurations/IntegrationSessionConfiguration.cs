...
internal sealed class IntegrationSessionConfiguration : IEntityTypeConfiguration<IntegrationSession>
{
    public void Configure(EntityTypeBuilder<IntegrationSession> builder)
    {
        ...
        // conversation_id and agent_definition_id carry no foreign key, for the reason AgentWorkSessionConfiguration gives — the conversation purge path is raw ADO
        // SQL — and a sharper one: the session row is written BEFORE its conversation exists, the accept transaction committing first (ADR 0008 Decision §3).
