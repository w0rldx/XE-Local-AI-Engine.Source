...
/// </summary>
/// <remarks>
///     Every method goes through EF so both encryption interceptors run: <c>variables_json</c> and <c>bridge_token</c> are encrypted columns, and only a
///     <c>SaveChanges</c> seals them, only a materialization opens them. The compare-and-swap idiom is <c>IntegrationExecutionStore</c>'s, verbatim: query the
///     tracked row, compare <c>Version</c> and the expected status set, mutate, <c>Version++</c>, save. A <see cref="DbUpdateConcurrencyException" /> is the loser
///     learning it lost, and any other exception clears the tracker before it propagates, so the next call on this scoped context cannot replay the failed one's
///     changes.
/// </remarks>
public sealed class ExternalAppInstanceStore : IExternalAppInstanceStore
{
    /// <summary>
    ///     The brief's bound on an event's detail, in UTF-8 bytes.
    /// </summary>
    /// <remarks>
    ///     There is no exempt kind here: unlike the integration family's <c>external.output</c>, no event on this feed
    ///     carries a caller payload — service names, exit codes and failure categories all fit inside it many times over.
    /// </remarks>
    ...
    public async Task<ExternalAppStatusWriteResult> UpdateStatusAsync(ExternalAppStatusUpdate command, CancellationToken cancellationToken = default)
    {
        ...
        // FIRST, before the read this write mutates: acquiring it afterwards puts the transaction outside the boundary that clears the tracker, so a failure to
        // begin one leaves the entity mutated and the event queued on this scoped context, and the next save — a different caller's — would commit this transition.
        ...
        // ASSIGNED rather than merged, the one exception to the rule above: a transition is the newest word on why the instance is where it is, so a successful
        // start carrying two nulls must not inherit the category a failed attempt left behind. ClearFailure says that out loud and wins over values passed beside it.
        ...
        {
            // The SAME loser, reported by the other half of the batch: this store MINTS the sequence, so the loser's event collides on ux_external_app_instance_events_instance_sequence
            // before EF counts the rows its UPDATE missed. Not a caller bug like the integration family's duplicate sequence: a taken sequence means another writer won the CAS, bumping the version.
            _dbContext.ChangeTracker.Clear();
            return Lost;
        }
        catch (Exception)
        {
            // EVERYTHING inside the transaction, not only the save: a commit that throws rolls the database back while EF still holds the mutated entity as
            // committed, and the next compare-and-swap on this scoped context would then contend against that stale identity-map version and lose forever.
            _dbContext.ChangeTracker.Clear();
            throw;
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // No status, no desired state and no event: this is the update's recovery boundary, not a transition. The columns move together or not at all, so a crash
        // can only leave the instance wholly on the old manifest or wholly on the new one.
        entity.ManifestSnapshotJson = manifestSnapshotJson;
        entity.VariablesJson = Encoding.UTF8.GetBytes(variablesJson);
        entity.PublishedPortsJson = publishedPortsJson;
        entity.ManifestVersion = manifestVersion;
        ...
        {
            // The backfill, inside the same transaction as everything else this commit moves: the replacement containers were created with this token, so a row
            // that kept its null would hand the next Start a grant the running containers do not have.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Materialize the entities FIRST and map afterwards, the shape every store in this file's family uses: projecting the columns inside the LINQ query
        // materializes no entity, so the decryption interceptor never runs — harmless while detail_json is plaintext, silently wrong once a column here is encrypted.
    ...
    public async Task<bool> DeleteAsync(Guid instanceId, long expectedVersion, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // The compare-and-swap is the WHERE clause here rather than a tracked entity, the one write in the store with no change tracker in it: EF cascades a
            // tracked principal's deletion onto the dependents it tracks, so already bulk-deleted events would queue a second no-row DELETE and fail a good uninstall.
