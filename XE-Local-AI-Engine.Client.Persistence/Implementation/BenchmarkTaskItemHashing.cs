...
/// </summary>
/// <remarks>
///     Both are recomputed by the store on every write, so a caller cannot present a stale or invented value, and both
///     are plaintext so the ranking read compares them without decrypting a payload. A length-prefixed byte feed
///     rather than canonical JSON: that writer lives in the application layer, which this one deliberately cannot see.
///     Same shape as <c>TrainingDatasetStore</c>'s content fingerprint — explicit separators, invariant formatting, no
///     format that can be reordered.
/// </remarks>
internal static class BenchmarkTaskItemHashing
{
    ...
    /// <summary>
    ///     <c>v1:</c> + SHA-256 over exactly what an item asks: its kind, its revision and its four payloads.
    /// </summary>
    /// <remarks>
    ///     Every payload participates by its bytes, so a reference answer or a verifier override changing is as visible
    ///     as the prompt changing — the run that answered the old instance is no longer answering this question.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     The ordering is the whole design. Adding or deleting an item changes which questions the project asks and
    ///     must move the hash, because a cell's mean is a mean over that set; reordering changes no question and must
    ///     not, or a cosmetic drag-and-drop would unrank a completed suite.
    /// </remarks>
