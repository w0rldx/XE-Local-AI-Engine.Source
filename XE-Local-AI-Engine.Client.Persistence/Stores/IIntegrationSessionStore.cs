...
/// </summary>
/// <remarks>
///     <see cref="ConversationId" /> is <b>pre-minted</b>: the caller generates the Guid before calling
///     <c>AcceptAsync</c> and creates the owning <c>NodeConversation</c> at that id only AFTER the accept transaction
///     commits. The session row therefore carries a conversation id with no conversation row behind it for the width
///     of that gap, on purpose — which is what makes an orphan conversation impossible.
/// </remarks>
...
///     <c>AppendEventAsync</c> moves it on every persisted event, and <c>TryTerminalizeAsync</c> on the terminal one.
/// </remarks>
public interface IIntegrationSessionStore
{
    ...
    /// <summary>
    ///     The session, but only when <paramref name="principalId" /> owns it; <see langword="null" /> for a missing
    ///     row AND for a foreign one, so the two are indistinguishable to every caller.
    /// </summary>
    /// <remarks>
    ///     The masking rule is the shape of the return, not an <c>if</c> a route has to remember. Ownership is the
    ///     row's own <c>PrincipalId</c> column, never a key prefix: two credentials of one integrator must reach the
    ///     same sessions, so a rotation does not strand them. The CURRENT key's trigger allowlist is a second,
    ///     separate limb applied by <c>IntegrationExternalAccess</c> — authorisation is not a persistence decision.
    /// </remarks>
    ...
    /// <summary>
    ///     The session that owns <paramref name="conversationId" />, or <see langword="null" />. At most one row — a
    ///     conversation is owned by one session.
    /// </summary>
    /// <remarks>
    ///     The lookup behind <c>emit_output</c>: a tool handler has only the ambient conversation id the invocation
    ///     runner seeded, and this is what turns it back into the execution's session.
    /// </remarks>
    ...
    ///     A null filter argument does not constrain.
    /// </summary>
    /// <remarks>
    ///     The id tie-break is load-bearing for the same reason it is on the executions list: two sessions touched in
    ///     the same millisecond would otherwise page non-deterministically and drop or duplicate a row across pages.
    /// </remarks>
    ...
    ///     know how far the list reaches.
    /// </summary>
    /// <remarks>
    ///     Shares its filter code with <see cref="ListAsync" /> rather than repeating the predicates, so the count and
    ///     the page it labels cannot disagree.
    /// </remarks>
    ...
    /// <summary>
    ///     Removes the session row, returning <see langword="false" /> when the purge already removed it — which is
    ///     the ordinary case.
    /// </summary>
    /// <remarks>
    ///     The BACKSTOP of an operator delete, not its mechanism: deleting a session means purging its owned
    ///     conversation, and <c>ConversationFootprintPurge</c> already takes the session row, its executions and their
    ///     events with it. This runs afterwards so a purge that could not complete still removes the row, rather than
    ///     leaving an operator looking at a session whose conversation is gone.
    /// </remarks>
