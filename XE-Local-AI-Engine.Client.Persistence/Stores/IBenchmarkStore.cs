...
public interface IBenchmarkStore
{
    ...
    /// </summary>
    /// <remarks>
    ///     A project that persisted with judging off because a second transaction never ran is one an operator can
    ///     only retry into a duplicate. <paramref name="initialItems" /> is created in that same transaction for the
    ///     same reason, so a project never exists without at least one question to ask and no read path invents one.
    /// </remarks>
    /// <param name="initialItems">
    ///     The project's task items. Empty or <see langword="null" /> leaves the project item-less;
    ///     <see cref="GetOrCreateItemsAsync" /> materializes item 0 for it on first touch.
    /// </param>
    ...
    ///     it has none — a LEGACY path only.
    /// </summary>
    /// <remarks>
    ///     A project created after task items existed gets them in the same transaction as itself, so the
    ///     write-on-a-read-path is unreachable from anything an operator can newly create. It runs inside the normal EF
    ///     write path so both encryption interceptors fire, which is why a migration cannot do this instead: it has no
    ///     node key, and the prompt is a required encrypted blob bound to its own item's id. Idempotent under the
    ///     unique (project, index) index, and it leaves <see cref="BenchmarkProjectRecord.TaskItemSetHash" /> null.
    /// </remarks>
    ...
    /// <summary>
    ///     Appends one task item and recomputes the project's item-set hash.
    /// </summary>
    /// <remarks>
    ///     A changed set hash resets the rank cohort, through the same path a judge-policy activation uses: the project
    ///     score is a mean over the item set, so changing the set changes what the score means. A generator's leaf case
    ///     is an ordinary item with its own id, so every cap, every hash and the export reach it without knowing what
    ///     generated it.
    /// </remarks>
    /// <param name="children">
    ///     The leaf cases a generator item expands into, written in the SAME transaction as the generator, which
    ///     therefore never exists without the cases it promises.
    /// </param>
    ...
    ///     recomputing its <see cref="BenchmarkTaskItemRecord.InputHash" />.
    /// </summary>
    /// <remarks>
    ///     The new hash is what makes every stored answer to the OLD instance identifiable as an answer to a question
    ///     that no longer exists. Regenerating children atomically is the point too: a case must never be left
    ...
    ///     place, inside the same transaction as the edit.
    /// </param>
    ...
    /// <summary>
    ///     Deletes one task item, and a generator's children before the generator itself.
    /// </summary>
    /// <remarks>
    ///     Nothing cascades here, so that order is what makes the delete legal. Deleting the last leaf is refused: a
    ///     project always asks at least one question.
    /// </remarks>
    Task DeleteTaskItemAsync(Guid projectId, Guid itemId, long expectedItemVersion, CancellationToken cancellationToken = default);

    /// <summary>
    ///     Renumbers the project's items into the given order.
    /// </summary>
    /// <remarks>
    ///     Deliberately NOT a revision bump and NOT a cohort reset: the index is a display position, absent from every
    ///     hash, and a drag-and-drop must not unrank a completed suite. <paramref name="orderedItemIds" /> must name
    ///     exactly the project's current items, which is also this call's concurrency check.
    /// </remarks>
    ...
    ///     their primary work items inserted in the given (FIFO execution) order.
    /// </summary>
    /// <remarks>
    ///     All-or-nothing is the point: one run per transaction, each chaining its CAS on its predecessor, lets a
    ///     concurrent writer land mid-group, so the caller gets a conflict and no ids while the runs already inserted
    ///     stay queued and consume the exclusive runtime, and a batch caller cannot chain because the project version
    ///     moved by a number it was never told. Every command must name the same project, and a
    ///     <see cref="BenchmarkStartRunCommand.FreezeCommitGuard" /> is evaluated once per distinct guard instance.
    /// </remarks>
    ...
    /// <summary>
    ///     Every run of a project in one call, ranked once.
    /// </summary>
    /// <remarks>
    ///     The paging overload recomputes the whole-project ranking — a full scan plus a judge-view join across three
    ///     more tables — for each page, and an export reads every page. The default body pages through
    ///     <see cref="ListRunsAsync" /> so a test double needs no extra member.
    /// </remarks>
    ...
    /// <summary>
    ///     The project's measurement CELLS: one model, one KV type, one repeat of the whole task-item suite.
    /// </summary>
    /// <remarks>
    ///     A cell is what ranks, so this is the shape a comparison reads — the per-run listing shows the same numbers
    ///     one row at a time and cannot say which items a cell is missing.
    /// </remarks>
    ...
    /// <summary>
    ///     The project's ACTIVE work — queued or running — counted per kind, empty when the project is idle.
    /// </summary>
    /// <remarks>
    ///     A run's <see cref="BenchmarkRunRecord.PrimaryStatus" /> does not answer this: a judged, measured or
    ///     pairwise-compared matrix keeps the single-consumer queue and the GPU busy long after every run of it reads
    ...
    ///     Why generation stopped, when the failure says so — <c>timeout</c> for a run cancelled at its invocation
    ///     budget. Null leaves the column untouched rather than guessing.
    /// </param>
    ...
    /// </summary>
    /// <remarks>
    ///     Re-measuring inserts a NEW attempt rather than reusing the previous one, so the numbers a KLD figure was
    ///     measured against survive a corpus or chunk-count change.
    /// </remarks>
    Task<Guid> EnqueueFidelityAsync(Guid runId, string kind, CancellationToken cancellationToken = default);

    /// <summary>
    ///     Terminalizes a fidelity work item and its attempt, and refreshes the run's fidelity projection from it only
    ///     when the attempt succeeded AND is the highest-sequenced succeeded attempt of the run.
    /// </summary>
    /// <remarks>
    ///     A stale re-measurement therefore cannot overwrite newer numbers, and a failed one leaves the previous
    ///     numbers alone.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     For a blocker that clears itself, such as a base-logit file another process is still writing: the work item
    ///     pins <c>attempt = 1</c>, so terminalizing it as failed is terminal in the literal sense and the "it will be
    ///     retried" the operator was promised has nothing behind it. A no-op once the attempt is terminal, so a requeue
    ///     racing a completion cannot start a second measurement.
    /// </remarks>
    ...
    ///     scope, the runs eligible to be paired inside it, and the comparisons that already exist.
    /// </summary>
    /// <remarks>One read, because all three questions are about the same consistent moment.</remarks>
    ...
    ///     items, in ONE transaction, and bumps the revision's <c>ComparisonSetVersion</c> in the same one.
    /// </summary>
    /// <remarks>
    ///     Idempotent: a slot a concurrent caller already created violates the filtered unique index, and the violation
    ///     is swallowed — a half-created cohort never completes and therefore never publishes a score.
    /// </remarks>
    ...
    ///     <c>ComparisonSetVersion</c> in the same transaction.
    /// </summary>
    /// <remarks>
    ///     Inserting and terminalizing are the only two ways the fitted set can change, so they are the only two places
    ///     that bump it.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     A duplicate publication violates <c>ux_benchmark_pairwise_fits_key</c> and no-ops, and the filtered active
    ///     index makes two active fits in one scope unrepresentable rather than merely unlikely.
    /// </remarks>
    ...
    /// <summary>
    ///     Records the primary phase's durable launch evidence: an insert-if-null write of the receipt and environment
    ///     columns, keyed by the immutable work item rather than by the run's mutable version.
    /// </summary>
    /// <remarks>
    ///     It never overwrites an existing block and never changes any status, so it is safe to call before inference
    ///     and again while terminalizing. Terminalizing a work item bumps its version by exactly one, which is the
    ///     cancel-first ordering <paramref name="claimedWorkVersion" /> accepts.
    /// </remarks>
    /// <returns><see langword="true" /> when this call wrote the block.</returns>
    /// <param name="workItemId">The claimed work item's queue sequence.</param>
    /// <param name="claimedWorkVersion">
    ///     The version the caller claimed: accepted while the work item is still <c>Running</c> at it, or already
    ///     <c>Cancelled</c> at its successor. Anything else is refused.
    /// </param>
    ...
    ///     execution identity was incomplete (fail-closed: it is never ranked).
    /// </param>
    ...
    ///     and every pairwise comparison it took part in on EITHER side.
    /// </summary>
    /// <remarks>
    ///     Refused while any of those is Queued or Running, including a comparison whose canonical first run is the
    ///     other one, which no work item names. Deleting comparisons bumps the affected revisions'
    ///     <c>ComparisonSetVersion</c> and deactivates the project's active pairwise fits, because a fit whose fitted
    ///     set names a deleted run ranks something that is not there. The next planner pass re-fits what is left.
    /// </remarks>
    ...
    ///     seen it, and resets that revision's rank cohort (reference key cleared, generation bumped).
    /// </summary>
    /// <remarks>
    ///     The reset means only attempts enqueued after this call can define the cohort. Activating the hash the
    ///     project is already on is a no-op that resets nothing, and the call is refused while any attempt of the
    ///     project is Queued or Running, so a reset always covers the complete eligible set — with a seed, the attempts
    ///     go in the same transaction, so a cohort is never left reset with only part of its set re-judged.
    /// </remarks>
    ...
    ///     <see cref="BenchmarkJudgeAttemptSeed.ExpectedJudgePolicyRevisionId" /> is ignored: the hash names it.
    /// </param>
    ...
    ///     project write honours.
    /// </summary>
    /// <remarks>
    ///     A frozen project refuses every other edit because its runs were measured against the task, context and
    ///     agent it carries; the fidelity settings are none of those — they decide what gets measured NEXT, and every
    ///     stored number keeps its own comparability digest. <paramref name="measureExisting" /> additionally enqueues
    ///     a fidelity item for every succeeded, non-warm-up, first-of-its-repeat-group run with no attempt yet, in the
    ///     same transaction — the rule freeze applies per cell — and the count is reported.
    /// </remarks>
    ...
    ///     succeeds while the revision still carries <paramref name="cohortGeneration" /> and no key yet.
    /// </summary>
    /// <remarks>
    ///     Called from inside the transaction that stores a successful attempt's result, never at launch readiness — a
    ///     failed first attempt must not poison the cohort.
    /// </remarks>
    ...
    ///     revision's cohort (reference key cleared, generation bumped), bumps the project version, returns the runs.
    /// </summary>
    /// <remarks>
    ///     The reset and the eligible set are decided in one transaction, so a re-judge is never partial. A project
    ///     that moved revision since the runtime was resolved throws
    ///     <see cref="BenchmarkJudgePolicyChangedException" />.
    /// </remarks>
    /// <param name="cohortAttemptSeed">
    ///     When supplied, one Queued attempt per eligible run is inserted in the same transaction as the reset, and
    ///     <see cref="BenchmarkJudgeAttemptSeed.ExpectedJudgePolicyRevisionId" /> is honoured.
    /// </param>
