...
internal sealed record class NodeMessage
{
    ...
    ///     request agent ?? conversation agent ?? Default Assistant).
    /// </summary>
    /// <remarks>
    ///     Stored as a plaintext, SQL-queryable column — the agent id is non-secret — so feedback can be aggregated
    ///     per agent without decrypting the metadata blob. Null for user turns and for assistant turns written before
    ///     this column existed.
    /// </remarks>
