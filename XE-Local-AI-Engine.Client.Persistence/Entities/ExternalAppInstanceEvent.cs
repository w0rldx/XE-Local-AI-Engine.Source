...
internal sealed class ExternalAppInstanceEvent
{
    ...
    /// <summary>
    ///     Monotonic per instance, starting at 1 with the install's <c>PermissionAccepted</c> event.
    /// </summary>
    /// <remarks>
    ///     Minted by the store inside the compare-and-swap transaction that writes the status, so there are no holes:
    ///     a lost CAS writes nothing at all, not even the event.
    /// </remarks>
    ...
    /// <summary>
    ///     A small JSON elaboration, or null. <b>Plaintext on purpose</b> and content-free by contract, bounded at
    ///     4 KiB by the store.
    /// </summary>
    /// <remarks>
    ///     It carries service names, exit codes and failure categories, never a variable value — which is why this
    ///     family's only encrypted column is the instance's <c>variables_json</c> and this one is replayable to the
    ///     hub as it stands.
    /// </remarks>
