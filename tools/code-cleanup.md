...
```bash
dotnet build XE-Local-AI-Engine.slnx          # must be 0 errors / 0 warnings
dotnet test  XE-Local-AI-Engine.AI.Agent.Tests/XE-Local-AI-Engine.AI.Agent.Tests.csproj
# …and the other test projects, one at a time (the MAF workflow suites are timing-sensitive
#   and must not run alongside other suites — see HandoffWorkflowSpikeTests).
```
