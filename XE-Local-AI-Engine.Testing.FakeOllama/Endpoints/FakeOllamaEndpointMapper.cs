namespace XE_Local_AI_Engine.Testing.FakeOllama
{
    ...
    internal static class FakeOllamaEndpointMapper
    {
        ...
        public static void MapFakeOllamaEndpoints(this WebApplication app, FakeOllamaState state)
        {
            ...
            app.MapGet("/api/version", () => Results.Json(new { version = "0.0.0-fake" }, JsonOptions));
        ...
        internal static bool StreamEnabled(JsonElement element)
            => !element.TryGetProperty("stream", out var stream) || stream.ValueKind != JsonValueKind.False;
        ...
        internal static async Task<bool> TryApplyFailureAsync(HttpContext context, FakeOllamaState state, string? model)
        {
            ...
            switch (failure)
            {
                case FakeOllamaFailure.ModelUnavailable:
                    ...
                    await context.Response.WriteAsJsonAsync(new { error = $"model '{model ?? "unknown"}' not found" }, JsonOptions, context.RequestAborted).ConfigureAwait(false);
                    return true;
                ...
                case FakeOllamaFailure.Http500:
                    ...
                    await context.Response.WriteAsJsonAsync(new { error = "fake ollama injected failure" }, JsonOptions, context.RequestAborted).ConfigureAwait(false);
                    return true;
                case FakeOllamaFailure.PartialStream:
                    await WriteNdjsonAsync(context, new[]
                    {
                        new { model, created_at = NowString(), response = "partial", done = false },
                        new { model, created_at = NowString(), response = " stream", done = false },
        ...
        internal static void Record(HttpContext context, FakeOllamaState state, string? model, int messageCount, string? prompt)
            => state.Record(new FakeOllamaRequest(
        ...
        internal static IReadOnlyList<double> Embed(string input, int dimensions) => EmbeddingDeterminism.EmbedDeterministic(input, dimensions);

        internal static string NowString() => DateTimeOffset.UtcNow.ToString("O", CultureInfo.InvariantCulture);
