...
internal sealed class GtkDesktopBridge : IAsyncDisposable
{
    ...
    internal void ReceiveMessage(string? body)
    {
        ...
        try
        {
            ...
            if (kind == "xe-save-unavailable")
            {
                _ = NotifyFailureAsync(_stopping.Token);
                return;
            }
    ...
    private void OnLoad(nint view, int state, nint data)
    {
        ...
        try
        {
            ...
            {
                _nonce = null;
                if (_saving is not null) { _ = _saving.CancelAsync(); }
            });
        }
    ...
    internal static string FailureText =>
        German
    ...
    private Task<bool> ConfirmOverwriteAsync(CancellationToken cancellationToken) =>
        DialogAsync(German ? "Die ausgewählte Datei wirklich ersetzen?" : "Replace the selected existing file?", true, cancellationToken);
    ...
    private async Task<bool> DialogAsync(string text, bool confirm, CancellationToken cancellationToken = default)
    {
        ...
        {
            Content = confirm ? DesktopText.Cancel : "OK",
            IsDefault = true,
            IsCancel = true
        };
        ...
        if (confirm)
        {
            ...
            {
                Content = German ? "Ersetzen" : "Replace"
            };
