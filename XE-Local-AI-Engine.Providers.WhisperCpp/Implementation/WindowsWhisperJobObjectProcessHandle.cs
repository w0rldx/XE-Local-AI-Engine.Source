...
internal sealed partial class WindowsWhisperJobObjectProcessHandle : IWhisperServerProcessHandle
{
    ...
    {
        // Closing the kill-on-close job terminates the whole tree. Idempotent: SafeHandle guards a double close.
    ...
    public static WindowsWhisperJobObjectProcessHandle Wrap(Process process)
    {
        ...
        catch (Exception ex)
        {
            ...
            TryKill(process);
            process.Dispose();
            throw new WhisperRuntimeException("The transcription runtime could not be contained for safe shutdown.", ex);
        }
    }
    ...
    [SuppressMessage("Major Code Smell", "S101:Types should be named in PascalCase", Justification = "Win32 interop struct — the name mirrors the native layout exactly.")]
