...
/// <remarks>
///     <para>
///         Recovery failure is fatal on purpose: continuing with an ambiguous adoption journal could expose an
///         unverified runtime as this node's transcription engine.
///     </para>
///     <para>
///         This is also what republishes the managed-runtime signal at start. The backend selector trusts a managed
///         CUDA build only once something has set that signal, so without this service an adopted build stops
    ...
    ILogger<WhisperCppSourceBuildLifecycle> logger) : IHostedService
{
    ...
    public async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        {
            // The host's shutdown token means "stop being graceful", not "throw". ShutdownAsync awaits the start gate
