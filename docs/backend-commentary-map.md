...
| Custom tools | Operator-authored HTTP/host-command definitions, validation, secrets, offering, approval, and execution. | `.../Services/CustomTools/**`, `.../Endpoints/CustomTools/V1/**`, `CustomToolStore`, `InvocationToolResolver` | Keep the node switch, enabled/acknowledged state, model locality, approval, SSRF validation, and direct host-execution boundary distinct. |
...
| Model runtime supervisor | In-app llama.cpp process supervision, prebuilt-binary resolution, GPU/CPU runtime detection, and model runtime operations. | `XE-Local-AI-Engine.Providers.LlamaServer/**`, `.../Services/Sandbox/**` (process sandbox provider) | Explain ownership, cleanup, and secret redaction for the host process the supervisor manages. |
...
- **gRPC is not on this list, because it does not exist in the tree.** It was deliberately removed with HostAgent — no `Grpc.*` package, no `.proto` file. A comment describing it as live is a defect, not a doc gap.
