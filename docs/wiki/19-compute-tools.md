...
1. **Deserialization** (`RunPythonToolHandler.ExecuteAsync`): the handler turns the model's JSON arguments into a `ComputeRunToolRequest` and delegates. It holds no copy of the kill-switch and no copy of the validation. **Gate** (`ComputeToolGateway.ExecuteDetailedAsync`, first two statements): the node kill-switch `Compute:Enabled` (default `false`) short-circuits, then the request is validated (non-empty, ≤20,000 chars). Both live on `IComputeToolGateway` rather than on the handler because a handler's copy is a property of one *caller*: a second caller reaching the gateway directly — a benchmark verifier, say — would get neither. One definition of each makes a bypass unrepresentable rather than merely discouraged.
...
**Docker**: this is not a choice `run_python` makes any more, and could not be made for it. Under [ADR 0007](../adr/0007-sandbox-execution-substrate-and-backend-selection.md) the compute workload's requirements are an engine-owned constant (`SandboxWorkloads.RunPython`: host toolchain, `SandboxIsolationMode.Filesystem` floor, no egress, disposable), and `SandboxSubstrateSelectionArchitectureTests` asserts that the *only* backend allowed to serve it is the process one — the container backend supplies an image toolchain compute never asks for, and the deterministic fake supplies no filesystem boundary. Beyond that, [ADR 0004](../adr/0004-development-mode-container-execution-docker-stopgap.md) explicitly excludes the chat/inference path from Docker with no fallback. Requiring a container daemon for every chat turn contradicts the product's driver-only footprint, and the containment Docker provides (toolchain supply, language isolation) is not needed here — only resource isolation and network denial, both of which the process sandbox already provides.
...
- **Schema compatibility**: `ComputeToolSchemaCompatibilityTests` confirms the parameter schema compiles through the GBNF sanitizer (parity with `LlamaGrammarToolOffer` tests). What a schema may carry on the llama.cpp wire — the 1024 repetition bound, why an over-bound keyword is stripped rather than clamped, and why an interior `^`/`$` in a `pattern` compiles to a literal that makes the validator reject every call — is [The grammar repetition bound](03-local-runtime-and-providers.md#the-grammar-repetition-bound). `ComputeToolDefinition` deliberately carries no `maxLength` for that reason; `CodeMaxLength` in the handler is the authoritative ceiling.
