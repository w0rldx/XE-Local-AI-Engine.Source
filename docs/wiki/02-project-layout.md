...
| Project | SDK / kind | Role |
|---|---|---|
| `XE-Local-AI-Engine.AppHost` | `Aspire.AppHost.Sdk` 13.4.5, `Exe`, `IsAspireHost` | **Dev-only orchestration.** `AppHost.cs` wires three resources: the `Client` app (`app`, https), the Vite React app (`client-react`, port 5175), and an encrypted SQLite resource (`node-sqlite`). It exposes Scalar/OpenAPI/DevUI URLs and health checks. There is **no Ollama and no llama.cpp resource** here — inference runs in-process inside `Client`, and the old HostAgent/Docker sandbox wiring was deleted (a comment in `AppHost.cs` records this). |
        ...
        Client (Web) ──────────► Client.Application ───┘
            ...
            └► Providers.Ollama ──┐   Providers.{Llama,HF,Codex,Capabilities,Ollama}
                       ...
                       Providers.Abstractions ◄────┴──┴──┘
                                  ▲
            Capabilities / CodexOAuth / HuggingFace / LlamaServer / Ollama
...
- **`Client.Application` is the hub of the product graph** — it references all six provider projects, `Client.Persistence`, `AI.Agent`, `ServiceDefaults`, and `AI.Contracts`.
...
- **Every provider references only `Providers.Abstractions`** (verified for `Capabilities`, `CodexOAuth`, `HuggingFace`, `LlamaServer`, `Ollama`). `Capabilities` is the only provider that another non-abstraction project depends on beyond the normal app/test edges.
...
- **`Directory.Build.props`** sets the single version source of truth (`VersionPrefix` `0.1.0`, `VersionSuffix` `rc.1`), `net10.0`, `Nullable`/`ImplicitUsings` enabled, `LangVersion latest`, and a **strict analyzer wall**: `TreatWarningsAsErrors=true`, `AnalysisMode=All`, plus `SonarAnalyzer.CSharp` everywhere. Production projects additionally get `Meziantou.Analyzer` + `Microsoft.CodeAnalysis.BannedApiAnalyzers` (with `BannedSymbols.txt`); test/tooling projects (`*.Tests`, `*.Testing*`, `*.AppHost`) are exempted via the `IsTestOrToolingProject` flag (note the deliberate `.Trim()` to survive XML reformatters). One practical gotcha: a literal `TODO`/`FIXME` in a comment fails the build (Sonar S1135 = error) — phrase deferred work as "... follow-up:".
- **`Directory.Packages.props`** enables **Central Package Management** (`ManagePackageVersionsCentrally=true` in `Directory.Build.props`); ~73 `PackageVersion` entries pin every dependency. Add new dependencies as a `<PackageVersion>` there, then a versionless `<PackageReference>` in the consuming `.csproj`.
- **`global.json`** pins SDK `10.0.0` (`rollForward: latestMajor`, no prerelease) and sets the test runner to **Microsoft.Testing.Platform (MTP)** — test projects are `OutputType=Exe` and run via MTP, not VSTest.
