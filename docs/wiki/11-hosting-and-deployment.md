...
`XE-Local-AI-Engine.AppHost/AppHost.cs` is a thin Aspire orchestration host (`IsAspireHost=true`). The AppHost SDK is 13.5.3. It references only the `Client` project and four hosting packages: `Aspire.Hosting.AppHost` 13.5.3, `Aspire.Hosting.JavaScript` 13.5.3, `Aspire.Hosting.Browsers` 13.5.3-preview.1.26425.3, and `CommunityToolkit.Aspire.Hosting.Sqlite` 13.5.0.
