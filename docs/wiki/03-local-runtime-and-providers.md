...
- **A turn's binding is pinned.** Tool authorization happens once, before the first send, from the connection's declared locality; a tool loop then sends many times. `ExternalProviderBindingPinScope` (an `AsyncLocal`, seeded in `InvocationRunner` beside the spawn context) records the generation, locality and origin the turn was authorized against, and `ExternalOpenAiChatClient` re-checks them on every send — a mid-invocation Local→Cloud flip or endpoint move throws `ExternalProviderBindingChangedException` instead of quietly redirecting already-authorized local tool results to the new endpoint.
