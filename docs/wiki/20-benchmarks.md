...
- **`POST benchmarks/projects/{id}/runs` answers 202 with the FIRST run of the group by design** — it is the one that starts, so it is the one an operator's live pane opens on; its siblings are reachable through its `repeatGroupId`, and `runs/batch` is the route whose response carries every created run id (`StartBenchmarkRunEndpoint`, `StartBenchmarkRunBatchEndpoint`).
- **Reasoning-budget enforceability is frozen off that same capability read** and is the conjunction `SupportsThinking && ReasoningBudgetEnforceable`. A model that does not reason at all cannot have its reasoning capped, and `GgufModelCapabilities` defaults `ReasoningBudgetEnforceable` to `true` — the inert answer for a model nothing was detected about — so freezing that one field stamped "enforceable" onto every non-thinking model: the budget went out on the wire, llama-server accepted and ignored it (no think-end tags in the template), and the `ReasoningBudgetSkipLog` notice that would have told the operator never fired, because the marker was written rather than skipped. "Enforceable" means the model's chat template renders a literal reasoning end marker; the answer is frozen rather than re-resolved at execution, so a run replays under the answer that was true when it was created, and `null` on a run frozen before the snapshot member existed reads as the inert `true` — never remove a cap that was working.
...
> **Accepted ceiling in the rank-cohort key.** `BenchmarkJudgeExecutionIdentity` hashes `RuntimeBundleIdentity`, which is the runtime's *cheap* identity — file names, sizes, mtimes and sampled validation hashes — plus the executable's full, freshly computed SHA-256. A shared library edited in an unsampled region with its size and mtime preserved would therefore alias onto the same identity. That is not an operational path: a runtime update moves the version, the sizes and the mtimes, all of which are in the identity. The upgrade path is a content-addressed bundle identity computed at install time, swapped in as a v2 field.
...
**Nothing hashes the prompt TEXT.** `ComputePolicyHash` covers `PromptVersion` (an integer), the rubric, the judge model identity, the sampling and the reference answer; the `JudgeExecutionKey` covers runtime and hardware identity only. So editing a word of `BenchmarkJudgePromptV2.SystemPrompt` changes what every *future* judging is asked while every version stays put, and verdicts taken either side of the edit share a policy revision and a cohort generation and are dense-ranked against each other after having been asked different questions. That is why the length-neutrality sentence bumped `PromptVersion` 2 → 3, forcing the existing `PromptVersionUnsupported` re-judge path — and why the truncated/incomplete notices are *appended* rather than folded into the base instruction, so a judging of a complete answer stays byte-identical to every judging that came before.
...
> **A comparison references two runs, and neither reference is a cascade.** `BenchmarkJudgeComparison` carries no declared relationship to either run, so nothing removes a comparison when one of its participants goes; the run's own children (`BenchmarkWorkItem`, `BenchmarkJudgeAttempt`, `BenchmarkFidelityAttempt`) declare `Restrict`, so with foreign keys enforced the run cannot be deleted until they are. `DeleteRunAsync` guards and deletes through `BenchmarkComparisons` on `RunAId == id || RunBId == id`, bumps each affected revision's `ComparisonSetVersion` and deactivates the project's active fits. The delete order is comparisons → work items → judge attempts → fidelity attempts → run; fidelity attempts carry an encrypted receipt, so skipping them is a leak, not untidiness.

> **A profiling refusal is waited out, never failed.** `LlamaServerProfilingRefusedException` is the same shape of blocker a capacity rejection is — a warm role for the model is serving a request right now, and the request ends on its own. Left to reach an executor's generic catch it terminalizes durable queued work as failed, under the generic invocation-failed message (the executors do not translate this type), over a chat that was about to finish; a primary/judge/comparison item has no second attempt behind that promise. Retrying repeats no work: the refusal is raised by the pre-spawn eviction, before anything is spawned and before the body runs, and it evicts nothing when it refuses. This wait is the more expensive of a phase's two — it holds the queue's shared GPU-work admission and the model lease like the capacity wait does, and **also** the capacity reservation, which is taken only once admission has succeeded — which is why `BenchmarkExclusiveSpawn` draws from the phase's remaining `BenchmarkAdmissionRetry` share instead of taking a budget of its own.
...
- **`scripts/dev-start.sh` points the node's GGUF registry at `${XDG_DATA_HOME:-$HOME/.local/share}/XE-Local-AI-Engine/models`.** A dev-stack live gate with a scratch `XDG_DATA_HOME` must export `HuggingFace__ModelsDirectory` at a populated store before `scripts/dev-start.sh`, or `benchmarks/eligible-models` is silently empty. A multimodal GGUF cannot be the judge even when the rubric never spawns one.
