...
- **A leaf with `countsTowardScore = false` is left out of the delta**, exactly as it is left out of the cell mean. The ranking excludes such an item from the mean by intersecting with the scorable id set rather than by nulling the run's score — the recall axis needs that score — so every other quality aggregate has to apply the same intersection itself. `sharedItemCount` therefore counts scoring items only.
