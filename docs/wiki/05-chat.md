...
Conversation titles and message content are **AES-encrypted at rest**, not stored in plaintext. `NodeChatDbContext` (`Client.Persistence/NodeChatDbContext.cs`) holds the node key via `INodeSqliteKeyHolder` and exposes `EncryptConversationTitle` / `DecryptConversationTitle` (`:72`, `:87`) and `DecryptMessageContent` (`:103`). Encryption goes through `NodePayloadProtector.Encrypt/Decrypt` with **per-record AAD**: for a title, `conversationId` is bound as both conversation and record id with column tag `"title"`; for message content, `conversationId + messageId + "content"`. An EF interceptor encrypts the entity path; the raw-SQL persistence path mirrors the same AAD scheme. (The "at-rest plaintext posture" phrase in the persistence facade's doc comment refers to the in-memory DTO surface the collaborators pass around — the stored blobs are ciphertext.) The node SQLite key itself stays local and is never returned to the browser — see [Security & privacy](12-security-and-privacy.md). Schema/migration details are in [Data & persistence](08-data-and-persistence.md).
