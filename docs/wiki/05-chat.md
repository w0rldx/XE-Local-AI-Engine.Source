...
Because a disconnected run keeps going, `RunInvocationAsync` also keeps holding that collision-slot lease until the runner finishes, so a disconnected mid-run turn keeps the slot alive. Accepted as-is for single-user local: at most one queued turn waits, then both persist correctly. If contended multi-session local ever matters, add an explicit disconnect→cancel path distinct from the SSE unsubscribe — do **not** free the slot from the SSE `finally`, which would resurrect the interrupted-terminal bug.
...
- Pick order: a chat model that a llama.cpp **Chat-role process already serves** wins first (`ILlamaServerProcessSupervisor.ListRunningProcesses`, an in-memory read: the persisted default if it is among the resident models, else the most recently used, tie-break by name), so a default send reuses loaded weights instead of asking the capacity gate for a second process. With nothing resident: the operator's persisted default **iff** it is an installed GGUF chat model, else the most-recently-modified chat model (tie-break by name, case-insensitive). Explicit model picks are unaffected.
...
- High-risk tools (the coder tools, `ask_user`, every MCP tool) are **permission-gated**: withheld unless the active model is in the `AgentHome:ToolCapableModels` allow-list (`LocalToolOfferProvider.IsToolCapable`). A null/unknown model id is treated as not-capable.
...
- `spawn_subagent`, `run_python` and `run_in_agent_home` are **profile-opt-in only** — never offered on the default/mode-off chat path; they are held out of the whole offer and added back only by `GetOfferedToolsForProfile[Async]`, and only for a model inside the trust boundary. See [Agent Mode](04-agent-mode.md#22-the-agenthome-write-back-loop) for the AgentHome offer rule.
...
- **Not locality-gated.** A tool-capable *cloud* model is still offered it: it carries the model's own question to the local UI, so no node-local document, file or workspace content leaves the node through it and the cloud-egress gate has nothing to withhold.
- **`RequiresApproval` is structural, not a risk judgement.** It is what routes the call through the runner's out-of-stream approval round-trip, where the human wait happens outside the stream-idle watchdog and outside the tool's own execution; a tool handler that simply blocked for a person would hold the turn until the whole-turn timeout, with no card asking anyone. The watchdog bounds provider silence only: while a requested or approved tool call has no result yet, a deadline re-arms instead of failing the turn, because each server-side tool carries its own bound (AgentHome's `MaxRunSeconds`, a tool's timeout). Tool time is likewise not charged to the model's turn budget, which pauses while a call is open and resumes with what was left. One consequence: a provider that emits a tool call before its final chunk and then stalls is bounded by the turn deadline, not the 60 s idle bound. It also means the three unattended paths (sub-agent, scheduler, delegate-scope inbound MCP), which strip every approval-required tool, strip this one for free — correct, since none of them has a person to answer. The integration coordinator is not one of them: it does not strip, it composes an explicit offer this builtin catalog never joins, so `ask_user` cannot reach an integration run at all. Agentic-scope inbound MCP is the deliberate exception — it may invoke approval-required tools through the strict audited auto-approval boundary, and there the handler's no-answer fail-safe returns immediately rather than creating a wait no unattended caller can satisfy.
- **`ToolCategory.ReadLocal`**, because it reads an answer from the node-local operator and has no side effect of its own.

`emit_output` uses the same seam from the other direction: it is held out of every projection and unioned in only by the integration coordinator, because delivering a payload to an external caller is a property of the *run*, not a per-agent permission.

### Model provenance and the auto-swap permission

`ChatTurnResolver` records not just *which* model runs the turn but *how it was chosen*, because `effectiveModel` keeps only the answer. `AllowAutoModelSwap` is true exactly when there was no explicit user pick **and** no honored agent pin: that is the only shape in which the turn ran on the node's default model with nobody asking for a specific one, and therefore the only shape the runner's reasoning-effort dispatcher may swap. An explicit pick and an honored pin are each a request for *that* model, so both clear the permission. `NodeChatStreamService` additionally clears it for any work-session turn: the graph was authored against a model, and a workflow step silently served by a different one is not a decision the graph's author made.

### Work-session write declaration

This is the runtime half of invariant **`GRAPH-C4-2`** (named in [API & hubs](09-api-and-hubs.md), embedded in the refusal the operator sees, and asserted on by name in the work-session and dev-workflow tests). A development-workflow Agent node that declares no `WriteExecute` capability arms a runtime refusal on every turn of the session it drives (`request.RefuseUndeclaredWrites` → `WorkSessionWriteDeclarationGuard.Refuse` in `NodeChatStreamService.SendMessageCoreAsync`). The earlier check at node dispatch resolves the definition separately and is the *friendly* refusal, not the enforcing one — between that check and the send, the definition can be widened, or deleted so the turn falls back to the default persona and its whole offer. The enforcing check therefore asks the rule of the **offer this turn will really hand the model**, resolved once and reused verbatim by the package: one resolution, one decision, nothing in between for an edit to land in. The supervisor turns the throw into the gate's own row and the session's terminal reason.

### Withdrawing `ask_user` from a workflow-owned session

`NodeChatStreamRequest.SuppressAskUser` sends a turn **without** the `ask_user` tool, overriding the "every tool-enabled turn can always ask its operator a question" invariant `AskUserToolOffer` otherwise holds. It is true only for a work session owned by a development-workflow node, because no operator is attached to one: that session's embedded chat is read-only — its lifecycle is driven through the workflow run, not the work-sessions surface — so no card exists to answer the question on. The question would then go unanswered for the full node-wide `WorkerNode:MaxPendingToolCallAgeMinutes` (10 minutes by default) before the model is handed a "no answer" result. `WorkSessions:MaxParkedSeconds` is meant to bound that wait sooner, but it did not fire in the live incident this flag was written for, so it is not a working fallback here. A tool the model never sees is a tool it cannot park on.

The withdrawal is scoped to the supervisor's step send. `NodeChatRegenerationService` and `NodeChatVariantBranchService` build their own packages and keep the tool deliberately: both are operator-initiated, so someone is there to answer. Withdrawing it **does** move a workflow-owned session's runtime-package config hash once, since `ask_user` leaves the hashed tool list — expect one resume invalidation for in-flight sessions on upgrade.

That offer is resolved early **only** for an armed turn. For every other send the offer read is part of the pre-ownership window (a client that disconnects while `GetEnableToolsAsync` is in flight must find a placeholder to terminalize), so an unarmed send resolves it in its usual place and moves not at all. An armed turn has no browser behind it, and refusing before the placeholder exists is what keeps a refused step from leaving a stranded row.
...
**Encrypted store** (`ConversationUploadedFileStore.cs`): the raw bytes and the cached extracted Markdown are encrypted on disk by `UploadedFileBlobProtector` (`UploadedFileBlobProtector.cs`) — AES-GCM (`nonce ‖ ciphertext ‖ tag`) keyed off the same `INodeSqliteKeyHolder` material as the DB columns, with per-blob associated data binding `conversationId + fileId + columnName` (distinct `file_bytes` / `file_md` column tags) so a blob can't be relocated. The bytes blob is `<fileId><extension>` and the Markdown companion is `extracted-<fileId>.md`, a name no upload extension can reach (the store still reads the legacy `<fileId>.md` companion); the Agent Home staging snapshot, like the chat and graph readers, stages only `Extracted` files. The original filename is encrypted into the DB row via `NodeChatDbContext.EncryptUploadedFileName`. See [Security & privacy](12-security-and-privacy.md).
...
**Live-validated behaviour (7/7 PASS on a fresh host, 2026-09-04).** Every
...
`auto` turn (previously the capacity snapshot kept listing the dead process). The codes are declared on
`IReasoningEffortDispatcher` and graded by `DefaultReasoningEffortDispatcher` (`ReasoningEffortDispatcherTests`);
the locality gate is `NodeLocalModelGate`, and the rejection message is the constant on
`NodeSettingsAdministrationService`. See also the runbook clause in
...
so a long conversation folds in few passes instead of many lossy ones). The per-call budget must also fit the rendered
...
`NodeChatPersistenceWriter` serializes every node chat write under a per-conversation reader-writer lock, so conversation-wide invariants — contiguous, unique message sequences and delete/purge atomicity — hold under concurrent sends, regenerations and deletes. The lock has three uses:

- **Conversation-exclusive** (writer) — sequence allocation plus message insert, the conversation lifecycle (create, ensure, rename, pin, archive, select-path) and delete/purge. It holds the writer side, so nothing else on that conversation runs concurrently, which is what makes `MAX(sequence)+1` allocation race-free and stops a delete interleaving with a message write.
- **Conversation-shared** (reader) — read-only conversation and message queries. Several run in parallel, and all are excluded against a conversation-exclusive op, so a read never observes a half-applied delete.
- **Message-update** — a payload UPDATE to an already-allocated row (streaming flush, queued, streaming, terminalize, cancel, feedback). It takes the conversation lock in *shared* mode plus a per-`(conversation, message)` exclusive lock, so updates to different messages run in parallel, updates to the same message serialize, and every update is mutually excluded against a delete through the shared/exclusive relationship.

The nesting order is always conversation lock first, then message lock, and the exclusive side never takes a message lock, so no cross-lock deadlock is reachable. Gates, and the per-message locks inside them, are reference-counted and removed the moment they fall idle, so the map is bounded by the number of *concurrently active* conversations rather than by every conversation or message ever seen.

### The turn-scoped read
...
Every terminalized assistant message also writes one content-free **run-envelope** row into `agent_execution_logs` (`NodeChatMessageCommands.WriteRunEnvelopeRowAsync`), in the *same* transaction as the terminal message row, so both commit or roll back together — never a swallowed best-effort write. It is metadata only: no prompt, completion or tool-argument content. Its terminal status, success flag and bound agent id are read from the winning persisted row *inside* that transaction, so the envelope can never disagree with the row it describes.

Two write modes reconcile it with an envelope the message may already carry, selected by `RunEnvelopeWriteMode`:

- **InsertIfAbsent** — `INSERT … WHERE NOT EXISTS` on `(record_kind, message_id)`. The first write wins, so a message already enveloped by a startup-recovery backfill, or by a race that terminalized first, is never duplicated or overwritten. This is what a mid-run cancel uses: a cancel is a terminal transition, so it writes its thin envelope in the same guarded UPDATE rather than leaving a cancelled row envelope-less until the next reconcile. There is no `InvocationState` at cancel time, so invocation id, tokens, duration and chunk counts are omitted.
- **Upsert** — `INSERT … ON CONFLICT(message_id) WHERE record_kind = envelope DO UPDATE`. The pump's authoritative terminalize is the winning outcome, so it enriches or overwrites a thin cancel envelope in place, keeping the envelope's `terminal_status` and tokens equal to the row's final status. The conflict target's `WHERE` mirrors the filtered unique index `ix_agent_execution_logs_envelope_message_id` so SQLite resolves it against that partial index; `id` / `record_kind` / `message_id` are the conflict keys and are not reassigned.

The `record_kind` rides the SQL as a literal, which must equal `(int)AgentExecutionLogRecordKind.ChatRunEnvelope` — a const string, not a runtime cast, so the statement stays a compile-time constant for CA2100. If it ever drifts from the enum the `ON CONFLICT` clause resolves against no index and SQLite throws, so the enrich-a-thin-cancel-envelope test fails loudly rather than silently.
...
`NodeChatVariantBranchService.BranchConversationAsync` copies a conversation up to a cutoff message into a new local thread, and the copy is a **linear** thread: exactly one revision per variant group, positioned at the group's **anchor** (its earliest member's sequence). That mirrors the frontend, which renders a variant group at its oldest sibling and lets any sibling be the active revision (`MessageRevisionGrouping.ts`).

Anchoring by group rather than by each message's own sequence is what makes late regenerations branch correctly: regenerating an early turn *after* later turns exist mints a sibling whose sequence lands past them, yet that sibling still belongs to the early turn and must branch at the early position. The same holds for the cutoff itself and for the caller-supplied selected-revision map, which is validated on the group anchor, not on the selected message's sequence. A group with no valid selection falls back to its newest member, and the branch-point turn always contributes exactly the cutoff message, overriding any caller entry for its group. Without the collapse the copied siblings would render as duplicate stacked assistant turns, since `variant_group_id` is dropped on copy.
