...
| `agentRuns` | **AgentHome run history** at `/agent-runs`, an ADVANCED-ONLY nav leaf at the end of the Automation group: one server-paged table over `agent-home/runs` (`useAgentRuns` + the shared `TablePaginationFooter`), one row per run with its node-minted id, start instant, outcome token, exported-patch summary and size on disk. A row whose run exported a patch opens the EXISTING `AgentHomePatchApplyDialog` (from `features/chat`) for that run id, so there is one review-and-land flow and one hash binding rather than two — the one tracked cross-feature edge this page adds. Read-only: deleting a run, deep-linking to its conversation and reading its logs are later slices. Like `/invocations` and `/usage` it carries no extra route guard and no capability flag: the authenticated `_layout` *is* the operator gate, and a node with AgentHome switched off simply has an empty history | [Agent Mode](04-agent-mode.md) |
...
| `chat` | Streaming chat UI, SignalR adapter, reasoning/tool-call rendering, sampling options, file-upload attachments + pane drag-and-drop (`usePaneFileDrop`, `ChatAttachmentChips`), manual compaction (`CompactButton`) and the read-only distilled-state drawer (`ContextStatePanel`) | [Chat](05-chat.md) |
...
| `development` | **Development Mode** operator surface at `/development`, surfaced under the "Preview" nav group (alongside Image Generation and Workflow Runs) because it is an experimental surface: project creation bound to a registered Git repository (`DevelopmentProjectForm`), the task/attempt workflow, and `DevelopmentLivePanel` streaming live attempt output over `DevelopmentAttemptHub` (`hooks/useDevelopmentAttemptHub.ts`). The page resolves the authenticated server capability (`development/capability`) before exposing projects or actions, so a backend kill switch fails closed without a separate frontend build | [Architecture Overview](01-architecture-overview.md#development-mode-registered-source-managed-worktree) |
| `devWorkflows` | **Development Workflows** at `/development-workflows` and `/development-workflows/{workItemId}`: the work-item list and create dialog, and a detail page whose run, node and tab selections live in **search params** rather than the path, so a view is linkable. Owns `DevWorkflowNodeRunTable`, the gate/intervention panel (approve, reject, request changes; retry, skip, abandon), the artifact and event tabs, the **rule-set catalogue** and the **definition form editor** (both tabs on the list page: `DevWorkflowRuleSetsPanel`/`DevWorkflowRuleSetDialog` write the policy documents a run injects into a node's objective, `DevWorkflowDefinitionFormPanel` edits a template's nodes and edges against `DevWorkflowDefinitionValidation`, a pure mirror of the graph rules the form can break — including the four capability invariants `deadGateEdge`, `strandedBranch`, `ungatedWrite` and `applyWithoutValidation`, whose shared fixpoint is keyed on `joinPolicy` and never on node type, since keying it on `Join` would reject the shipped template. The form authors an Agent node's `requiredCapabilities` as a typed effect set with a bounded reason, the graph-level `allowUngatedWrites` waiver and a node's `maxLoopIterations` beside its retry target, and shows every node's DERIVED effects as badges), and `useDevWorkflowRunHub` — one `devWorkflowChanged` notification invalidates the run's queries, so the hub never carries state the REST reads do not. The event feed pages **forward by sequence watermark** with `useInfiniteQuery`; sequences are not contiguous, so the cursor is the last sequence seen, never a row count. Both the list page and the detail page read `development-workflows/capability` before anything else and, on the `DevWorkflows:Enabled=false` a node ships with, render `DevWorkflowsDisabledAlert` — "disabled by this node's runtime configuration" — in place of every tab, firing no sibling request and never acquiring the run hub (the detail route is reachable by bookmark, where the family's bodyless 404 otherwise read as "this work item could not be loaded") — the same page-level pattern `DevelopmentPage` uses, and the reason a switched-off feature no longer reads as a broken one | [API & Hubs](09-api-and-hubs.md) |
| `externalApps` | **External Apps** at `/external-apps/catalog`, `/external-apps/installed` and `/external-apps/instances/{instanceId}` (`/external-apps` redirects to the catalog). The catalog page owns `RuntimeStatusCard` — the one place the container runtime is named, gated on the server's `requiresOperatorConfirmation` and posting the **observed daemon id** it displayed — and `InstallDialog`, a four-step disclosure (permissions → settings → resources → installing) whose acceptance is bound to the `(manifestVersion, manifestSha256)` pair the operator was shown: any later preview carrying a different pair clears it and returns to step 1. `PermissionsPanel` states network access as fact rather than denial (V1 enforces no outbound restriction) and renders the server's four application-level aggregates plus one block per part, never a union computed here. The installed list reads addresses and row actions straight off the widened `ExternalAppInstanceView[]`; the detail page is driven by `instance.manifest` alone, so **no catalog query is mounted on it**. Every lifecycle call echoes the row's `version` as `expectedVersion` (uninstall as a query parameter, it has no body) and Cancel is the only action offered while an instance is busy. `useExternalAppHub` subscribes one instance over `external-apps/hub`, reads only the ping's `sequence` — never its lowerCamelCase `kind` — and keeps pull progress in local state, because a query per layer would turn a multi-gigabyte pull into a request storm | [External Apps](23-external-apps.md) |
...
| `graphWorkflows` | **Graph Workflows** at `/graph-workflows` (one route; the definition, run, node and tab selections live in **search params**, so a view is linkable). A React Flow editor for the eight v1 node kinds (`Start`, `Agent`, `Tool`, `Condition`, `Parallel`, `Join`, `Pause`, `End`) plus a read-only run view over the pinned graph — node runs, the Pause decision panel, node input/output documents and the event trail — and `useGraphWorkflowRunHub`, whose content-free `graphWorkflowChanged` ping invalidates the run's queries. Ships **on** (`nodeCapabilities.graphWorkflows`) as a **top-level** navigation entry. `useUnsavedChangesGuard` is set with `allowSameRoute`, so writing a search param does not trigger the leave prompt while a real route change still does | [Graph Workflows](21-graph-workflows.md) |
| `images` | Local text-to-image at `/images`, surfaced under the "Preview" nav group (alongside Development Mode and Workflow Runs) because the runtime is not yet confidently verified end-to-end: generation form, job list/cards, result view, installed-model manager, plus `useImageJobHub` (subscribe-then-replay, dedupe on `seq`) and `useImageObjectUrl` (fetches the encrypted PNG as a `Blob`) | [Image Generation](14-image-generation.md) |
...
| `transcription` | **Audio Transcription** at `/transcription` and `/transcription/{sessionId}`, under the "Preview" nav group: the session list and create/upload dialog, and a detail page rendering the committed transcript with a channel badge for any non-`Mono` segment. The audio upload is the one hand-written multipart call in the feature — `axiosInstance.post` with `Content-Type: multipart/form-data` and an upload-progress callback, following `features/knowledge/…/useKnowledgeUpload.ts`, because the generated client expresses no upload progress. "Send to chat" hands the transcript to the composer through `core/ui/stores/PendingComposerTextStore.ts`, a Zustand store whose `setPendingText`/`consume` pair exists to carry text **across a navigation**; dictation (a later slice) appends in place and deliberately does not use it. **Live capture** lives under `capture/`: `CaptureSource.ts` is the seam (`CaptureSource`, `CaptureFrame`, `CaptureError`/`CaptureErrorCode`, `listAudioInputDevices`), implemented by `MicrophoneCaptureSource` (`getUserMedia`) and `SystemAudioCaptureSource` (`getDisplayMedia`, whose video tracks are stopped immediately — the picture is never read). Both share `PcmCapture.ts`: one `AudioContext` per stream, `Pcm16DownsamplerWorklet.js` on it, and `source → worklet → zero-gain → destination`. The worklet is a plain `.js` file imported for its URL alone with Vite's `?url`, which puts it outside `tsconfig.json`'s `include` and knip's project set and means it must stay free of imports. `hooks/useTranscriptionHub.ts` owns the hub connection — `useTranscriptionHub` for the handle, `useLiveTranscript` for the push-fed query read — and `capture/useLiveCapture.ts` orchestrates the two, calling the hub hook itself so a page mounts exactly one. `components/LiveTranscriptPanel.tsx` and `components/CaptureControls.tsx` render the live half; `stores/TranscriptionCaptureStore.ts` is UI-only and persisted (last source kind, last device id, whether provisional text is shown) and never holds transcript data | [Audio Transcription](24-audio-transcription.md) |
...
| `workSessions` | Long-running agent work sessions at `/work-sessions` and `/work-sessions/{id}`: the list + create dialog, and a 3-pane detail page whose centre pane **is** `features/chat`'s `Chat` component. Owns `useWorkSessionHub` and the query hooks over the generated work-session operations. The list page and the detail page both read `work-sessions/capability` first and render `WorkSessionsDisabledAlert` — "disabled by this node's runtime configuration" — when an operator has turned the feature off, gating every query and the hub on it — the same page-level pattern `DevelopmentPage` uses. The one feature that deliberately composes another feature's page — see [Work Sessions: the chat-embed seam](#work-sessions-the-chat-embed-seam) | [Agent Mode](04-agent-mode.md) |
...
- `raised` is what a dialog opened **over another `DialogShell`** sets. Mantine portals every modal into one shared node, so two dialogs at the same z-index are ordered by insertion and the one underneath wins whenever it was remounted last — a `key` on the outer editor is enough, and the confirmation then renders behind the thing it is confirming. `DialogShell` owns the one raised layer; a call site passes the flag and never a z-index literal. `ConfirmProvider` and the benchmark re-judge confirmation are the two that set it.
...
Dates follow the **UI language, not the browser locale**. `formatTimestamp` and its time-only sibling `formatTime` (`core/formatting/TimeFormatting.ts`) read the active i18next language and format with it, so a session switched to German no longer renders US-ordered dates next to German labels. Both accept `number | string | null | undefined`, because half the wire carries epoch millis and half an ISO string and most generated timestamp fields are optional, and both go through one private guard so the language rule cannot drift between them. **Every** date and clock rendering goes through one of the two rather than a bare `toLocaleString()`, which knows nothing about i18next — feature-local copies of the same formatter are gone, `features/devWorkflows` included. The helper answers a dash for an absent or unusable value instead of printing "Invalid Date" into a table row, and falls back to the environment's default when a hand-edited `i18nextLng` is not a well-formed tag; an absent instant is never coalesced to `?? 0`, which would date the row to 1970. Both take an optional `Intl.DateTimeFormatOptions`, which is how the four sites that render a part of an instant (the chat clock, the conversation-list day, the model-fit catalog release date and the usage dashboard's day label) moved here rather than growing a third formatter: naming any date field suppresses `toLocaleString`'s clock defaults, so those sites kept their output character for character while gaining the language rule. It is why `core` may import i18next directly, the same reason `ApiErrorMessage` and `Toast` do.
...
- [Agent Mode](04-agent-mode.md) — agents, playbooks, skills, tools.
- [Graph Workflows](21-graph-workflows.md) — the `graphWorkflows` feature, its editor, run view and hub hook.
