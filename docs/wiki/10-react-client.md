...
Dates follow the **UI language, not the browser locale**. `formatTimestamp` (`core/formatting/TimeFormatting.ts`) reads the active i18next language and formats with it, so a session switched to German no longer renders US-ordered dates next to German labels. New date rendering goes through that helper rather than a bare `toLocaleString()`, which knows nothing about i18next; the helper also answers a dash for an absent or unusable value instead of printing "Invalid Date" into a table row, and falls back to the environment's default when a hand-edited `i18nextLng` is not a well-formed tag. It is why `core` may import i18next directly, the same reason `ApiErrorMessage` and `Toast` do.
