...
Route paths and their capability flags are declared centrally in `src/capabilities/NodeCapabilities.ts` (`nodeRoutePaths`, `nodeCapabilities`), not scattered across route files. Capability-flagged pages (`agents`, `skills`, `mcp`, `scheduler`, `modelFit`, `loadedModels`, `knowledgeBase`, `images`, `development`, `benchmarks`, `training`, `workSessions`, `devWorkflows`, `graphWorkflows`, `integrations`, `externalApps`) are all **on** by default; a page whose flag is off redirects home and has its nav entry filtered out. Custom Tools shares the `agentManagement` capability, while `commands`, `usage`, and `diagnostics` are authenticated but otherwise ungated. `development` additionally re-checks the *server* capability at runtime — a frontend flag alone is not treated as authoritative.
