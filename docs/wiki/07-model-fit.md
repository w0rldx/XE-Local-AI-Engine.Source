...
- **Catalog lane (`BuildCatalogRecommendationsAsync`, primary)** — delegates to `ICatalogRecommendationService` and supplies the `"recommended"` and `"canRun"` sections, each capped at the request limit.
- **Explore lane (`BuildExploreRecommendationsAsync`, secondary)** — the live-HF two-pass discovery described [below](#the-refresh-service-the-advisor), tagged `"explore"`.

A catalog-lane failure (`HttpRequestException` / `IOException` / `TimeoutException` / `InvalidOperationException`) is caught, logged, and degrades to an **empty catalog section** — it never fails the run, because the explore lane alone is still a useful result.

**The catalog itself.** `IModelCatalogProvider` → `ModelCatalogProvider` (`Catalog/Implementation/`, **Singleton**) loads the bundled `Catalog/model-catalog.seed.json` at construction (`ModelCatalogBundledLoader.Load`) and serves that in-memory snapshot on every read. `ModelCatalogOptions.RefreshUrl` is **empty by default, and an empty URL means the provider never makes a network call** — out of the box the catalog is bundled-only. When an operator points it at a hosted catalog JSON, a read past `RefreshTtl` (default 24 h) re-fetches it under a `SemaphoreSlim` gate with a per-fetch `FetchTimeout` (default 20 s), and the fetched document is adopted only if `ModelCatalogValidator` accepts it; `IModelCatalogProvider.RefreshAsync` (`POST model-fit/catalog/refresh`) forces a fetch, bypassing the TTL debounce. A failed fetch or validation **never regresses a working cache**: an already-effective remote snapshot is kept as-is, else the persisted last-good remote JSON (`IModelCatalogCacheStore`), else the bundled seed. A successful fetch both replaces the snapshot and persists the raw JSON, so an offline restart still serves last-good-remote rather than dropping back to bundled.

**Ranking (`CatalogRecommendationService.BuildRecommendationsAsync`).** Entries are filtered by the requested use-case and by `ModelCatalogArchGate.Supports` — the entry's `minLlamaCppTag` against the node's **installed-else-pinned** llama.cpp tag, so a model the current runtime cannot load is never offered. Each survivor's GGUF repo is inspected (`IHuggingFaceGgufDiscovery.InspectRepoAsync`, TTL-cached, bounded concurrency, per-repo timeout; one stalled or failing repo is skipped, never fatal) and walked down the [quant ladder](#the-quant-ladder--quality-tiers) with `MemoryFitEstimator` — fed `MoeFacts` built from the entry's curated `activeParamsB` in preference to the header's expert fields, which are not always present on a quantized file — keeping the best-fitting quant. Survivors order by curated tier (`S` → `A` → `B` → other), then expert-offload class, then quant quality, then a KV-sizing tiebreak, then release date, then id. A candidate is **Recommended** iff its quant is at or above `Q4_K_M` quality **and** `HeadroomBytes > 0`; everything else that fit is **Can run**.
...
4. **Profile** hardware (`IHardwareProfiler.GetProfileAsync`), then `BuildRecommendationsAsync`: list the already-downloaded GGUF keys once (best-effort; a registry failure just marks everything not-installed), run the [catalog lane](#the-curated-catalog-lane-primary-recommendation-source), then the **explore lane** over the same installed-key set:
...
| `SourceBuild*` (5 constants + `SourceBuildHub`) | `model-fit/llamacpp/source-build(/prerequisites\|status\|cancel\|remove)` | `Start`/`Cancel`/`Remove LlamaCppSourceBuildEndpoint` · `GetLlamaCppSourceBuildPrerequisitesEndpoint` · `GetLlamaCppSourceBuildStatusEndpoint` | The Linux in-app source-build lifecycle (official pin, custom repo, or explicit 40-hex commit). A blocked start answers a **typed 409 domain object**, not ProblemDetails — see [API & hubs](09-api-and-hubs.md). Progress rides `SourceBuildHub`; a source build is also the only runtime that ships `llama-quantize`, which merged [training](18-training.md) exports need. |
