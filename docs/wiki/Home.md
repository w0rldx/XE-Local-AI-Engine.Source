...
SignalR hubs, persists chat/agent state in **encrypted SQLite**, and runs the local model runtimes
**in-process** by supervising host child processes — **llama.cpp** (`llama-server`) for text, and
...
worktree — 15 `development/*` endpoints, a live-attempt hub, its own React feature, and two accepted ADRs
...
| 09 | [API & Hubs](09-api-and-hubs.md) | FastEndpoints `/api/local/v1` (23 route families), the nine SignalR hubs, WorkerHub, OpenAPI→hey-api |
