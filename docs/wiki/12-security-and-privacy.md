...
### 7.1 The isolated launch mode (`SandboxIsolationMode.Filesystem`) — opt-in, and not yet consumed
...
**Status: the mechanism is built and probed; no caller opts in yet.** AgentHome, Coder and Development Mode create sandboxes without naming an isolation mode and therefore run the byte-identical chain they always did (asserted by `SandboxFilesystemIsolationContractTests`). Wiring `ComputeToolGateway`'s `run_python` to it is a separate change. Nothing on this page's default posture has moved.
