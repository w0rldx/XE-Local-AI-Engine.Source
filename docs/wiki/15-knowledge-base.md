...
The Knowledge Base is a **fully offline** document store with hybrid retrieval. An operator uploads documents; the node extracts their text, chunks them on header boundaries, embeds each chunk with a local embedding model, and indexes everything into **encrypted SQLite**. Retrieval fuses a **lexical** arm (SQLite FTS5 / BM25) and a **semantic** arm (vector cosine similarity) with Reciprocal Rank Fusion, optionally rescoring with a **local cross-encoder reranker**, and exposes the result to agents as a tool.
...
**Attachments and workspace file tools are per-conversation user consent, not locality-gated.** Files a user attaches to a specific conversation (inlined in plain chat, or staged into the AgentHome workspace and read via the coder `list_files`/`read_file`/`search_text` tools) are content the user deliberately supplied to *that* conversation, whose model they chose. That is treated as explicit per-conversation consent, so attachment/workspace content is **not** subject to the knowledge-tool cloud gate — unlike the Knowledge Base, which is a persistent, shared, node-local corpus a user did not necessarily intend to expose on any given send. Attachment content is still fenced as untrusted data (file name inside the fence) so it is never read as instructions.
