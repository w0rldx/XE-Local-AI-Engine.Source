...
Development tool path checks protect every path whose first segment starts with `.`, except the dot-entries a repository tracks and expects contributors to change (`.github/` and the root dot-files `.dockerignore`, `.editorconfig`, `.gitattributes`, and `.gitignore`); review and apply paths revalidate patch/base evidence before changing the registered source.
