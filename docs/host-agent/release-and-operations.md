...
> **RC1 note — updated 2026-06-10 (supersedes this section for RC1):** Decision D1′ (locked 2026-06-10, `Plans/2026-06-10-windows-installer-cli-plan.md §2`) further refined the RC1 Windows distribution format: the install surface is a **dedicated installer CLI (`xe-installer.exe`)** shipped inside a self-contained ZIP, not a bare PowerShell script. The zip contains `xe-installer.exe` (self-contained .NET 10 win-x64 single-file exe) plus a `payload/` subtree (rootfs tar, app image tar, host-agent binaries, runtime manifest, vendored scripts). MSI and deb/rpm packaging remain the GA target and the requirements below still apply from RC2 onward. The RC1 ZIP prerequisites are documented in `Plans/2026-06-10-rc-zip-prereq-decision-note.md`. The packaging script is `ci/host-agent/build-rc-zip.ps1`; the tester quick-start is `ci/host-agent/bundle/README-TESTER.md` (copied into the bundle root at build time).
