...
- **Provider unload is all-role, while warm/runtime-info probes are deliberately chat-only.** `LlamaCppProviderService.UnloadModelAsync` must call the supervisor's enum-complete `EvictAllRolesAsync`; otherwise embedding and reranker processes survive an operator unload. Do not replace chat-only warm/runtime-info behavior with all-role iteration: those calls describe the interactive chat runtime, not the provider-wide teardown contract.
