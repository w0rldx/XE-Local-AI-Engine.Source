...
The status is a property of the **endpoint surface**, not of the throw site — the same guard in `DevelopmentRepositoryBindingService` fires for a folder the caller just supplied (register/create-project/detect-profile/reconnect → **400**) and for the folder a project was bound to months ago (start-next-action/preview/apply, whose requests carry no folder at all → **409**, the request is fine and the stored state blocks it). Throw sites that are *only ever* about persisted state — trust acknowledgement no longer current (`DevelopmentTrustPolicy`), "must be reconnected", stored repository identity mismatch, apply-subject mismatch — throw the derived `DevelopmentRepositoryStateConflictException` so `ReconnectDevelopmentRepositoryEndpoint`, the one endpoint that is both surfaces at once, can answer 409 to those and 400 to "the folder you picked is not a Git root". It derives from the base type on purpose (like `SelectedFolderConflictException` does): every service-layer `catch (DevelopmentWorkspaceSecurityException)` — availability probing, profile backfill/summary degradation, attempt-runner failure reasons — keeps working unchanged. Pinned by `DevelopmentEndpointTests.Operation_When*`.

### Knowledge repository import: only `…ImportRejectedException` is a 400

`KnowledgeRepositoryImportService` throws `KnowledgeRepositoryImportRejectedException` for what the caller can fix (past a configured file-count/byte bound, repository unavailable) and `KnowledgeRepositoryReadException` for what only the host can (Git index unreadable, file could not be opened, file changed under the reader). `ImportKnowledgeRepositoryEndpoint` catches the first and deliberately not the second — a bare `catch (… or InvalidOperationException)` there used to echo I/O failures as 400 client errors. Don't widen it back.
