...
- **The `pgrep` guard against overlapping builds matches itself.** `pgrep -f "dotnet (build|test)"` matches the zsh wrapper of your OWN backgrounded command, whose command line contains that literal string, so a "wait until no build is running" loop waits forever. Use `pgrep -f "dotnet-root/dotnet (build|test)"`, which matches only the real process.
