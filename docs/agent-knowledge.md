...
**Rule:** when a scan-shaped architecture test asserts a minimum scanned count, obtain that number by temporarily raising the floor to an absurd value, building Release and reading the count out of the failure message. A grep over source declarations counts a different population: NetArchTest counts types in the compiled assembly, and nested and local declarations carry no namespace of their own, so grep over-counts them. **Failure prevented:** three floors written from the grep proxy sat **above** the real count and turned the guard red on a clean tree. Measured on 2026-09-16, the assembly counts for `Providers.LlamaServer`, `Providers.HuggingFace` and `Providers.WhisperCpp` came in roughly an eighth below their grep figures. **Authority:** S1 regression-guards slice, `LayerDependencyTests.AssertTypesScanned`, 2026-09-16.

### A ratchet allowlist is keyed by a composite fully-qualified string, and the forbidden-type walk recurses

**Rule:** the allowlists in `Architecture/` tests are keyed by a composite, fully-qualified string, not by a bare simple type name. `EndpointDependencyTests` keyed on `"{EndpointTypeFullName}|{ParameterTypeFullName}"` while it still had a list (S6m emptied and deleted that list; `ThirdPartySdkBoundaryTests` and `ProviderTelemetryWrapGuardTests` still carry their own), because a bare endpoint-type key would hide a *second* forbidden dependency on an already-listed endpoint, and a simple-name key could collide across endpoint areas. The forbidden-type search in that test is also recursive into generic arguments, array element types and `Nullable<T>`: `IOptions<CodexOptions>` counts as a dependency on `CodexOptions`, not merely on `IOptions<T>`'s own allowed namespace. **Failure prevented:** stopping at the outer type silently exempts every forbidden type wrapped in an allowed generic. **Authority:** S1 regression-guards slice, 2026-09-16. Read the actual key shape and recursion behaviour out of the test before writing anything that edits an allowlist; never assume either from another slice's plan.
...
**Rule:** new tests in `XE-Local-AI-Engine.Tests/Architecture/` hit analyzer errors that Debug never shows. `IDE0042` refuses `var result = Scan();` over a named tuple and wants `var (violations, count) = Scan();`. `S3878` refuses a collection expression passed to a `params` parameter, so `LoadAssemblies([a, b])` must be `LoadAssemblies(a, b)`. `ArchUnitNET.Loader` declares its own `Type`, which collides with `System.Type`, so import the loader as `using ArchLoader = ArchUnitNET.Loader.ArchLoader;` rather than by namespace, the same shape `PlacementConventionTests` already uses for `Assembly`. **Failure prevented:** three separate Debug-green iterations handing the Release gate a red it never saw. **Authority:** S1 regression-guards slice, lanes A and B, 2026-09-16.
...
### Refresh rotation's grace window is discriminated by the SUCCESSOR's instant, never by "revoked recently"

**Rule:** `NodeAuthService.RefreshAsync` honours a token that rotation replaced for ten seconds. `revoked_at_utc` is the ONE column every revoke path writes — logout (`RevokeRefreshTokensAsync`), `ChangePasswordAsync` and `ResetAdminPasswordAsync` all use it — so "revoked, but recently" would resurrect a logged-out session. The discriminator is that rotation stamps the revocation and its replacement from a SINGLE clock read (the caller threads `now` into `CreateTokenResultAsync` and `RevokeActiveTokensAsync`): a live token whose `CreatedAtUtc` **equals** the presented token's `RevokedAtUtc` can only be rotation's own successor. `>=` is not equivalent — `LoginAsync` also revokes-all-then-issues, so a login after a logout creates a token "after" the revocation and would vouch for the pre-logout cookie. The grace path must not re-stamp `RevokedAtUtc`, or replaying every nine seconds walks the window forward forever. The match therefore rests on a third assumption that nothing in the schema enforces: that two UNRELATED writes for one user — a logout's `RevokedAtUtc` and a later login's `CreatedAtUtc` — never carry the identical instant. Only clock granularity separates them (in practice two HTTP requests with a password verification between), and `Refresh_WhenLogoutIsFollowedByAFreshLogin_FailsForThePreLogoutToken` advances the clock between the two rather than proving a same-tick collision impossible. Two further limits are inherent, not bugs to fix: only the token HASH is stored and the filtered unique index allows one live token per user, so the grace path must rotate again rather than hand back the winner's token — therefore (a) a THIRD presenter of the same original token inside the window fails, because the grace rotation revoked the first successor and the live token's `CreatedAtUtc` no longer equals the original `RevokedAtUtc`; and (b) if the winner's and loser's `Set-Cookie` responses reach one browser out of order, the jar keeps the first successor, which is itself only honoured for its own window — a refresh inside it recovers the session, a refresh after it fails. **Failure prevented:** a fix for "the second tab gets logged out" silently turning a logged-out refresh cookie back into a live session; and, on the other side, someone "fixing" (a) or (b) by widening the match or re-stamping the revocation, which is exactly the hole. **Authority:** `XE-Local-AI-Engine.Tests/Auth/NodeAuthRefreshRotationGraceTests.cs`, 2026-09-17 — a naive grace with no successor check reds three of its tests, and the `>=` form reds exactly `Refresh_WhenLogoutIsFollowedByAFreshLogin_FailsForThePreLogoutToken`.
...
Only `LlamaCppSourceBuildServiceTests` mutates that parent environment variable, and it is exclusive. Other matches seed a host configuration dictionary, set a **child** `ProcessStartInfo.Environment`, or mention the name in comments; they do not mutate the parent. Audit the write site before adding serialization.
...
### Live transcription backpressure measures lag, and a lane that is behind skips ticks
...
- **The device probe never downloads a runtime, on any external-access profile.** `LlamaDeviceInventoryProbe` resolves through `ILlamaCppBinaryManager.TryGetInstalledBinaryAsync` (read-only: no network, no directory creation, no `installed-runtime.json` write) and reports `runtimeMissing` when nothing is installed. Prevents: the SPA app shell's `GET model-fit/hardware-profile` — mounted on every authenticated page by `CpuFallbackBanner` — fetching a multi-hundred-MB llama.cpp runtime within a second of an operator choosing **Offline / Manual** on the onboarding screen (measured on a fresh node 2026-09-18; the three `ExternalAccessGate`-protected services were correctly gated and were not the source). Authority: owner ruling 2026-09-19. Acquisition belongs to the explicit paths only (ensure endpoint, spawn, first-run provisioning). Any new automatic caller of `EnsureBinaryAsync` is the same defect.
...
- Helper tools (`llama-quantize`, `llama-perplexity`) are resolved **by name beside the resolved `llama-server`** (`LlamaCppToolBinaries`, surfaced as `LlamaBinary.QuantizerExecutablePath`/`PerplexityExecutablePath`, evaluated on read). Presence is never a precondition for adoption — a runtime missing one still serves; the caller that needs it fails specifically. The prebuilt archives carry `llama-perplexity` but no `llama-quantize`. **The source-build path only gained `llama-perplexity` when its cmake `--target` list was widened (`LlamaCppSourceBuildService`, and the since-deleted CUDA-only build service beside it) — any tree built before that, including a BYO `~/cuda-llama/<tag>` tree, has `llama-server` and nothing else, so benchmark fidelity there must rebuild the runtime or point at a prebuilt.** (The deleted CUDA-only service never built `llama-quantize` either.)
...
**Rule:** once `LocalModelProxyForwarder` has written the upstream status, the status is spent — no failure after that point may become a 503, and none may be left to propagate. `PumpWithIdleDeadlineAsync` therefore ends the exchange itself for the "the child is gone" family, matched with `DeferredLlamaServerChatClient.IsServerGone` (the provider's own matcher, reached through an `InternalsVisibleTo` grant rather than a second copy of the match). An event-stream response gets ONE OpenAI error envelope frame — the same `message`/`type`/`code` shape the pre-header `WriteErrorAsync` writes — followed by `data: [DONE]`, and never a `finish_reason: "stop"` chunk, which would tell the caller a generation completed that did not. Any other body shape is aborted, like the idle-deadline arm, because a half-written JSON document cannot be repaired into a valid one. The terminal write itself is guarded against the caller also being gone. The forwarding client must also carry NO resilience pipeline (`AddNodeModelProxyExtensions.AddNodeModelProxy` strips the one Aspire installs on every named client): with `ResponseHeadersRead` a non-streaming completion sends no headers until generation ends, so the whole generation sits inside one attempt of that pipeline — its attempt timeout then severs a healthy long generation and answers 500, its blanket retry re-runs a non-idempotent inference POST, and the `TimeoutRejectedException` it raises is neither `HttpRequestException` nor `IOException`, so it escapes even the pre-header 503 arm (guard: `LocalModelProxyResilienceTests`). **Prevents:** a forced eject — an expected operator action, not an error — surfacing as an "Unhandled exception" ERROR from the ASP.NET pipeline while the external tool's SSE stream simply stops mid-token, with no error frame, no `[DONE]`, and nothing in the response that says why. **Authority:** `LocalModelProxyForwarderTests.ForwardChatCompletions_WhenTheUpstreamDiesMidEventStream_EndsWithAnErrorFrameAndDone` and `…_WhenTheUpstreamDiesMidNonEventStream_AbortsInsteadOfWritingAFrame`; the family itself is pinned by `DeferredLlamaServerChatClientServerGoneTests`.

### Every factory client is born with Aspire's resilience pipeline; a non-idempotent or long request must strip it

**Rule:** under Aspire (`ASPIRE_ENABLED`, dev only), `ServiceDefaults` installs `AddStandardResilienceHandler()` on EVERY `IHttpClientFactory` client — named, typed and default, registered before or after — through `ConfigureHttpClientDefaults`. Its retries cover every HTTP method and its attempt timeout is far shorter than local inference. So: a client that sends a **non-idempotent** request (an inference or rerank POST, a single-use OAuth grant, a job submit) or a **long** one must own its pipeline — `RemoveAllResilienceHandlers()` under a reasoned `#pragma warning disable EXTEXP0001`, with a strip-then-add-one-POST-safe-pipeline shape. An **idempotent** download/catalog GET deliberately KEEPS the pipeline (llama.cpp's release catalog and binary manager share the default client for exactly that reason) — say so in a comment, or the next reader reads the keep as an oversight. **Stripping also takes the client's deadline with it:** `AddStandardResilienceHandler` sets `HttpClient.Timeout` to `Timeout.InfiniteTimeSpan` because its pipeline owns the deadline, and `RemoveAllResilienceHandlers` removes the HANDLER, never that mutation — so a client that only strips runs under Aspire with no pipeline AND no timeout. Set one explicitly (infinite only where every call site provably owns a linked-token deadline), above whatever per-call budget should fire first. Chat, embedding, Ollama and cloud transports are built with `new HttpClient`, never from the factory, so they never receive it and never need the strip. A guard test must register `ConfigureHttpClientDefaults(http => http.AddStandardResilienceHandler())` BEFORE the production registration and assert on the handler chain; one that omits it passes whether or not the strip was ever called. **Prevents:** a retried POST billing the operator's GPU twice or replaying a rotated refresh token into `invalid_grant` and a forced re-login; an attempt timeout severing a healthy long request and raising `TimeoutRejectedException`, which is neither `HttpRequestException` nor `IOException` and so escapes the catch sets every caller has. **Authority:** `LocalModelProxyResilienceTests`, `LlamaServerRerankerResilienceTests`, `CodexOAuthTokenEndpointResilienceTests`, `WhisperRuntimeHttpTimeoutTests`, `StableDiffusionCppRuntimeResilienceTests`.
...
- Linux publish is self-contained single-file. Windows is framework-dependent, requires x64 ASP.NET Core Runtime 10.0.12+, and must not ship runtime binaries/licenses. Client trimming remains off.
...
### A GPU-vendor check is not a device check: pick a CUDA prebuilt only after `ICudaDeviceProbe` agrees
...
`Microsoft.Agents.AI`, `Microsoft.Agents.AI.Abstractions` and `Microsoft.Agents.AI.Workflows` move as one cohort (`Directory.Packages.props`). A patch bump may land on CI alone. **Any minor bump of the trio, and any bump at all that moves `Microsoft.Agents.AI.Workflows`, requires all three of the following before merge:** run `HandoffWorkflowSpikeTests`, `FrameworkApprovalGateTests` and `AgentSkillsProviderContractTests` and name the result; re-run the O1 workflow-backbone axes and re-record the verdict against the new version; re-read the MAF traps above and correct anything the bump made pin-wrong. Prevents the failure already in the tree — `8459cb962` fixed a `ChatClientAgent` positional-argument order that changed under an unaccompanied Dependabot merge — and prevents O1's premise going stale unnoticed, which is exactly what 1.17.0 → 1.20.0 did. Authority: ledger D13, operator ruling 2026-09-09. Re-run for 1.20.0 → 1.21.0 on 2026-09-17: `HandoffWorkflowSpikeTests`, `FrameworkApprovalGateTests` and `AgentSkillsProviderContractTests` passed, an `ilspycmd` diff of the two net10.0 assembly sets showed `Microsoft.Agents.AI.Workflows` and `Microsoft.Agents.AI.Abstractions` identical in public surface (version strings only) with `Microsoft.Agents.AI`'s single break being PR #7671 (`file_access_read_lines`; `AgentFileStore.SearchAsync` abstract→virtual, `AgentFileSkillResource` ctor gained an internal-typed parameter), and all four O1 workflow-backbone axes are unchanged — O1 stands at 1.21.0.

### `Microsoft.Extensions.AI.OpenAI` and the plain `OpenAI` SDK move together, and NuGet will not tell you

**Rule:** treat `Microsoft.Extensions.AI.OpenAI` and `OpenAI` as one cohort in `Directory.Packages.props`. Raise the `OpenAI` pin only to the version the adapter release was built against — never merely to something the nuspec floor allows. **Failure prevented:** MEAI 10.10.0 declares `OpenAI >= 2.13.0`, so NuGet resolves 2.14.0 without a warning and the whole solution compiles clean in Release; at run time `OpenAIResponsesChatClient.ToResponseTool` references `OpenAI.Responses.GlobalMcpToolCallApprovalPolicy`, which 2.14.0 deleted, so **every Responses call carrying `ChatOptions.Tools` throws `TypeLoadException`** — the entire Codex tool loop, silently, with a green build behind it. **Diagnosis tell:** the request never reaches the transport, and a test that swallows the send exception reports only the missing body. Unwrap the exception before blaming the wrapper. **Authority:** `CodexToolCallingWireTests.FirstTurn_WithTool_WrapperAddsEncryptedReasoningInclude_AndKeepsTool` is the guard that caught it; 2026-09-17 dependency round, which reverted `OpenAI` 2.14.0 to 2.13.0 and recorded the reason on the pin.
...
### PROPOSED (awaiting operator approval): Graph Workflows: a steer is intent the service commits and the TICK applies; same-node only

**Rule:** `IGraphWorkflowRunService.SteerAsync` only appends an entry to the encrypted `graph_workflow_node_runs.steering_json` and signals. The `Queued|Running → Pending` reset is written by `GraphWorkflowDispatcher.ApplySteeringAsync` inside the advance gate, before the poll, and only while the row is on the entry's attempt and invocation. Otherwise the entry is stamped `applied: false` with `node.steer-ignored`. A steer never increments `Attempt`, so `MaxSteersPerNode` is its only bound. Same-node steer is operator ruling D4 (2026-09-23); cross-node routing remains absent. **Prevents:** a service-side reset racing a settle, since `ApplyNodeRunTransitionAsync` has no from-status predicate and would reset a node that already succeeded. It also prevents a steer charged against `MaxTotalAttempts`, and a steer loop with no bound, since each reset restarts the node deadline. **Authority:** `GraphWorkflowDispatcher.ApplySteeringAsync`, `GraphWorkflowStore.AppendNodeRunSteeringAsync`, `GraphWorkflowChatSteerTests`, `GraphWorkflowSteerEndpointTests`, `docs/wiki/21-graph-workflows.md` §3.7, register rows "Cross-node fix loop".
...
**Rule:** with `XE_OLLAMA_RUNTIME_ENABLED=false` the no-op runtime answers "nothing there" — available, empty list (§2) — which renders as an ordinary idle daemon, so node settings branches on the `ollamaConfigured` flag instead. (The loaded-models page no longer shows Ollama at all since 2026-09-23.) The node-settings *page* calls `useOllamaRuntimeConfigured` (in `src/core/runtime/hooks/`) and threads `ollamaRuntimeDisabled` down through `NodeSettingsFieldsCard` to `NodeSettingsRuntimeCard`, which stays presentational and replaces the Ollama endpoint input with a dimmed line (`node-settings-ollama-disabled`, key `pages.nodeSettings.fields.ollamaEndpoint.disabled`). The hook is its own query over the same running-models endpoint at infinite `staleTime`, and lives in `core/` rather than in a feature, because dependency-cruiser's `no-cross-feature` rule forbids that edge and the fingerprint baseline is no-growth. The caller FAILS OPEN: only a definite `false` changes anything, so a loading or failed probe leaves today's UI intact. Only the input is hidden, never the form model, so the stored endpoint still round-trips on save. **Prevents:** an operator hunting for a daemon this node will never start — and, by keeping the call at the page, a card that fetches. Only `NodeSettings.test.tsx` still stubs the hook, because the probe reaches the generated SDK whose axios interceptors import the app router; the two card tests pass the boolean and mock nothing. **Authority:** `NodeSettingsRuntimeCard.test.tsx` and `NodeSettings.test.tsx`.
