...
The inverse also holds for **409s specifically**: a typed exception that means "conflict" must reach `ConflictExceptionHandler` — never hand-build a 409 body in an endpoint. Every mapping there answers with the one `ConflictProblemDetails` envelope (`application/problem+json`, `conflictType` = the `NodeConflictProblemType` enum NAME, `detail` = the exception message, `traceId`), and the SPA discriminates on `conflictType` (`features/chat/api/NodeChatConflict.ts`); extra payload (e.g. `maxConcurrentRuns`) rides as a problem-details extension, so the envelope stays one shape. Adding a mapping = one arm in the handler's switch + one enum member + `ProducesProblemDetails(409)` on the endpoint, and the local `catch` goes away. Trap: `Response.WriteAsJsonAsync(value, ct)` **overwrites** `Response.ContentType` with `application/json` — pass the content type to the overload or a problem body ships mislabelled.
