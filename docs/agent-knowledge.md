...
`.opencode/` carries an **offline behavioral agent eval** harness — no model calls, so it runs anywhere and is deterministic. Run it with `.opencode/scripts/run-agent-evals.ts` (the older, separate `.opencode/scripts/run-evals.ts` runs the golden-workflow/config regression suite; they are different things).
...
- **`-SkipUpload` is not a "skip validation" flag.** Every build and test gate still runs; it relaxes exactly one thing, the client-ID *requirement*. A supplied ID is always validated (placeholder or non-`Iv…` → error); an absent one is tolerated only here, bakes no ID at all so the updater ships inert rather than placeholder-configured, and stamps `REHEARSAL-DO-NOT-SHIP.txt` into the publish dir — which means it rides *inside* the Portable.zip. `dotnet publish` doesn't clean that dir, so a real run deletes a stale marker.
  - **`-SkipUpload` composes with `-GitHubAppClientId`** — only their *absence together* makes it a rehearsal (`$isRehearsalPackage = $SkipUpload -and -not $hasGitHubAppClientId`). So passing a syntactically valid `Iv…` value with `-SkipUpload` exercises the whole injection + validation + no-marker + stale-marker-removal path and stops before touching GitHub. That is the cheapest way to rehearse a real release without a live upload; live-verified 2026-08-03. Note it produces an artifact with a baked ID and **no** `REHEARSAL-DO-NOT-SHIP.txt` — i.e. one that looks shippable — so delete it, or follow with a plain rehearsal to leave a marked one behind.
...
- **Consolidating to one repo.** Source now lives in `w0rldx/XE-Local-AI-Engine.Source` (public) — the single home for source and releases going forward; the separate `w0rldx/XE-Local-AI-Engine.Tester-App` is being retired. Historically the two shared only a version string: the `v<version>` git tag went on HEAD of the source repo while `vpk upload github --tag` created a same-named release on the tester repo, so tester releases through `0.1.0-rc.5.0` have no tag in this repo — don't look for them in `git tag -l`. The packaging/upload tooling (`package-tester-win.ps1`, `release.yml`) still targets the tester repo; repointing it is the remaining migration step.
...
- **Both update channels now point at this repo.** `appsettings.AppUpdate.main.json` and `appsettings.AppUpdate.tester.json` both carry `GitHubRepositoryUrl = https://github.com/w0rldx/XE-Local-AI-Engine.Source` with an **empty** `GitHubAppClientId` — a public repo needs no app auth. The `main` channel was previously inert (`REPLACE_*` placeholders, an owner decision while distribution was tester-only); it is now wired. Do not "redact" these URLs back to placeholders — that breaks self-update for installed builds.
...
- **The GitHub App client ID must start with `Iv`, and the numeric App ID and the `Ov…` OAuth App ID are both wrong.** `AppUpdateChannelOptions` documents why: the device flow accepts all three, so an `Ov` ID packages cleanly and then mints a **zero-scope token** that cannot read the private release repo — a build-time rejection turned into a 403 on every tester's update check. The C# predicate and the packaging regex are deliberately identical; do not widen either without giving `StartAsync` an OAuth `scope` path.

Two more, each already actioned but worth not re-deriving:

- **`CopyToPublishDirectory="Always"` does not mean "every publish overwrites".** Measured: after the destination is disturbed, repeat `dotnet publish` runs leave `appsettings.AppUpdate.json` **missing** — silently, exit 0 — until the **source** timestamp changes. The csproj comment claiming "regardless of timestamps" is wrong. The packed artifact is safe only because `package-tester-win.ps1` `Test-Path`s the file and throws; a manual publish has no such guard. The channel switch (`-p:UpdateChannel=main`) does work correctly.
