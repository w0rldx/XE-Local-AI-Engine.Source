...
**Rule:** `NodeAuthService.RefreshAsync` honours a token that rotation replaced for ten seconds. `revoked_at_utc` is the ONE column every revoke path writes — logout (`RevokeRefreshTokensAsync`), `ChangePasswordAsync` and `ResetAdminPasswordAsync` all use it — so "revoked, but recently" would resurrect a logged-out session. The discriminator is `replaced_by_token_id` (`NodeRefreshToken.ReplacedByTokenId`), which ONLY rotation writes (`INodeIdentityStore.RotateAsync`, one save with the successor). The grace follows that link to the chain's head and honours the presented token only while the head is live and unexpired; the revoke-all paths link nothing and revoke the head, so a logged-out cookie can never qualify. Sessions are independent chains (operator decision 2026-09-23, live-QA F-01): login issues a new chain and revokes nothing, refresh revokes only the presented token, and only logout, password change and reset revoke every chain. That is why the discriminator cannot be a timestamp any more: the former "a live token of this user created at the revocation instant" match would let ANOTHER client's login or rotation on the same tick vouch for a dead chain (`Refresh_WhenTheOwnChainIsDeadButAnotherSessionIsLive_FailsInsideTheWindow`). The grace path must not re-stamp `RevokedAtUtc` or re-link, or replaying every nine seconds walks the window forward forever. The grace pair is a SIBLING of the chain's head, not its replacement, so the winner's token stays live and out-of-order `Set-Cookie` responses no longer lose the session; the price is one extra live token until expiry (14 days) or logout per graced race, and a copy of the cookie captured inside the window can mint pairs until it closes (bounded by the auth rate limit). Each such pair is an independent full-lifetime session, so cookie theft inside the window is no longer revealed by the other client being signed out; only logout, a password change or a reset ends it. **Failure prevented:** a fix for "the second tab gets logged out" silently turning a logged-out refresh cookie back into a live session; a revert to "one live token per user" signing every other client out on each login (the F-01 symptom); and matching the successor by user + instant, which crosses sessions. **Authority:** `XE-Local-AI-Engine.Tests/Auth/NodeAuthRefreshRotationGraceTests.cs` (2026-09-17, rewritten 2026-09-23 for concurrent sessions); migration `AllowConcurrentRefreshSessions`.
...
### Graph Workflows: a PARKED run holds no concurrency slot, and a resuming run does not re-pass admission
...
### Graph Workflows: a steer is intent the service commits and the TICK applies; same-node only
