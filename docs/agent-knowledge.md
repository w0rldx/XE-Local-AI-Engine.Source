...
A spawned child is built with `spawn_subagent` **unconditionally stripped from its tool set** (`SubAgentSpawnService.ResolveBindingAsync` → `ResolveChildToolsAsync`, `SubAgentSpawnService.cs:220`). The runtime guard (`SpawnContext.Current is { Depth: >= 1 } → reject`, `:110`) is defense-in-depth for a misconfiguration, **not** the primary control. Never rely on the runtime check alone.
...
- **A C# `long?` becomes a runtime `bigint` in TS.** With hey-api `validator:true`, `long?` generates `z.coerce.bigint()` — the TS type claims `number` but the runtime value is a `bigint`, and arithmetic throws "Cannot mix BigInt and other types". Always `Number()`-convert such fields in the mapper (see `NodeChatMapper.ts:183`); never hand-edit `zod.gen.ts`.
