...
- **Orchestration is handoff-only.** `AgentWorkflowBuilder.CreateGroupChatBuilderWith` and the Magentic, sequential and concurrent members of that family are deferred, not missing: `Plans/2026-05-27-agent-mode-loop-architecture-plan.md` §3.3 scoped v1 to handoff, and the deferral was re-confirmed on 2026-09-09 — not adopted, no current demand. Prevents each fresh assessment re-discovering it as an unstated gap. Authority: operator ruling 2026-09-09 (D9).
