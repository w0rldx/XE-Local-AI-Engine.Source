...
The Training group (datasets → QLoRA runs → export/smoke/promotion → evaluation/comparison) was built and then driven end to end on this box (RTX 5090, CUDA `llama-server` override). Every rule below encodes something the live gate broke that the unit suites could not see. Anchors: `Client.Application/Services/Training/**`, `Providers.Training/**`, `tools/training/*.py`, ADR 0005.

- **Structured output needs NO provider change — and the teacher sees a REWRITTEN schema.** The MEAI OpenAI adapter already emits `{"type":"json_schema","json_schema":{…}}`, which llama-server b10201 parses (`tools/server/server-common.cpp` at the tag), and its strict-schema transform strips every keyword the GBNF sanitizer bounds. That transform also promotes **every property to `required`** — so a "leave `toolName` empty for a no-tool answer" contract is unsatisfiable under constrained decoding: a small teacher writes `"None"`. Read sentinels at the interpretation boundary (`TeacherSampleRecordV1.DemonstratesToolCall`), validate records against the ORIGINAL schema, and never design an optional-string protocol for a constrained teacher. Pinned by `DeferredLlamaServerStructuredOutputTests` (a failure after an MEAI bump is a release blocker, not a test to update).
- **Every model turn on a queue needs its own deadline.** A non-streaming completion to llama-server was lost (server idle, client waiting) and the generation queue parked on it until the OpenAI SDK's own 10-minute `NetworkTimeout` failed the WHOLE dataset. `StructuredAgentRunner.TurnTimeout` (5 min) and the evaluation executor now bound each turn; an overrun is that sample's recorded failure, never a wedge. Nothing upstream (`IChatClient`, MAF agent, MEAI pipeline) carries a deadline for you.
- **`str(EOFError())` is empty.** The trainer's protocol `error` event carried `message: ""`, which sanitized to `null`, and a failed run showed no reason. The exception type is the floor in `train.py`, and the executor falls back to the exit-code message. Any "reason" field fed from `str(exception)` needs the same floor.
- **`datasets.map` forks; a sandboxed CUDA-initialized trainer cannot.** With `num_proc` defaulted from the CPU count the tokenization Manager's forked child died (`EOFError` on the manager pipe). `SFTConfig(dataset_num_proc=1)` — the datasets are small and the model is already resident.
- **A rejection's reason must outlive the hub buffer.** The dataset's rejected COUNT is durable; the reason rode only the transient event buffer, evicted at terminalization. It is logged now (validator/transport messages, never sample content). Same shape as the "fail-visible" invariant everywhere else: if only the count survives, the failure is silent.
- **The uv lockfile pins are not the research pins.** `numpy~=1.26.4` (llama.cpp CI) has no cp313 wheel; torchvision must ALSO come from the cu128 index (PyPI's is cu130 → "compiled with different CUDA major versions" at import); resolution must be narrowed to linux/x86_64 (`tool.uv.environments`) or `mlx-lm` makes the lock unsatisfiable on darwin. Comments in `tools/training/pyproject.toml` carry the why. `probe.py`/`train.py` stdout is DIRTY (unsloth banners) — parsers scan for JSON lines, never read line 1.
- **`LocalModelOrigin` has FOUR change sites** (JSON converter, `BenchmarkRunConfiguration` EF pair, the benchmark table CHECK constraint, and `GgufRegistryRevision.SerializeOrigin`), and **every new `GgufModelRegistryEntry` field must also land in `InstalledGgufRegistryValue`** or the deletion store recomputes a different revision token and its compare-and-swap fails from the rollback path — which masks the original failure. Both bit the Slice-4 prep lane as red tests, not as review findings.
- **`ReadOnlyMemory<byte>?` never reads as null through the implicit `byte[]` conversion.** `entity.Blob?.ToArray()` and ternaries hand a nullable a `HasValue==true, Length==0` value for a NULL column. Route nullable blobs through `OptionalBlob` (`Client.Persistence/Implementation/`) — a license gate branching on `LicenseJson is null` silently took the wrong branch before this.
- **TanStack file routes nest by filename.** `training.datasets.tsx` under `training.tsx` renders the PARENT unless it has an `<Outlet>`; siblings need `training.index.tsx` and each sibling carries its OWN `beforeLoad` capability gate. `NodeCapabilities.test.ts` pins the whole route map — extend it with every route.
- **"placed all N layers on the GPU" is the fit-plan, not the audit.** On this box's default Vulkan runtime (no ICD) that line printed while `nvidia-smi` showed zero compute processes and llama-server pegged four CPU threads. `IRuntimeDeviceAudit` remains the authority (§2); for any live GPU gate use the CUDA override (`source ~/cuda-llama/env.sh`).
- **Training exclusivity is `ITrainingActivity`, not the mutation lease, for anything that must LOAD a model.** A training run holds both (eject-first); an evaluation run holds only the activity — the lease refuses while inference is running, so an evaluation that took it would deadlock against its own model load. The queue peeks the head's kind before claiming (`PeekNextKindAsync`) because a claim is attempt-pinned to 1 and can never be handed back.
- **The pinned convert scripts import a `conversion/` package.** At b10201 `convert_hf_to_gguf.py`/`convert_lora_to_gguf.py` do `from conversion import …` (upstream split the per-architecture converters out of the monolithic script); provisioning only the two scripts + `gguf-py` fails at the script with `ModuleNotFoundError: 'conversion'`. `ConvertScriptProvisioner` ships it beside them and treats an older tree without it as not-provisioned. Any llama.cpp pin bump must re-check the scripts' top-level imports.
- **A run must be LINKED to its installed base GGUF or adapters are dead ends.** Nothing wrote `LinkedInstalledModelName` until the live gate: an adapter export could not be smoke-tested (no base for `--lora`) or promoted, and a comparison had no base side. `IInstalledBaseModelLinker` resolves the wizard's explicit choice, else the official `<base>-GGUF` repo or the same repo id — never a display-name guess. The defaults endpoint returns the suggestions.
- **Merged export on a prebuilt runtime stops at the quantizer, by design.** Upstream ships no `llama-quantize` in release archives; only an in-app source build has one. The export refuses with "build the runtime from source, or export at F16" — F16 merged export needs no quantizer and was live-verified (994 MB Qwen2.5-0.5B merge → convert → smoke → promote).
- **The capacity gate's RAM refusal is usually the build daemons.** "Not enough free system memory to start this run" fired with 4 GB available against a 6 GB estimate while `VBCSCompiler` held 3.9 GB; `dotnet build-server shutdown` freed it. Same hygiene rule as §1's release-pack note.
- **Nav flag `training` in `NodeCapabilities.ts` is compile-time and OFF (dark-ship).** Endpoints ship registered and Operator-gated regardless; direct URLs redirect home while it is off.
