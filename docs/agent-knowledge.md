...
With all four fixed, `dotnet-gcdump` shows disposed **TestServer-fixture** hosts fully collected (2 live fixtures = the running test), and the full-suite peak batch RSS dropped 1366 → ~590 MB. The **WebApplicationFactory** fixture still leaks per host even with these fixes (measured: 15 live `TestingWebAppFactory` after ~20 disposals) — there the `HostingListener` root is real and unfixable from the fixture, so retiring `WebApplicationFactory` per class remains the path to a leak-free one-process run. The fresh-process runner stays the mitigation of record until that migration completes (started: the 13 `Endpoints.Agents` files run on `TestServerWebAppFactory`).
