...
- A pairwise activation still seeds POINTWISE judge attempts for the cohort (the existing `ActivateJudgePolicyAsync` seed). That is four extra judge calls per activation whose scores nothing ranks — but they are also what makes the pointwise-vs-pairwise comparison above possible on one project, so it was left alone rather than suppressed.
