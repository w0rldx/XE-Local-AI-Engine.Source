...
### The Ollama gate-off branch registers a no-op `IModelCapabilityClient`, so the node still boots

**Rule:** `AddNodeModelRuntimeExtensions.AddOllamaRuntime` registers `UnavailableModelCapabilityClient` (`Client.Application`, `Services/Capabilities/Implementation/`) on the gate-off branch, because `IModelCapabilityClient` has exactly one other registration and it lives inside the `AddOllamaLocalModelProvider` call the same gate skips. The no-op answers every probe as "nothing to probe" — unreachable, no version, empty installed and running inventories, a null context length — so a node started with `XE_OLLAMA_RUNTIME_ENABLED=false` reports exactly what a desktop without an Ollama daemon reports: `OllamaReachable=false`, the `ollama-unreachable` diagnostic, `ManagementMode=unknown`, and the configured-model fallbacks as the installed set. One nuance: `InstalledModelInventoryResult.OllamaQuerySucceeded` is `true` under the no-op, because an empty list is a successful query, whereas a dead daemon throws and yields `false`; `CapabilityReportComposer` ANDs it with the runtime status, so the reported reachability is unchanged either way. **Prevents:** an operator who merely opted out of a SECONDARY runtime finding the host unbuildable, and a test that disables the gate having to substitute the client itself — `RunningLocalModelEndpointTests` carried that workaround and no longer needs it. **Authority:** `OllamaRuntimeGateStartupTests.Host_WithTheOllamaRuntimeGateOff_BuildsAndReportsTheRuntimeUnreachable`.
...
| A node cannot start with `XE_OLLAMA_RUNTIME_ENABLED=false`. | It can since S8; the gate-off branch registers a no-op `IModelCapabilityClient` (§2). |
