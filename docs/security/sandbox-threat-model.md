...
| AB3 | An operator installs an MCP server from a README; it reads `~/.ssh` and the node database on first tool call | **Closed at the default tier (G2).** A `Sandboxed` server runs with the host filesystem absent from its mount namespace — live-evidenced: the fixture server reports `CANARY=ABSENT ENV=OK CWD=/work NET=DENIED`. Two residues, both visible rather than silent: an operator who grants `PrivilegedHost` re-opens it deliberately for that server, and on a host without bubblewrap the tier is unavailable and the server refuses to start |
