...
| T3 | **Packages resolved at restore** | Arbitrary code at restore, build and test time. Reached today because Development Mode egress is `Unrestricted` (**GAP → G1**) |
| T4 | **Outbound MCP servers** | stdio servers are operator-configured third-party executables launched **on the host** (`McpClientFactory.BuildStdioTransport`) (**GAP → G2**). HTTP servers are loopback-only by design (`McpOptions.HttpLoopbackHosts`, exact match) |
...
```
   ...
   │ AgentHome,   │ │ run_python    │ │ Dev Mode    │ │ stdio MCP        │
   │ Coder, Dev   │ │ net: DENIED   │ │ net: OPEN   │ │ GAP → G2         │
   │ Mode default │ │               │ │ GAP → G1    │ │                  │
   │ net: denied  │ │               │ │             │ │ training (uv,    │
   │ where probed │ │               │ │             │ │ ADR 0005, by     │
   └──────────────┘ └───────────────┘ └─────────────┘ │ decision)        │
                                                      └──────────────────┘
           │                │               │
...
Three properties of this picture are worth stating plainly. **B7 exists and should not** — a stdio MCP server is the one
model-reachable execution path with no sandbox boundary under it at all (G2). **B4's strength is measured per host, not
assumed** (`SandboxContainment`); off Linux it is `SandboxContainment.None` and B4 collapses into B7 in everything but
supervision. **B6 is currently the *weakest* boundary on the egress axis and the strongest on the toolchain axis** — it
is the only sandbox with an open network (G1).
...
|---|---|---|
| 1 | No agent-authored command line or agent-authored code executes as a plain host child. Development Mode executes only ids from `DevelopmentCommandIds`, resolved through `DevelopmentCommandProfile.ResolveCommand`, which throws for anything else | `DevelopmentCommandProfileCatalog`, `DevelopmentProfileGuardTests`. **Partial: GAP → G2** for stdio MCP |
...
| 3 | The child's environment contains no inherited engine secret. `ProcessSandboxRuntimeProvider.ExecuteAsync` clears and repopulates from `InheritableEnvironmentAllowlist`; the isolated chain uses `--clearenv`; stdio MCP sets `InheritEnvironmentVariables = false` | `ProcessSandboxRuntimeProviderTests`, `SandboxContractGuardTests`, the `BuildStdioTransportOptions` hardening test |
| 4 | A sandbox with no declared network need has no route off the box. True for AgentHome/Coder where the probe succeeded, and for `run_python` (bwrap `--unshare-net`, refused fail-closed otherwise) | `AgentHomeService.ResolveNetworkPolicy`, `ComputeToolGateway`, `SandboxIsolationLiveTests`. **False for Development Mode: GAP → G1** (`NetworkPolicy = SandboxNetworkPolicy.Unrestricted`, with the two required halves named in the source comment) |
...
| 10 | An operator-configured MCP server cannot execute outside a sandbox boundary | **GAP → G2.** `McpClientFactory.BuildStdioTransport` launches `StdioClientTransport` on the host; only the environment is scrubbed. HTTP is loopback-only (`McpOptions.HttpLoopbackHosts`), which is a reachability control, not a containment one |
...
| 13 | A cache shared between sandboxes cannot be poisoned by one of them. Vacuously true today: `NUGET_PACKAGES` and `DOTNET_CLI_HOME` are **per-task** roots under `RuntimePath` (`DevelopmentWorkspaceTools.BuildEnvironment`), so no cache is shared | `DevelopmentWorkspaceTools`. Becomes a live property the moment G1's warm-restore cache or G7's package cache exists — and must be re-asserted then |
...
|---|---|---|
| AB1 | A malicious package's restore/build hook reads the cloned repository and POSTs it out | **Live.** Development Mode egress is `Unrestricted` (G1). The clone excludes gitignored files but not untracked secrets (G5) |
| AB2 | A test prints `.env` so its bytes land in captured stdout and ride the attempt context to a cloud model | **Live and not closed by G1.** Read guards gate the engine's tools, not repository execution. `DevelopmentArtifactSanitizer` + `MemoryProposalSecretScanner` reduce persistence, not the model's exposure |
| AB3 | An operator installs an MCP server from a README; it reads `~/.ssh` and the node database on first tool call | **Live (G2).** Environment scrubbing does not confine the filesystem |
