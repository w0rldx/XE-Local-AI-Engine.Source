...
| Microsoft Agent Framework 1.20.0 | `AIAgent` / `ChatClientAgent` integration, handoff workflows, tool invocation, approval, and workflow-event streaming. Framework types remain behind this repository's orchestration-session boundary. | <https://learn.microsoft.com/en-us/agent-framework/> and the official [1.20.0 package artifact](https://api.nuget.org/v3-flatcontainer/microsoft.agents.ai/1.20.0/microsoft.agents.ai.nuspec) |
...
8. Treat current online snippets as version-sensitive. At the repository's MAF 1.20.0 / MEAI 10.9.0 pins, verify approval and workflow types against the resolved package artifacts before copying framework examples.
