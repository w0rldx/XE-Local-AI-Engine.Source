...
/// <summary>
///     A provider-neutral snapshot of the host's inference-relevant hardware: RAM, GPU VRAM, GPU vendor, CPU cores
///     and free disk on the models volume.
/// </summary>
/// <remarks>
///     Produced by <see cref="IHardwareProfiler" />; the model advisor picks its memory-fit budget from it (VRAM when
///     <see cref="GpuAccelAvailable" />, otherwise available RAM). Carries aggregates only — no machine identifiers
///     (hostnames/serials) — so it is safe to surface to the operator UI. <see cref="VramKnown" /> gates GPU-mode:
///     when VRAM could not be measured the advisor must degrade to a CPU/RAM-only recommendation.
/// </remarks>
public sealed record HardwareProfile
{
    ...
    ///     the GPU-mode fit budget, the direct analogue of <see cref="AvailableRamBytes" /> for CPU mode.
    /// </summary>
    /// <remarks>
    ///     Nets out VRAM already resident in loaded processes (the main chat model and any warm sub-agent servers).
    ///     Only measured for NVIDIA (via <c>nvidia-smi memory.free</c>); <see langword="null" /> for every other vendor
    ///     even when <see cref="VramBytes" /> (total) is known. The capacity gate may use that total only for a first,
    ///     single-model load; once an unledgered resident or external draft exists it rejects conservatively.
    /// </remarks>
