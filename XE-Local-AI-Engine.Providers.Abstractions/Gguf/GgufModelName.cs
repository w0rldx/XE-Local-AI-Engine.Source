...
/// <summary>
///     Single source of truth for the GGUF registry-key convention shared across lanes: a model name is
///     <c>{repoId}[:{quant}]</c> (e.g. <c>bartowski/Llama-3.2-3B-Instruct-GGUF:Q4_K_M</c>). A Hugging Face repo id is
///     <c>org/name</c> and never contains a colon, so a trailing <c>:quant</c> unambiguously selects the quant. Lane A's
///     <c>PullModelAsync</c>/<c>DeleteModelAsync</c> parse a bare model name through here; the store formats keys here.
/// </summary>
