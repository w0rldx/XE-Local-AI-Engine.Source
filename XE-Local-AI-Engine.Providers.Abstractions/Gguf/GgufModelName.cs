...
/// <summary>
///     Single source of truth for the GGUF registry-key convention: a model name is <c>{repoId}[:{quant}]</c> (e.g.
///     <c>bartowski/Llama-3.2-3B-Instruct-GGUF:Q4_K_M</c>).
/// </summary>
/// <remarks>
///     A Hugging Face repo id is <c>org/name</c> and never contains a colon, so a trailing <c>:quant</c> unambiguously
///     selects the quant. The provider's <c>PullModelAsync</c>/<c>DeleteModelAsync</c> parse a bare model name through
///     here; the store formats keys here.
/// </remarks>
