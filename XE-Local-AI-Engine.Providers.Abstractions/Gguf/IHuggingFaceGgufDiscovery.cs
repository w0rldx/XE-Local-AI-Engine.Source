...
/// <summary>
///     Queries the Hugging Face Hub for GGUF repos and inspects their actual <c>.gguf</c> files.
/// </summary>
/// <remarks>
///     Repos with zero usable GGUF files are excluded from search results. Public listing is anonymous; gated repos
///     appear with <see cref="GgufRepoSummary.IsGated" /> set and require a token only to <em>download</em> (the store's
///     concern). The model-fit advisor is the consumer.
/// </remarks>
public interface IHuggingFaceGgufDiscovery
{
    ...
    ///     GGUF header range-read.
    /// </summary>
    /// <remarks>
    ///     A lighter, faster inspection for interactive surfaces (the download quant picker) that do not need the header
    ///     metadata. <c>mmproj</c> projector companions are excluded. The header fields on each returned
    ///     <see cref="GgufRepoFile" /> are <see langword="null" />.
    /// </remarks>
    ...
    ///     <see langword="null" /> when the repo ships none (a text-only model).
    /// </summary>
    /// <remarks>
    ///     Unlike the selectable-file listings above (which exclude projectors), this surfaces the projector so the
    ///     store can download it alongside the chosen quant. When a repo ships several projector precisions, the
    ///     highest-precision one is returned.
    /// </remarks>
