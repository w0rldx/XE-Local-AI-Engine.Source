...
///     store (selecting a file by quant).
/// </summary>
/// <remarks>
///     Parsing returns <see langword="null" /> when no recognizable quant token is present, so a caller skips an unparseable file rather
///     than dropping the whole repo. Unsloth "Dynamic" quants carry a <c>UD-</c> marker in the filename (e.g.
///     <c>gemma-3-12b-it-UD-Q4_K_XL.gguf</c>). The marker is preserved as part of the canonical token (<c>UD-Q4_K_XL</c>) so a Dynamic
///     quant is a distinct, selectable identity rather than silently collapsing onto its base token — a repo can ship both
///     <c>Q4_K_M</c> and <c>UD-Q4_K_M</c>. Use <see cref="IsDynamic" /> / <see cref="StripDynamicPrefix" /> to compare the two.
/// </remarks>
public static partial class GgufQuantParser
{
    ...
    /// <summary>
    ///     Matches a llama.cpp quant token as a WHOLE token (delimited by <c>-</c>, <c>.</c>, <c>_</c> or string bounds),
    ///     with an optional Unsloth "Dynamic" <c>UD-</c>/<c>UD_</c> marker captured separately (the <c>dynamic</c> group).
    /// </summary>
    /// <remarks>
    ///     Alternatives are ordered longest-first, so Q3_K_XL wins over Q3_K and Q4_0_4_4 (ARM) wins over Q4_0. Covered: K-quants Q2_K,
    ///     Q3_K_S/M/L/XL, Q4_K_S/M/L, Q5_K_S/M/L, Q6_K(_L), Q8_0; legacy Q4_0/Q4_1/Q5_0/Q5_1; ARM Q4_0_4_4/4_8/8_8; IQ1_S/M,
    ///     IQ2_XXS/XS/S/M, IQ3_XXS/XS/S/M, IQ4_XS/NL; floats F16/FP16, BF16, F32/FP32, F64; the native formats MXFP4 and NVFP4, which
    ///     models ship weights at directly; and an optional UD- prefix on any of them (UD-Q4_K_XL, UD-IQ2_M, …). Verified against
    ///     bartowski/unsloth GGUF filenames, including multi-part -00001-of-00002 suffixes.
    /// </remarks>
    [GeneratedRegex(
    ...
    /// <remarks>
    ///     An UNRECOGNIZED token is not merely mis-priced: the file fails <c>IsUsableGgufFile</c>, so a repo whose files all use it
    ///     disappears from search behind "No GGUF repositories matched that search" — as tngtech/Qwen3.6-27B-NVFP4-GGUF does entirely,
    ///     and as s-batman/Ornith-1.0-9B-NVFP4-MTP-GGUF's NVFP4 file does from the picker.
    /// </remarks>
