...
/// <summary>
///     Read-mostly query over the image models present locally, backed by the on-disk manifest the store writes.
/// </summary>
/// <remarks>
///     The runtime resolves a <c>ModelName → file-set</c> mapping; the store is the only writer, so this seam never
///     mutates state. Mirrors <see cref="Gguf.IGgufModelRegistry" /> for the diffusion-model file-set.
/// </remarks>
