...
internal sealed class LlamaServerProcessLauncher : ILlamaServerProcessLauncher
{
    ...
    private ILlamaServerProcessHandle LaunchWindows(ProcessStartInfo startInfo, string label)
    {
    ...
    private ILlamaServerProcessHandle LaunchLinux(ProcessStartInfo startInfo, string label)
    {
...
#pragma warning disable CA2000 // The returned handle takes ownership of the process and disposes it on tree-kill; Wrap disposes on a construction failure.
        return LinuxProcessGroupHandle.Wrap(StartProcess(startInfo, label));
#pragma warning restore CA2000
    }

    private ILlamaServerProcessHandle LaunchPlain(ProcessStartInfo startInfo, string label)
    {
#pragma warning disable CA2000 // The returned handle takes ownership of the process and disposes it on tree-kill; Wrap disposes on a construction failure.
        return PlainProcessHandle.Wrap(StartProcess(startInfo, label));
#pragma warning restore CA2000
    }
    ...
    private Process StartProcess(ProcessStartInfo startInfo, string label)
    {
        ...
        // Forward both streams to the app log. Attached before Start (per Process API) and pumped via the async
        // begin-read APIs so the pipes are drained continuously and never stall the child.
