...
public interface ILlamaLayerPlacementReport
{
    ...
    /// </summary>
    /// <remarks>
    ///     A PARTIAL observation is preferred over a full one, so loading a small model that fits cannot mask a large
    ///     model spilling layers to system RAM, and ties break toward the most recent observation. That preference is
    ///     only honest because <see cref="Remove" /> is called as each process is torn down, so every reading still
    ///     held describes a model loaded RIGHT NOW — an unremoved partial would outrank every full reading forever.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     Keyed on <c>(model, role)</c> and deliberately not on the recorded <c>(model, role, variant)</c>: the
    ///     supervisor tears a process down without carrying the llama.cpp build it was launched against, and a re-load
    ///     that selects a DIFFERENT variant must not leave the previous variant's reading behind to be reported
    ///     alongside — or instead of — the model that is actually resident.
    /// </remarks>
