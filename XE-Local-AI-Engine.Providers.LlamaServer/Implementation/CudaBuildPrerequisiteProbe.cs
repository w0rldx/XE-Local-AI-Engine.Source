...
/// <summary>
///     Default <see cref="ICudaBuildPrerequisiteProbe" />. Reports — never installs — the toolchain an in-app CUDA
...
///     a single unsatisfied OS item with <see cref="CudaBuildPrerequisiteReport.CanBuild" /> false.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class CudaBuildPrerequisiteProbe : ICudaBuildPrerequisiteProbe
{
    ...
    public CudaBuildPrerequisiteProbe(IGpuVendorProbe vendorProbe)
    ...
    internal CudaBuildPrerequisiteProbe(IGpuVendorProbe vendorProbe, string buildCacheRoot, long requiredFreeDiskBytes = RequiredFreeDiskBytes)
    {
    ...
    /// <inheritdoc />
    public async Task<CudaBuildPrerequisiteReport> ProbeAsync(CancellationToken ct)
    {
        ...
        if (!OperatingSystem.IsLinux())
        {
            var osOnly = new[]
            {
                new CudaBuildPrerequisiteItem("os-is-linux", Satisfied: false, "In-app CUDA build is available on Linux only.")
            };
            return new CudaBuildPrerequisiteReport(CanBuild: false, osOnly);
        }

        var nvidiaPresent = await DetectNvidiaAsync(ct).ConfigureAwait(false);

        var items = new List<CudaBuildPrerequisiteItem>
        {
            new("os-is-linux", Satisfied: true, "Linux host detected."),
            new("nvidia-gpu", nvidiaPresent, nvidiaPresent ? "NVIDIA GPU/driver detected." : "No NVIDIA GPU or driver detected."),
            await ProbeToolAsync("nvcc", ["--version"], "NVIDIA CUDA compiler (nvcc)", ct).ConfigureAwait(false),
        ...
        return new CudaBuildPrerequisiteReport(canBuild, items);
    }
    ...
    private static async Task<CudaBuildPrerequisiteItem> ProbeBuildToolAsync(CancellationToken ct)
    {
        var make = await TryProbeAsync("make", ["--version"], ct).ConfigureAwait(false);
        if (make is { Length: > 0 })
        {
            return new CudaBuildPrerequisiteItem("make-or-ninja", Satisfied: true, "make detected.");
        }

        var ninja = await TryProbeAsync("ninja", ["--version"], ct).ConfigureAwait(false);
        if (ninja is { Length: > 0 })
        {
            return new CudaBuildPrerequisiteItem("make-or-ninja", Satisfied: true, "ninja detected.");
        }

        return new CudaBuildPrerequisiteItem("make-or-ninja", Satisfied: false, "Neither make nor ninja was found.");
    }

    private static async Task<CudaBuildPrerequisiteItem> ProbeToolAsync(string fileName, IReadOnlyList<string> args, string displayName, CancellationToken ct)
    {
        var banner = await TryProbeAsync(fileName, args, ct).ConfigureAwait(false);
        return banner is { Length: > 0 }
            ? new CudaBuildPrerequisiteItem(fileName, Satisfied: true, $"{displayName} detected: {banner}")
            : new CudaBuildPrerequisiteItem(fileName, Satisfied: false, $"{displayName} was not found on PATH.");
    }
    ...
    private CudaBuildPrerequisiteItem ProbeFreeDisk()
    {
        try
        {
            ...
            return new CudaBuildPrerequisiteItem("free-disk",
                satisfied,
                satisfied
        ...
        catch (Exception)
        {
            ...
            return new CudaBuildPrerequisiteItem("free-disk", Satisfied: false, "Free disk space could not be determined.");
        }
    }
