...
internal sealed class OsStaleLlamaServerProcessScanner : IStaleLlamaServerProcessScanner
{
    ...
    private const string LlamaServerProcessName = "llama-server";

    /// <inheritdoc />
    public IReadOnlyList<StaleLlamaServerProcess> EnumerateLlamaServerProcesses()
    {
        var results = new List<StaleLlamaServerProcess>();

        // Matches every llama-server (ours and any unrelated one, e.g. Ollama's) — the reaper's path filter excludes the
        // unrelated ones. A process-table read failure surfaces nothing rather than throwing (the reaper is best-effort).
        Process[] processes;
        try
        {
            processes = Process.GetProcessesByName(LlamaServerProcessName);
        }
        ...
        foreach (var process in processes)
        {
            try
            {
                results.Add(new StaleLlamaServerProcess(process.Id, ResolveExecutablePath(process)));
            }
            catch (InvalidOperationException)
            {
