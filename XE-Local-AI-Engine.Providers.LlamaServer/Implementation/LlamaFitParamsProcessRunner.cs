...
internal sealed class LlamaFitParamsProcessRunner : ILlamaFitParamsRunner
{
    ...
    // The expert-placement flags land in the same tensor_buft_overrides list a tensor override writes (llama.cpp common/arg.cpp), and the helper both fits AGAINST
    // those overrides and echoes them back (tools/fit-params/fit-params.cpp), so passing them through is what makes the fitted layer count honest. Dropping them fits wrong.
