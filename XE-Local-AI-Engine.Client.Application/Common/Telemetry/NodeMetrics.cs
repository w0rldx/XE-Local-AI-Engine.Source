...
public static class NodeMetrics
{
    ...
    /// <summary>
    ///     Incremented when a stalled provider stream is abandoned because its <c>MoveNextAsync</c> or <c>DisposeAsync</c> ignored
    ///     cooperative cancellation past the watchdog's bounded grace.
    /// </summary>
    /// <remarks>
    ///     Content-free (a count only) — the deliberate cost of a wall-clock bound that a non-cooperative provider cannot defeat; the
    ///     abandoned enumerator's native resources may not be reclaimed until (if ever) it returns.
    /// </remarks>
    ...
    /// <summary>Incremented when a chat stream event could not be buffered because the stream's bounded queue was full.</summary>
    /// <remarks>
    ///     Should stay at ZERO — the queue holds ~80 s of consumer lag at the live emit cadence — so a non-zero value means a real consumer
    ///     stall rather than a merely slow browser. Content-free (a count only). Labels: reason (queue_capacity | queue_bytes).
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented each time the server tells a client to resynchronize an in-flight turn (re-subscribe through <c>ResumeMessage</c>
    ///     and take a fresh snapshot).
    /// </summary>
    /// <remarks>
    ///     The reconcile is silent to the user by design, so this counter is the only signal that it is happening. Labels: reason
    ///     (queue_overflow | replay_cap). The third cause — a delta offset gap — is detected on the client and rides the frontend
    ///     error-snapshot breadcrumb trail, not this meter.
    /// </remarks>
    ...
    /// <summary>
    ///     Runs currently in flight with NO client attached. Incremented when an invocation's last stream subscriber goes away and
    ///     decremented when one re-attaches or the run terminalizes.
    /// </summary>
    /// <remarks>
    ///     This is the "leases held with nobody watching" gauge: a detached run still holds its collision slot and its llama-server
    ///     process, so a value that stays above zero is what a disconnect-grace regression looks like. Content-free (a count only).
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented when the disconnect grace expires and a detached run is cancelled. Answers "is the grace deadline actually firing",
    ///     which the gauge above cannot — that falls back to zero whether the client returned or the run was reaped.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>One terminal, content-free observation per admitted production agent invocation.</summary>
    /// <remarks>
    ///     Labels are deliberately bounded to provider (local | remote | unknown), outcome (completed | cancelled | failed), and
    ///     orchestration (true | false), so benchmark and production-mode comparisons cannot create per-user/model/tool series.
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented each time a model-invoked MCP tool call exceeds its per-call <c>Mcp:ToolCallTimeoutSeconds</c> deadline and is
    ///     cancelled, returning a typed tool-failure result to the model (the run continues; the call is never retried).
    /// </summary>
    /// <remarks>
    ///     A non-zero rate flags a slow or wedged MCP server. Content-free — a count only; carries no tool name or arguments.
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented once per RESOLVED tool-approval decision. Tagged by <c>category</c> (the tool's <c>ToolCategory</c> name) and <c>decision</c>
    ///     (approve | deny | timeout). Content-free — a count only; it carries no tool arguments, message content, or ids.
    /// </summary>
    /// <remarks>
    ///     A rising deny/timeout rate flags a policy that is prompting more than operators will accept.
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented each time a Hugging Face model download's body-copy loop stalls longer than the configured read-idle timeout and is
    ///     cancelled (surfaced as a transient failure the resume/retry path re-attempts).
    /// </summary>
    /// <remarks>
    ///     A non-zero rate flags a CDN that accepts the connection and then stops sending data mid-body. Content-free — a count only;
    ///     carries no URL, repo, or file name. Bridged from the Providers.HuggingFace layer via <c>IHfDownloadMetrics</c> (which cannot
    ///     reference this meter).
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented each time a native hardware probe (e.g. <c>nvidia-smi</c>) exceeds its wall-clock deadline and is killed (process
    ///     tree).
    /// </summary>
    /// <remarks>
    ///     A non-zero rate flags a wedged GPU driver / stalled tool that would otherwise hang first-run provisioning or a capacity
    ///     decision; the profiler degrades to the last cached profile or the CPU default. Labels: probe (the probe tool name).
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented when a node SQLite operation surfaces write contention that outlived <c>busy_timeout</c> (SQLITE_BUSY /
    ///     SQLITE_LOCKED).
    /// </summary>
    /// <remarks>
    ///     A non-zero value signals real contention on the shared database file — under WAL + busy_timeout this should be rare. Labels:
    ///     code (busy | locked), path (ef | raw). Bounded cardinality — never carries SQL text.
    /// </remarks>
    ...
    /// <summary>
    ///     Wall-clock duration (milliseconds) of the model-readiness (warm) phase that runs BEFORE the stream-idle watchdog is armed — the
    ///     separated cold-load window. Tagged by <c>outcome</c> (ready | failed).
    /// </summary>
    /// <remarks>
    ///     A warm reuse is a small value; a genuine cold load is a large one, so the histogram shows how long readiness actually takes and
    ///     whether it is being paid on the warm path by mistake. Content-free (a duration only).
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented on each operator eject request and its result. Labels: outcome (requested | ejected | timed_out_still_busy | forced
    ///     | not_running).
    /// </summary>
    /// <remarks>
    ///     The <c>requested</c> increment fires once per call; the result increment fires once with the terminal outcome, so
    ///     accept/drain/timeout/force ratios are observable without any model identity.
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented when the runtime device audit detects a silent CPU fallback: the host advertises a GPU but the
    ...
    /// <summary>
    ///     One terminal observation per llama-server load attempt. Labels are bounded enums: role, variant, outcome, placement (cpu | full
    ///     | partial | unknown), attempt (primary | safe_retry), and speculation class.
    /// </summary>
    /// <remarks>
    ///     It carries no model name, path, arguments, prompt, or runtime hash and is report-only.
    /// </remarks>
    ...
    /// <summary>
    ///     Wall-clock duration (milliseconds) a GPU-backed model load waited to acquire the process-wide GPU-load admission gate before its
    ///     spawn began.
    /// </summary>
    /// <remarks>
    ///     Near-zero under no contention; a large value means a load queued behind another load's spawn-through-readiness window.
    ///     Content-free (a duration only).
    /// </remarks>
    ...
    /// <summary>
    ///     Wall-clock latency (milliseconds) from the start of an invocation turn to the moment the local model load begins — the audited
    ///     "silent pre-spawn gap" (a first-ever send stalled ~7.8 s here with no log).
    /// </summary>
    /// <remarks>
    ///     Only recorded for a local llama.cpp turn (the one that pays a cold load); cloud/Ollama turns never reach the warm phase. Tagged
    ///     by <c>provider</c> (local). Content-free (a duration only).
    /// </remarks>
    ...
    /// <summary>
    ///     Wall-clock latency (milliseconds) from the model becoming READY (the local warm phase finished, or turn start for a runtime with
    ///     no cold-load) to the FIRST streamed output chunk — time-to-first-token.
    /// </summary>
    /// <remarks>
    ///     Tagged by <c>provider</c> (local | remote); <c>remote</c> covers cloud and non-warming local runtimes (Ollama), whose
    ...
    /// <summary>Incremented once per cancelled invocation, tagged by <c>category</c>.</summary>
    /// <remarks>
    ///     <c>user</c> (an explicit user cancel), <c>watchdog</c> (the invocation-level timeout fired), <c>operator_eject</c> (the model was
    ///     force-ejected out from under the turn), or <c>shutdown</c> (host shutdown cancelled the run). Distinct from
    ///     <see cref="InvocationFailedTotal" />, which deliberately EXCLUDES cancellations: a cancel is an outcome, not a failure, so it is
    ///     counted here with its cause. Content-free (a count only), bounded cardinality.
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented (by the reported token count) at the terminal usage-finalize of an invocation turn.
    /// </summary>
    /// <remarks>
    ///     Cumulative model token consumption — the real cost surface for cloud providers — is therefore observable live without waiting on
    ///     the persisted <c>agent_execution_logs</c> ledger. Fires once per turn per direction (never per tool-loop round, so it cannot double-count). Labels: provider (local | remote — the
    ///     coarse routing dimension; remote covers cloud and Ollama), model (the resolved model id), direction (input | output).
    ///     Content-free — a token count only; never any prompt, completion, or transcript text. Bounded cardinality (installed/cloud model
    ///     ids are a small, stable set).
    /// </remarks>
    ...
    /// <summary>
    ///     Incremented (by the abandoned count) when the memory-extraction worker abandons in-flight extraction job(s) at shutdown because
    ///     they ignored cooperative cancellation past the drain deadline plus the fixed grace.
    /// </summary>
    /// <remarks>
    ///     Content-free (a count only) — the deliberate cost of a bounded shutdown that never waits indefinitely.
    /// </remarks>
    ...
    /// <summary>
    ///     Number of candidate chunk-vector rows scanned by one managed cosine vector search (content-free — a row count only).
    /// </summary>
    /// <remarks>
    ///     The search reads every stored vector for the resolved embedding model, so this is the brute-force fan-out the bounded top-k
    ///     selection runs over; a rising distribution flags a corpus large enough to warrant an ANN index.
    /// </remarks>
    ...
    /// <summary>
    ///     Wall-clock duration (milliseconds) of one managed cosine vector search — the pure scan/score/select cost,
    ///     measured inside the search itself.
    /// </summary>
    /// <remarks>
    ///     Distinct from the service-observed <c>stage=vector</c> timing, which also spans the factory resolution and
    ///     the awaits around it.
    /// </remarks>
    ...
    /// <summary>
    ///     Bytes reclaimed by the query-embedding cache's eviction passes. Read against the lookup hit ratio: sustained eviction with a
    ///     falling hit ratio means the cache is thrashing its budget rather than serving repeats.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     Registers the observable gauge that reports the current depth of the bounded knowledge-ingestion queue on the shared <c>XE.Node</c>
    ///     meter.
    /// </summary>
    /// <remarks>
    ///     The queue owner (the singleton dispatcher) supplies the live count callback and holds the returned instrument for its lifetime.
    ///     Kept as a factory (rather than a static instrument) because the depth is read from the live channel, which only the dispatcher
    ...
    /// <summary>
    ///     Registers the observable gauges that report how many GPU-backed model loads are currently HOLDING the process-wide admission
    ///     gate (0 or 1 — the gate is a serializer) and how many are WAITING behind it.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     The pair of admission-gate gauges handed back to the gate singleton, which holds them for its lifetime
    ///     (an observable gauge stops reporting once collected).
    /// </summary>
