...
/// <summary>
///     <see cref="NodeMetrics" />-backed <see cref="IHardwareProbeMetrics" />. Bridges the Providers.Capabilities probe seam (which cannot
///     reference the application-layer meter) to the shared <c>XE.Node</c> meter, incrementing
///     <see cref="NodeMetrics.HardwareProbeTimeoutTotal" /> when a native hardware probe is killed for overrunning its deadline.
/// </summary>
/// <remarks>
///     Registered by the host so the profiler emits the counter; tests/headless hosts use the null default.
/// </remarks>
