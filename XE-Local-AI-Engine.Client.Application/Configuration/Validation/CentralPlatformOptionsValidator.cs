...
public sealed class CentralPlatformOptionsValidator : IValidateOptions<CentralPlatformOptions>
{
    public ValidateOptionsResult Validate(string? name, CentralPlatformOptions options)
    {
        ...
        var errors = Enumerable.Empty<string>()
                               ...
                               .AppendIf(string.IsNullOrWhiteSpace(options.DeviceBindingStartEndpoint), "CentralPlatform:DeviceBindingStartEndpoint is required.")
                               .AppendIf(!IsRelativePath(options.DeviceBindingStartEndpoint), "CentralPlatform:DeviceBindingStartEndpoint must be an absolute application path starting with '/'.")
                               .AppendIf(string.IsNullOrWhiteSpace(options.DeviceBindingTokenEndpoint), "CentralPlatform:DeviceBindingTokenEndpoint is required.")
                               .AppendIf(!IsRelativePath(options.DeviceBindingTokenEndpoint), "CentralPlatform:DeviceBindingTokenEndpoint must be an absolute application path starting with '/'.")
                               ...
                               .AppendIf(options.ReconnectBackoffMaxMs is < 1 or > 1800000, "CentralPlatform:ReconnectBackoffMaxMs must be between 1 and 1800000.")
