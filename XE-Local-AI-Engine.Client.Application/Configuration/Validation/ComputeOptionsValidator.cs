...
public sealed class ComputeOptionsValidator : IValidateOptions<ComputeOptions>
{
    public ValidateOptionsResult Validate(string? name, ComputeOptions options)
    {
        ...
        var errors = Enumerable.Empty<string>()
                               ...
                               // Zero would pin every numeric library to no threads at all, which is not a posture. The sandbox create request
                               // rejects a non-positive thread limit too, but failing at startup names the setting; the first tool call would not.
                               .AppendIf(options.ThreadLimit <= 0,
                                   "Compute:ThreadLimit must be greater than zero.")
                               // Unlike LocalContainer:MaxJailDiskBytes, a non-positive value is NOT a supported way to disable the watchdog
                               // here: this ceiling can only tighten the node-wide one, so zero would mean "inherit", reading as the opposite.
