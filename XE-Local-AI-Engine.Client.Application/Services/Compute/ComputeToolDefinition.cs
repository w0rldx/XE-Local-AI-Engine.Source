...
/// <summary>
///     Worker-side name / description / parameter-schema constants for the <c>run_python</c> compute tool.
/// </summary>
/// <remarks>
///     The handler advertises its model-visible schema from here and the offer provider merges the same descriptor into
///     the profile pool, so what the model is offered can never drift from what the handler validates. The schema is
///     advisory; the handler's own validation is authoritative.
/// </remarks>
internal static class ComputeToolDefinition
{
    ...
    // Deliberately carries no `maxLength`: the authoritative ceiling is CodeMaxLength in the handler, and any bound past 1024 is
    // stripped from the llama.cpp wire by LlamaGrammarToolSchemaCompatibility, so a schema with no repetition bound is grammar-safe by construction. See ComputeToolSchemaCompatibilityTests.
