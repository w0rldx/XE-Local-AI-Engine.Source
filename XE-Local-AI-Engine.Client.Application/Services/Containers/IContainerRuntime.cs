...
/// <summary>
///     The engine-owned container runtime: everything ADR 0010's application containers need from a container engine,
///     and nothing else.
///     <para>
///         It derives from <see cref="IDockerRuntimeClient" /> rather than standing beside it so that there is one
///         transport and one lying fake. Development Mode's audited surface does not widen as a result: every new
///         member is declared here, on the derived interface, and <c>DockerSandboxRuntimeProvider</c> takes the base
///         one and never names this type.
///     </para>
///     <para>
///         The names are new rather than overloads of the base members on purpose. Overloading across a base and a
///         derived interface makes the resolved overload depend on the static type of the reference, so a caller
///         holding the base type would silently get Development Mode's create. <see cref="RunContainerAsync" /> also
///         creates <em>without</em> starting, so the caller keeps its create, verify-read-back, then start ordering.
///     </para>
/// </summary>
public interface IContainerRuntime : IDockerRuntimeClient
{
    /// <summary>
    ///     Create a container from <paramref name="specification" /> and return its id, without starting it.
    ///     <para>
    ...
    ///         verifier because <c>required</c> means "assigned", not "non-empty": an empty host IP renders as a
    ///         binding Docker resolves to <c>0.0.0.0</c>, and there must be no window in which such a container exists.
    ///     </para>
    /// </summary>
    ...
    /// <summary>
    ///     Every container carrying all of <paramref name="labels" />, running or not, with the state and exit code
    ///     the observer and the boot reconciler need.
    ///     <para>
    ///         The inherited <see cref="IDockerRuntimeClient.ListContainersAsync" /> returns bare ids, so an exited
    ///         container is indistinguishable from a running one and a crashed application would be reported healthy.
    ///     </para>
    /// </summary>
    ...
    /// <summary>
    ///     Whether the running container can create and delete a file under <paramref name="containerPath" />.
    ///     <para>
    ///         The only exec-shaped operation this layer exposes, and it deliberately takes no command: the caller
    ///         cannot choose what runs. It exists because the uid mapping between a container and an engine-created
    ///         bind mount is a premise rather than a construction — under a rootless daemon in-container root maps to
    ///         the invoking user, under a rootful one it does not — so every install proves the mapping instead of
    ///         assuming it, and fails naming the daemon mode when it does not hold.
    ///     </para>
    /// </summary>
