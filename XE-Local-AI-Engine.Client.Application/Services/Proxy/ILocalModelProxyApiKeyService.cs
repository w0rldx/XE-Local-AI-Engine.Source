...
///     inbound OpenAI-compatible model proxy.
/// </summary>
/// <remarks>
...
{
    /// <summary>Mints a fresh key, REPLACING any existing one, and returns it in full.</summary>
    /// <remarks>
    ///     This is the ONLY time the plaintext key exists outside the caller that presents it: only its SHA-256 digest is persisted, so a
    ///     key not captured from this return value is gone and can only be replaced by generating another. Every other surface —
    ...
    /// <summary>Returns the current credential's non-secret metadata, or <see langword="null" /> when none has been generated.</summary>
    /// <remarks>
    ///     Deliberately cannot return the key: the node stores only a one-way digest, so a lost key is unrecoverable and the operator must
    ///     generate a replacement and reconfigure every client.
    /// </remarks>
...
/// <summary>
///     A freshly minted credential: the one-time plaintext <see cref="Key" /> plus the metadata that will remain retrievable afterwards.
///     Separate from <see cref="LocalModelProxyApiKeyView" /> so the type system — not a comment — is what stops the secret being returned
///     from a retrieval path.
/// </summary>
/// <remarks>
///     Never log it, never persist it, never put it in an audit record or an error body: once this value is dropped, the key is gone.
/// </remarks>
