...
public sealed class DevWorkflowDefinitionSeeder : IHostedService
{
    ...
    /// <summary>
    ///     The <c>feature-development-v1</c> graph a previous build shipped, kept so an installation still holding it
    ///     verbatim can be brought up to the current one. A revision leaves this list only when no installation could
    ///     still be on it, which is not a thing this code can know — so they stay.
    /// </summary>
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await SeedAsync(store, FeatureDevelopmentSlug, FeatureDevelopmentName, FeatureDevelopmentGraph, [FeatureDevelopmentGraphRevision1], cancellationToken)
