...
/// <summary>
///     What an agent's effective model is, and whether BOTH tool gates a work session depends on admit it.
///     <para>
...
internal sealed class WorkSessionToolGate
{
    ...
    ///     pin, or the node's default chat model when it has none.
    ///     <para>
    ...
    ///         underneath it and must not fail the step for it.
    ///     </para>
    /// </summary>
    public async Task<WorkSessionToolGateVerdict> InspectAsync(Guid agentDefinitionId, CancellationToken cancellationToken)
    {
        ...
        {
            return new WorkSessionToolGateVerdict(AgentExists: false, string.Empty, EffectiveModel: null, SupportsTools: false, IsAllowListed: false);
        }
        ...
        {
            return new WorkSessionToolGateVerdict(AgentExists: true, definition.Name, EffectiveModel: null, SupportsTools: false, IsAllowListed: false);
        }
