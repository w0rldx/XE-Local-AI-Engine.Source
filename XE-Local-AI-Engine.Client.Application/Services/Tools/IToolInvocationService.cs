...
/// <summary>
///     Why a single named-tool invocation ended the way it did. <see cref="Executed" /> = ran in-process. <see cref="UnknownTool" /> = no
///     catalog entry. <see cref="NotInvocable" /> = the tool exists but fails the Tool-node envelope (<c>ToolCategory.ReadLocal</c> AND a
///     composed approval of <see langword="false" />) or has no in-process executable.
/// </summary>
/// <remarks>
///     <see cref="InvalidArguments" /> = not a JSON object, or fails the tool's own schema. <see cref="Timeout" />,
///     <see cref="Cancelled" /> and <see cref="Faulted" /> are self-describing.
/// </remarks>
...
/// <summary>
///     Invokes ONE named engine tool in-process, enforcing the whole invocation envelope inside itself so no caller can skip a gate.
/// </summary>
/// <remarks>
///     Promoted from <c>HeadlessToolExecutor</c>'s enforcement pattern, with two differences: it consults the model-agnostic catalog and
///     BOTH executable registries (so the worker-owned read tools are reachable), and anything outside the envelope is refused rather than
///     mocked.
/// </remarks>
public interface IToolInvocationService
{
    ...
    /// <summary>Every tool this service would actually invoke, with the schema it validates against.</summary>
    /// <remarks>
    ///     The ONE source the save-time validator, the run-start re-validation and the picker all read, so they cannot disagree with the
