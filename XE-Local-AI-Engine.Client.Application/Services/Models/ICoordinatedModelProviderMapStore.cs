...
{
    /// <summary>Every stored mapping, ordered by model name. Takes NO lease.</summary>
    /// <remarks>
    ///     A lease is keyed per model, so there is no such thing as a lease over "the whole map", and holding one over every row would
    ///     serialize the map against every concurrent claim on the node. This is therefore a point-in-time SNAPSHOT, valid only for
    ///     discovering candidates: its one caller — the external-provider reconciliation pass, which has to find <c>ext:</c> rows whose
    ///     registration is gone — takes a per-model mutation lease and re-reads the row under it before touching anything, so a row that
    ///     moved between the listing and the repair is acted on at its current revision rather than the snapshot's.
    /// </remarks>
