...
public interface IDockerDaemonPreflightService
{
    ...
    /// <summary>Record the operator's explicit approval of the daemon currently reachable, and return the resulting preflight.</summary>
    /// <remarks>
    ///     <paramref name="expectedDaemonId" /> is the daemon the operator was SHOWN. It is required, and the confirmation is refused when
    ///     it does not match what is reachable now: without it a confirmation issued against one daemon could land on whichever daemon
    ///     answered by the time the request arrived, making the control approve something nobody looked at.
    /// </remarks>
