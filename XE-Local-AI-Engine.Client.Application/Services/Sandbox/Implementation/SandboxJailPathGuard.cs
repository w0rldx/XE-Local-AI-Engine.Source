...
/// <summary>The process sandbox's path-safety guards, gathered so the security invariant is one file to audit.</summary>
/// <remarks>
///     The lexical jail check (<see cref="ResolveJailPath" />) and the per-component symlink check
///     (<see cref="EnsureNoSymlinkComponentsUnderJail" />) are mandatory TOGETHER on every read and write leg, because
///     canonicalization collapses <c>..</c> but does not resolve a symlink a sandboxed command planted inside the jail, and every open
///     that follows them goes through a no-follow <c>open(2)</c>. Callers hold the jail root and this type holds no state, so a new
///     sandbox surface reaches the whole guard pair by calling in rather than re-deriving half of it.
/// </remarks>
internal static class SandboxJailPathGuard
{
    // O_RDONLY (0x0) | O_NOFOLLOW (0x20000) | O_CLOEXEC (0x80000) on Linux. A raw (FileOptions) cast for O_NOFOLLOW throws, so the libc
    // open() DllImport below is required, in parity with AgentHome's host-file no-follow guard.
    private const int ReadOnlyNoFollowCloseOnExecFlags = 0x0 | 0x20000 | 0x80000;

    // O_WRONLY (0x1) | O_CREAT (0x40) | O_TRUNC (0x200) | O_NOFOLLOW (0x20000) | O_CLOEXEC (0x80000) on Linux, 0o644 for the new file. A
    // no-follow create fails with ELOOP if the leaf already exists as a symlink, so a planted leaf cannot redirect the copy-into write.
    ...
    /// <summary>
    ///     Canonicalizes a possibly sandbox-absolute path into a host path that MUST live under the jail root; this is the load-bearing
    ///     jail control.
    /// </summary>
    /// <remarks>
    ///     Any path escaping the jail, by <c>..</c> traversal or by naming somewhere outside it, is rejected lexically
    ///     (<see cref="Path.GetFullPath(string)" /> collapses <c>..</c>). It does NOT resolve symlinks, and a path under the jail can still
    ///     TRAVERSE one a command planted with the jail as its CWD, so the caller must pass the canonical path through
    ///     <see cref="EnsureNoSymlinkComponentsUnderJail" /> before opening.
    /// </remarks>
    ...
    /// <summary>
    ///     Rejects a jail-relative path whose final component, or ANY component between the jail root and the leaf, is a symlink.
    /// </summary>
    /// <remarks>
    ///     A command running with the jail as its working directory can plant one (<c>workspace/x -&gt; /etc</c>): the lexical
    ///     <see cref="ResolveJailPath" /> check passes such a path, but opening through it would read or write OUTSIDE the jail. Walking
    ///     each component with a no-follow probe closes that escape for the read legs and the copy-into write.
    ///     <paramref name="canonicalPath" /> must already be proven under the jail, and only existing components are probed. Throws
    ///     <see cref="UnauthorizedAccessException" /> on the first symlink component — a plant-after-resolve signal.
    /// </remarks>
    ...
    /// <summary>Reads the host file under the no-follow and byte-recheck guards.</summary>
    /// <remarks>
    ///     Throws <see cref="InvalidDataException" /> when the file exceeds the per-file cap on this re-read or grew after sizing, and
    ///     <see cref="UnauthorizedAccessException" /> when the final component is a symlink or the open cannot be performed safely — a
    ///     swap-after-walk signal.
    /// </remarks>
    ...
    /// <summary>Opens the host file refusing a symlink at the final component.</summary>
    /// <remarks>
    ///     On Linux an atomic <c>open(2)</c> with <c>O_NOFOLLOW</c>, the kernel failing with <c>ELOOP</c> if the leaf is a symlink, which
    ///     closes the check-then-open race a managed <c>lstat</c> plus open would leave; a raw <c>(FileOptions)</c> cast for it throws, so
    ///     the libc <c>open()</c> DllImport is required. Off Linux this provider is not the primary runtime, so it falls back to a plain
    ///     open and relies on the canonicalized jail check plus the byte re-check. Throws
    ...
    // A single libc open(). The caller marshals the path into a null-terminated UTF-8 byte array so any filename round-trips, and the
    // import takes the raw bytes. DllImport rather than LibraryImport keeps the project free of AllowUnsafeBlocks for one call.
    ...
    /// <summary>Reads a jail-side file's raw bytes through a no-follow open.</summary>
    /// <remarks>
    ///     On Linux the open is atomic with <c>O_NOFOLLOW</c>, so a leaf swapped to a symlink after the per-component check fails the open
    ///     instead of redirecting the read; off Linux it falls back to a plain handle and relies on the per-component check plus the jail
    ///     canonicalization. Throws <see cref="UnauthorizedAccessException" /> when the leaf is a symlink or the open fails.
    /// </remarks>
    ...
    /// <summary>Writes bytes to a jail-side path through a no-follow create.</summary>
    /// <remarks>
    ///     On Linux <c>O_NOFOLLOW</c> makes the create fail with ELOOP if the leaf already exists as a symlink, so a planted leaf cannot
    ///     redirect the copy-into write outside the jail; off Linux it falls back to a plain create, the per-component check still guarding
    ///     intermediate components. Throws <see cref="UnauthorizedAccessException" /> when the leaf is a symlink or the create fails.
    /// </remarks>
