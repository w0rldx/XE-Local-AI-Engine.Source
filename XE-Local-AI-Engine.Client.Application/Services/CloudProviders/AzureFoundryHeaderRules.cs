...
///     the reserved set is reused by the outbound pipeline policy for a belt-and-suspenders skip (Locked #6–#9).
/// </summary>
public static class AzureFoundryHeaderRules
{
    /// <summary>Maximum number of custom headers per connection (Locked #9).</summary>
    public const int MaxHeaderCount = 32;

    /// <summary>Maximum header-name length in characters (Locked #9).</summary>
    public const int MaxHeaderNameLength = 128;

    /// <summary>Maximum header-value length in characters (Locked #9).</summary>
    public const int MaxHeaderValueLength = 4096;

    /// <summary>Maximum number of operator-added allowed host suffixes per connection (Locked #9/#14).</summary>
    public const int MaxHostSuffixCount = 16;

    // Case-insensitive reserved set (Locked #8): names that carry auth/transport semantics and must never be
    ...
    /// <summary>Returns true when the name is in the reserved set (case-insensitive, Locked #8).</summary>
    ...
    /// <summary>
    ///     Returns true when the name is a non-empty RFC 7230 token (Locked #6):
    ///     <c>A-Z a-z 0-9 ! # $ % &amp; ' * + - . ^ _ ` | ~</c>.
    /// </summary>
    ...
    /// <summary>
    ///     Returns true when the value contains only RFC 7230 field-value characters (Locked #7): rejects CR, LF, NUL,
