...
/// <summary>A pending <c>ask_user</c> question surfaced for the in-flight invocation.</summary>
/// <remarks>
///     Deliberately shaped like <see cref="ApprovalLifecyclePayload" /> — same fan-out, same correlation keys — so the local
///     send/regenerate/resume paths cannot drift apart: <see cref="CallId" /> attaches the question card to the tool-call card the model is
///     waiting on. Unlike an approval, which the client can render from the request id alone, a question is only answerable if the client
///     also has the QUESTIONS, so they ride this payload in full — which is what lets a reconnecting browser be handed a still-pending
///     question rather than losing the prompt (see the reconnect replay).
/// </remarks>
