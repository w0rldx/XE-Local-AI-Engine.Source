...
{
    /// <summary>The live in-flight invocation, or null.</summary>
    /// <remarks>
    ///     This exposes the dispatcher's mutable instance, which the streaming path keeps updating under its internal lock. Its
    ///     <see cref="InvocationState.StreamedContent" /> and <see cref="InvocationState.StreamedThinkingContent" /> getters materialize from
    ///     an IMMUTABLE append-only accumulator, so reading them off the lock is memory-safe (no torn buffer) — but it can still observe a
    ///     transient value while an append is in flight. A consumer that needs a consistent point-in-time view should subscribe to
    ///     <see cref="InvocationStateChanged" /> (whose args are an immutable clone); the scalar members are fine to read directly.
    /// </remarks>
    ...
    /// <summary>Raised once per tool-approval request the in-flight invocation is paused on.</summary>
    /// <remarks>
    ///     The local chat stream subscribes to surface these as <c>approval-requested</c> stream events so the browser
    ///     can render Approve/Deny controls on the waiting tool-call card.
    /// </remarks>
    event EventHandler<ApprovalRequestedChangedEventArgs>? ApprovalRequestedChanged;

    /// <summary>Raised once per <c>ask_user</c> question the in-flight invocation is paused on.</summary>
    /// <remarks>
    ///     The local chat stream subscribes to surface these as <c>question-requested</c> stream events so the browser
    ///     can render the question card. Unlike an approval, the payload carries the QUESTIONS themselves — a client
    ///     cannot render an answerable prompt from a correlation id alone, which is also what makes the reconnect
    ///     replay possible.
    /// </remarks>
    event EventHandler<UserQuestionRequestedChangedEventArgs>? UserQuestionRequestedChanged;

    /// <summary>Carries an operator's approval decision to the runner.</summary>
    /// <remarks>
    ///     <paramref name="scope" /> defaults to <see cref="ApprovalScope.Once" />; the loopback resolve endpoint can
    ///     ask instead for a decision that lasts the rest of the conversation.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     The cold-load phases fire BEFORE the stream-idle watchdog is armed, so the UI can render a legitimate load
    ///     rather than an apparent hang while a large local model warms. A no-op when the id is not the current
    ///     invocation.
    /// </remarks>
    ...
    /// </param>
    /// <remarks>
    ///     <paramref name="finishReason" /> is recorded on the terminal state as
    ///     <see cref="Events.InvocationState.FinishReason" />; it never changes the status, so a turn cut off at the
    ///     token budget still completes.
    /// </remarks>
    ...
    ///     onto the run-envelope row.
    /// </summary>
    /// <remarks>
    ///     Reported by the runner immediately BEFORE each terminal report, on the completed, cancelled and failed paths
    ///     alike, because the estimate is as interesting on a turn that ran out of context as on one that finished.
    ///     Counts only — no tool name ever reaches this seam.
    /// </remarks>
    ...
    ///     turn spent making a LOCAL runtime ready, and the token usage SUMMED over its provider rounds.
    /// </summary>
    /// <remarks>
    ///     Readiness covers launching <c>llama-server</c> and loading the model. Reported by the runner on the completed, cancelled and failed
    ///     paths alike, for the same reason as <see cref="ReportToolSchemaTokensAsync" />: a cold start and the tokens already paid for are
    ///     most interesting on the turn that paid for them, however that turn ended. Durations and counts only — no model identity or prompt
    ///     text reaches this seam.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     Reported by the runner immediately after the dispatch, and only on a turn that was authored <c>auto</c> —
    ///     every other turn leaves both members null. Category labels only: the tier and the authored effort, never a
    ///     reason code, a signal value or any message text.
    /// </remarks>
    ...
    /// <summary>
    ///     Records the model that ACTUALLY served the turn, when it is not the one the runtime package named.
    /// </summary>
    /// <remarks>
    ///     Reported by the runner only once an admitted <c>auto</c> model swap has run its send, because <see cref="Events.InvocationState.ModelUsed" />
    ///     is seeded from the package and is what BOTH the persisted message row and the run envelope's provider attribution are read from —
    ///     without this a swapped turn is recorded against a model that never saw it, and the measurement queries attribute the fast model's
    ///     tokens and latency to the big one. A turn that falls back to the original model never reports, so its seeded value stands.
    /// </remarks>
    ...
    ///     so a subscribed local chat stream can surface it as an <c>approval-requested</c> stream event.
    /// </summary>
    /// <remarks>
    ///     Distinct from <see cref="ReportApprovalRequestedAsync" />, which updates the invocation-monitor state; this
    ///     fans the request out to the local browser so the operator can resolve it.
    /// </remarks>
    ...
    /// <summary>Feeds the operator's answers into the waiting turn and clears the pending-question slot.</summary>
    /// <remarks>
    ///     Idempotent: an unknown or already-resolved request id logs and no-ops, so a duplicate or stale post never
    ///     faults the turn.
    /// </remarks>
...
/// <summary>
///     A turn's token usage SUMMED over its provider rounds — what the turn COST.
/// </summary>
/// <remarks>
///     A tool-calling turn is several provider requests inside one run, each reporting its own usage, so the totals add up. Deliberately NOT
///     the same numbers as <see cref="InvocationState.InputTokens" /> and friends, which stay the LAST round's: a round's prompt is the whole
///     conversation so far, so the final round's input count is what the model's context actually HELD, and that is the occupancy the chat
///     meter reads off the assistant message. Cost sums; occupancy does not. These totals are persisted onto the run-envelope row instead.
/// </remarks>
