...
/// <summary>Estimates the token footprint of chat messages for input-context budgeting.</summary>
/// <remarks>
///     An abstraction so the conservative character-heuristic default (<see cref="HeuristicTokenEstimator" />) can be
///     swapped for a provider-accurate estimator, such as one backed by the model's <c>/tokenize</c> endpoint, without
///     touching the budgeting policy. Estimates are advisory only: they bound how much history is sent, never what the
///     model bills.
/// </remarks>
public interface ITokenEstimator
{
    ...
    ///     <see cref="ITokenEstimatorCalibrationStore.ResolveObservedCorrection" />).
    /// </summary>
    /// <remarks>
    ///     Exposed here rather than by injecting the store into every budgeter: the correction and the divisor are two
    ///     halves of the same calibration, and both belong behind the estimator abstraction the budgeting policy
    ///     already depends on. The neutral default leaves an estimator that knows nothing about calibration unchanged.
    /// </remarks>
