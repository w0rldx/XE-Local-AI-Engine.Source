...
/// <summary>
///     Renders a turn's replayed <see cref="ConversationToolExchange" /> list as the message pair a provider expects
///     for a tool round: one assistant message carrying ONLY the call, then one tool message carrying its result.
/// </summary>
/// <remarks>
///     Shared rather than written twice on purpose. <c>InvocationRunner.BuildChatMessages</c> is what the model
///     actually receives and <c>ConversationStepContextBound.Project</c> is the estimate the fold decision rests on, so
///     a second rendering would make the bound measure something the turn does not send — the failure it exists to
///     prevent.
/// </remarks>
internal static class ConversationToolExchangeMessages
{
    /// <summary>Appends the call/result pair for every exchange, in list order.</summary>
    /// <remarks>
    ///     The assistant message deliberately carries no text part: Microsoft.Extensions.AI's OpenAI client takes its
    ///     tool-calls-only branch only when the message has no content part, and an empty text part alongside
    ///     <c>tool_calls</c> is what some chat templates reject.
    /// </remarks>
    public static void Append(List<ChatMessage> messages, IReadOnlyList<ConversationToolExchange> exchanges)
    {
        ...
        foreach (var exchange in exchanges)
        {
            ...
            // The persisted result is passed UNWRAPPED: a string result is emitted verbatim by the provider adapters, where a re-serialized object
            // would reach the model quoted and escaped. An error result replays as its text, which is what the model acted on when it was live.
