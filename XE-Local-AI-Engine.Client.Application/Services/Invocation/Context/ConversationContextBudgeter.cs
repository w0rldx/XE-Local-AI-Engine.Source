...
public sealed class ConversationContextBudgeter : IConversationContextBudgeter
{
        ...
        string? modelName = null)
    {
        ...
        var fixedOverhead = EstimateFixedOverhead(systemPrompt, toolDefinitions, modelName);
        var effectiveBudget = Math.Max(contextTokenCapacity - reservedOutputTokens - fixedOverhead, 0);
        var estimatedBefore = _estimator.EstimateTokens(messages, modelName);
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            working[i] = message;
            perMessageTokens[i] = _estimator.EstimateTokens(message, modelName);
        ...
        for (var i = 0; i < count && currentEstimate > effectiveBudget; i++)
        {
            ...
            working[i] = truncated;
            var newTokens = _estimator.EstimateTokens(truncated, modelName);
    ...
    /// </summary>
    private int EstimateFixedOverhead(string? systemPrompt, IReadOnlyList<string>? toolDefinitions, string? modelName)
    {
        var overhead = 0;

        if (!string.IsNullOrEmpty(systemPrompt))
        {
            overhead += _estimator.EstimateTokens(new ChatMessage(ChatRole.System, systemPrompt), modelName);
        }
        ...
        if (toolDefinitions is { Count: > 0 })
        {
            foreach (var definition in toolDefinitions)
            {
                ...
                overhead += _estimator.EstimateTokens(new ChatMessage(ChatRole.System, definition), modelName);
            }
        }
