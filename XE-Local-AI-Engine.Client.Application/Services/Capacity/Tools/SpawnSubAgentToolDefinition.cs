...
/// <summary>Worker-side name, description and parameter-schema constants for the <c>spawn_subagent</c> tool.</summary>
/// <remarks>
///     The handler advertises its model-visible schema from here and the offer provider merges the same descriptor into the offer, so the
///     schema the model is offered can never drift from what the handler validates. The schema is advisory to the model; the handler's own
///     validation and the capacity gate are authoritative.
/// </remarks>
internal static class SpawnSubAgentToolDefinition
{
    ...
    // `task.maxLength` and `instructions.maxLength` (8000 each) are deliberately NOT clamped, even though llama.cpp's GBNF converter cannot compile repetition
    // bounds that large: lowering them narrows the contract for every provider, so LlamaGrammarToolSchemaCompatibility (Providers.LlamaServer) sanitizes the wire form.
