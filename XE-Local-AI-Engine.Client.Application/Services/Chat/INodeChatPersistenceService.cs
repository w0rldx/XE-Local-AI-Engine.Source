...
public interface INodeChatPersistenceService
{
    ...
    /// null when the conversation does not exist. Used by the Origin=Remote mutation guard (Phase 1.5).
    /// </summary>
    ...
    /// <summary>Renames a conversation (Phase 4.2). Returns the updated conversation, or null if not found.</summary>
    ...
    /// <summary>Pins/unpins a conversation (Phase 4.2). Returns the updated conversation, or null if not found.</summary>
    ...
    /// <summary>Archives/unarchives a conversation, distinct from purge (Phase 4.2). Returns the updated conversation, or null if not found.</summary>
    ...
    /// <summary>
    /// Branches a conversation (Phase 5.1): clones every message up to and including the target message into a
    ...
    /// <summary>
    /// Records a regenerated assistant turn as a SIBLING VARIANT (Phase 5.2) — never an in-place overwrite. The
    ...
    /// <summary>Upserts node-local feedback (thumbs + optional comment) for a message (Phase 5.3). One row per message.</summary>
