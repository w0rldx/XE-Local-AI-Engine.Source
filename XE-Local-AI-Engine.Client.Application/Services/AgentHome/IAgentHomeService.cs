...
{
    /// <summary>
    ///     The single lifecycle entry the AgentHome gateway calls: it resolves owner/node identity once and acquires
    ///     the shared execution lease keyed by that owner-node.
    /// </summary>
    /// <remarks>
    ///     A run requested while another owner-node operation holds the lease is rejected with
    ///     <see cref="AgentHomeBusyException" />, not queued. Preparation and execution complete before the lease is
    ///     released.
    /// </remarks>
