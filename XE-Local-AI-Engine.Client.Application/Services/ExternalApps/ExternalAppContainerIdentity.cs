...
public static class ExternalAppContainerIdentity
{
        ...
        Func<int> groupIdReader)
    {
        ...
        return daemonIsRootless
            ? new ResolvedContainerIdentity(RootlessMappedId, RootlessMappedId)
            : new ResolvedContainerIdentity(userIdReader(), groupIdReader());
    }

    private static bool TryParseOption(string? option, out ResolvedContainerIdentity identity)
    {
        identity = new ResolvedContainerIdentity(0, 0);
        ...
        identity = new ResolvedContainerIdentity(userId, groupId);
        return true;
    }
