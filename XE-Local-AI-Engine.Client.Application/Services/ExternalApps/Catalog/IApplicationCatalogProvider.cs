...
/// <summary>Serves the curated External Apps catalog: bundled by default, optionally kept fresh from an operator-configured remote URL.</summary>
/// <remarks>
///     Fail-closed: the served snapshot has always passed <see cref="ExternalAppCatalogValidator.Validate" />, and a
///     fetch failure falls back to the last-good persisted copy, else the bundled seed, rather than surfacing to the
///     caller.
/// </remarks>
public interface IApplicationCatalogProvider
{
    ...
    /// <summary>Forces an immediate refresh attempt, ignoring the TTL, when a refresh URL is configured; a no-op returning the current snapshot otherwise.</summary>
    /// <remarks>
    ///     The result carries the snapshot now being served plus a human-readable
    ///     <see cref="ExternalAppCatalogRefreshResult.FailureMessage" /> when the attempt failed and the provider fell
    ///     back. The same string is carried on <see cref="ExternalAppCatalogSnapshot.LastRefreshFailure" />, so a
    ///     later read reports it without re-attempting a fetch.
    /// </remarks>
