...
| `update_node_settings` | agentic | Apply a partial update to the exact 16-field whitelist. | See **Settings whitelist** below. |
...
`update_node_settings` accepts only these 16 optional fields:
...
`speculative_draft_gpu_layers`, and `reranker_model_name`.
