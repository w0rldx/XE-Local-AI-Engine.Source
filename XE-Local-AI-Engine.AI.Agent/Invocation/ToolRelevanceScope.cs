...
///     <c>ToolResultBudgetScope</c> and <c>ProviderCallBudget</c>.
/// </summary>
/// <remarks>
///     The invocation runner seeds exactly one scope per turn, and the send-time hop several awaited frames below reads
///     it without a parameter on the MAF/<c>IChatClient</c> surface. <b>The slot holds a mutable holder, not a
///     value</b>, because an <see cref="AsyncLocal{T}" /> assignment made in a callee never propagates back out.
///     See docs/wiki/04-agent-mode.md ("The per-turn relevance scope").
/// </remarks>
...
///     the name sequence itself.
/// </summary>
/// <remarks>
///     Equality compares the hash first and then the names, so two arrays that hash alike are two DISTINCT keys with
///     two distinct decisions rather than one silently shared one — a collision costs one extra decision, never a
///     wrong answer.
/// </remarks>
internal readonly struct ArrayKey : IEquatable<ArrayKey>
{
    ...
    /// <summary>Builds a key with an explicit hash.</summary>
    /// <remarks>
    ///     Exists so a test can force two distinct name sequences to share a hash and prove the collision path keeps
    ///     two decisions.
    /// </remarks>
...
/// <summary>
///     The decision for one tool array: which names were offered, which were held back, and which of the held-back
///     ones a <c>list_tools</c> call has since revealed.
/// </summary>
/// <remarks>
///     The decision OBJECT, not a key, is what the <c>list_tools</c> instance is bound to, so a reveal always lands on
///     the array the model was actually looking at.
/// </remarks>
...
/// <summary>The mutable holder in the ambient slot, one per turn.</summary>
/// <remarks>
///     The per-array decisions under it are computed exactly once each and reused across the turn's rounds, so a
///     tool-calling loop pays one selection rather than one per round — which is also what keeps the prompt prefix and
///     the compiled GBNF grammar stable within the turn.
/// </remarks>
internal sealed class ToolRelevanceState
{
    ...
    /// <remarks>
    ///     The send-time hop asks before deciding whether it still needs a relevance query: one decision per array per
    ///     turn means a round carrying no text of its own — an approval resume — must reuse the decision rather than
    ///     fall through to the full array.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     <see cref="LazyThreadSafetyMode.ExecutionAndPublication" /> is exactly-once by contract, so every caller
    ///     after the first awaits the same task. <b>The shared computation takes no caller token</b>, and eviction is
    ///     keyed to the SHARED task's terminal state, never to a caller's cancelled wait. See
    ///     docs/wiki/04-agent-mode.md ("The per-turn relevance scope") for why both matter.
    /// </remarks>
    public async Task<ArrayDecision> GetOrComputeAsync(ArrayKey key, Func<Task<ArrayDecision>> factory, CancellationToken callerToken)
    {
        ...
        // Eviction cannot depend on a waiter being present, so this hangs off the SHARED task and fires even when
        // nobody awaits; the finally stays as the belt. Observing Exception keeps an unawaited fault off the scheduler.
        ...
        {
            // TERMINAL-but-unsuccessful, not merely faulted: a computation ending Canceled is not IsFaulted and caching
            // it would poison the key. An INCOMPLETE task fails IsCompleted, so a cancelled caller wait evicts nothing.
