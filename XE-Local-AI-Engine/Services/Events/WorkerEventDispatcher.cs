    ...
    public sealed class WorkerEventDispatcher : IWorkerEventDispatcher
    {
        ...
        public async Task DispatchInvocationAssignedAsync(RuntimePackage package)
        {
            ...
            lock (_syncRoot)
            {
                ...
                {
                    _logger.LogWarning(
                        "Ignoring invocation assignment for {InvocationId} because invocation {CurrentInvocationId} is still active.",
        ...
        public Task DispatchApprovalResolvedAsync(ApprovalResolvedEvent evt)
        {
            ...
            _logger.LogInformation(
                "Received approval resolution for request {RequestId}. Approved: {Approved}",
                evt.RequestId,
                evt.Approved);
        ...
        public Task DispatchInvocationCancelledAsync(InvocationCancelledEvent evt)
        {
            ...
            lock (_syncRoot)
            {
                ...
                {
                    _logger.LogDebug(
                        "Ignoring cancellation for {InvocationId} because it does not match the current invocation.",
                        evt.InvocationId);
