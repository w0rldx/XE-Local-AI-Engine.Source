...
export const devWorkflowConflictTypes = {
	 ...
	 */
	gateAlreadyDecided: "DevWorkflowGateAlreadyDecided",
} as const;

export interface DevWorkflowConflict {
	readonly conflictType: string;
	/** Set only for `DevWorkflowGateAlreadyDecided`: the `DevWorkflowDecisionKind` name already on the node run. */
 ...
 */
export function readDevWorkflowConflict(error: unknown): DevWorkflowConflict | undefined {
