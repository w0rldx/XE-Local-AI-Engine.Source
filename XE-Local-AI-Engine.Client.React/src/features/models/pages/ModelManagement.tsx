import { ActionIcon, Alert, Badge, Button, Card, Container, Group, Loader, Progress, SimpleGrid, Stack, Table, Text, TextInput, Title } from "@mantine/core";
...
export function ModelManagement() {
	...
	useEffect(() => {
		...
		const selectedStillExists = selectedModelName ? modelsResponse.items.some((model) => model.modelName === selectedModelName) : false;
		...
		<Container size="xl" py="lg">
			<Stack gap="lg">
				<Group justify="space-between" align="flex-start">
					...
					<Group gap="sm">
						<Badge color={modelsResponse?.isAvailable ? "green" : "red"}>{modelsResponse?.isAvailable ? "Ollama online" : "Ollama offline"}</Badge>
						<Button variant="subtle" leftSection={<IconRefresh size={16} />} onClick={() => modelsQuery.refetch()} disabled={modelsQuery.isFetching}>
							Refresh
						</Button>
					</Group>
				</Group>
				...
				<SimpleGrid cols={{ base: 1, lg: 2 }} spacing="lg">
					<Card withBorder={true} radius="md" p="lg">
						<Stack gap="md">
										...
										{modelViewModels.map((model) => (
											<Table.Tr key={model.modelName}>
												<Table.Td>
													...
													{model.isSelected ? <Badge ml="xs" color="green">Default</Badge> : null}
												</Table.Td>
												...
												<Table.Td>
													<Group gap="xs">
														<ActionIcon aria-label={`Set ${model.modelName} as default`} variant="subtle" color="green" disabled={isActionPending} onClick={() => selectMutation.mutate(model.modelName)}>
															<IconCheck size={16} />
														</ActionIcon>
														<ActionIcon aria-label={`Delete ${model.modelName}`} variant="subtle" color="red" disabled={isActionPending} onClick={() => confirmDelete(model.modelName)}>
															<IconTrash size={16} />
														</ActionIcon>
													</Group>
												</Table.Td>
							...
							{modelViewModels.length === 0 ? <Text c="dimmed">{modelsResponse?.isAvailable ? "No local models found." : "Ollama is unavailable."}</Text> : null}
						</Stack>
					</Card>
						...
						<Stack gap="md">
							...
							{selectedModel ? (
								<Stack gap="sm">
									...
									{detailsQuery.data?.template ? <Text style={{ whiteSpace: "pre-wrap" }}>Template: {detailsQuery.data.template}</Text> : null}
									{detailsQuery.data?.system ? <Alert color="blue">System prompt: {detailsQuery.data.system}</Alert> : null}
									{detailsQuery.data?.license ? <Text size="sm" c="dimmed" style={{ whiteSpace: "pre-wrap" }}>License: {detailsQuery.data.license}</Text> : null}
								</Stack>
							) : (
				...
				<Card withBorder={true} radius="md" p="lg">
					<Stack gap="md">
						...
						<TextInput data-testid="pull-model-name-input" label="Model name to pull" placeholder="orca-mini:latest" value={pullModelName} onChange={(event) => setPullModelName(event.currentTarget.value)} disabled={pullMutation.isPending} />
						<Group>
							<Button data-testid="download-model-button" leftSection={<IconCloudDownload size={16} />} disabled={!pullNameToSubmit || isActionPending} loading={pullMutation.isPending} onClick={() => pullMutation.mutate()}>
								Download model
							</Button>
						</Group>
