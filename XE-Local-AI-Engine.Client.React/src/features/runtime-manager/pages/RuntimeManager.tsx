...
async function followRuntimeLogs(
	...
	try {
		for await (const line of streamRuntimeLogs(
			{ containerName, tailLines: runtimeLogTailLines, follow: true },
...
export function RuntimeManager() {
	...
	// eslint-disable-next-line react-doctor/no-event-handler, react-doctor/no-chain-state-updates
	useEffect(() => {
				...
				{snapshot ? (
					<Tabs defaultValue="status" keepMounted={false}>
						<Tabs.List>
							...
							<Tabs.Tab value="components" leftSection={<IconCpu size={16} />}>
								Components
							</Tabs.Tab>
							<Tabs.Tab value="manifest" leftSection={<IconFileDescription size={16} />}>
								Manifest
							</Tabs.Tab>
							<Tabs.Tab value="logs" leftSection={<IconFileDescription size={16} />}>
								Logs
							</Tabs.Tab>
						</Tabs.List>
										...
										<Table verticalSpacing="sm">
											<Table.Tbody>
												<Table.Tr>
													...
													<Table.Td>{snapshot.status.state}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Desired state</Table.Td>
													<Table.Td>{snapshot.status.desiredState}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Runtime lifecycle</Table.Td>
													<Table.Td>{snapshot.status.runtimeLifecycle}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Bootstrap model ready</Table.Td>
													<Table.Td>{formatRuntimeBoolean(snapshot.status.bootstrapModelReady)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Web UI</Table.Td>
													<Table.Td>{formatRuntimeText(snapshot.status.webUiUrl)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Observed</Table.Td>
													<Table.Td>{formatRuntimeTimestamp(snapshot.status.observedAt)}</Table.Td>
												</Table.Tr>
											</Table.Tbody>
										</Table>
								...
								<Card withBorder={true} radius="md" p="lg">
									<Stack gap="md">
										...
										<Table verticalSpacing="sm">
											<Table.Tbody>
												<Table.Tr>
													<Table.Td>CPU available</Table.Td>
													<Table.Td>{formatRuntimeBoolean(snapshot.capabilities.cpuAvailable)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>NVIDIA GPU inference</Table.Td>
													<Table.Td>{formatRuntimeBoolean(snapshot.capabilities.nvidiaGpuInference)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>GPU runtime configured</Table.Td>
													<Table.Td>{formatRuntimeBoolean(snapshot.capabilities.gpuRuntimeConfigured)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>AMD GPU status</Table.Td>
													<Table.Td>{formatRuntimeText(snapshot.capabilities.amdGpuStatus)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Runtime disk</Table.Td>
													<Table.Td>{formatRuntimeBytes(snapshot.capabilities.runtimeDiskBytes)}</Table.Td>
												</Table.Tr>
												<Table.Tr>
													<Table.Td>Observed</Table.Td>
													<Table.Td>{formatRuntimeTimestamp(snapshot.capabilities.observedAt)}</Table.Td>
												</Table.Tr>
											</Table.Tbody>
										</Table>
											...
											<Table.Tbody>
												{sortedComponents.map((component) => (
													<Table.Tr key={component.name}>
														...
														<Table.Td>
															<Badge color={getComponentHealthColor(component.health)}>{component.health}</Badge>
														</Table.Td>
														<Table.Td>
						...
						<Tabs.Panel value="manifest" pt="lg">
							<Stack gap="lg">
								<Alert
									color={snapshot.manifest.available ? "blue" : "yellow"}
									icon={snapshot.manifest.available ? undefined : <IconAlertTriangle size={16} />}
								>
											...
											<Table verticalSpacing="sm">
												<Table.Tbody>
													<Table.Tr>
														...
														<Table.Td>{snapshot.manifest.schemaVersion ?? "Unknown"}</Table.Td>
													</Table.Tr>
													<Table.Tr>
														<Table.Td>Runtime mode</Table.Td>
														<Table.Td>{snapshot.manifest.runtimeMode}</Table.Td>
													</Table.Tr>
													<Table.Tr>
														<Table.Td>Bootstrap model</Table.Td>
														<Table.Td>{snapshot.manifest.bootstrapModel}</Table.Td>
													</Table.Tr>
													<Table.Tr>
														<Table.Td>Default chat model</Table.Td>
														<Table.Td>{snapshot.manifest.defaultChatModel}</Table.Td>
													</Table.Tr>
													<Table.Tr>
														<Table.Td>Runtime disk limit</Table.Td>
														<Table.Td>{snapshot.manifest.maxRuntimeDiskGb ?? "Unknown"} GB</Table.Td>
													</Table.Tr>
													<Table.Tr>
														<Table.Td>Stop drain timeout</Table.Td>
														<Table.Td>{snapshot.manifest.stopDrainTimeoutSeconds ?? "Unknown"} seconds</Table.Td>
													</Table.Tr>
												</Table.Tbody>
											</Table>
										</Stack>
											...
											<SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
												...
												<Stack gap="xs">
													...
													{container.volumes.map((volume) => (
														<Text key={`${volume.source}:${volume.target}`} size="sm">
															{volume.source} → {volume.target} ({volume.readOnly ? "read-only" : "read-write"})
														</Text>
													))}
								...
								<Stack gap="md">
									<Group justify="space-between" align="flex-end">
										<Stack gap={4}>
											...
											<Text c="dimmed">
												Tail and follow HostAgent container logs. The stream is cancelled when you stop following or leave the
												page.
											</Text>
										</Stack>
										<Group gap="sm" align="flex-end">
											...
											/>
											{isFollowingLogs ? (
												<Button variant="light" color="red" onClick={stopLogFollow}>
													Stop
												</Button>
											) : (
												<Button onClick={startLogFollow} disabled={!selectedLogContainer}>
													Follow logs
												</Button>
											)}
										</Group>
									...
									{logContainerOptions.length === 0 ? (
										<Text c="dimmed">No runtime containers are available for logs.</Text>
									) : null}
									...
									<ScrollArea h={360} type="auto">
										<Stack gap={4} p="xs" bg="dark.8" style={{ borderRadius: "var(--mantine-radius-sm)" }}>
											{runtimeLogLines.map((line) => (
												<Text
													key={`${line.observedAt}:${line.containerName}:${line.stream}:${line.line}`}
													component="pre"
													c="gray.1"
													size="sm"
													m={0}
