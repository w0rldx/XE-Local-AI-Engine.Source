		...
		<PageShell>
			...
			{status ? (
				<SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
					<SectionCard
						title={t("pages.dashboard.platformConnection.title")}
						actions={<Badge color={connectionStatusColor(status.state)}>{getConnectionStatusLabel(status.state)}</Badge>}
					>
					...
					<SectionCard
						title={t("pages.dashboard.startupConnection.title")}
						actions={
					...
					<SectionCard title={t("pages.dashboard.nodeCredentials.title")}>
							...
							<Table withTableBorder={true} withColumnBorders={true}>
								<Table.Tbody>
									...
									<Table.Tr>
										...
										<Table.Td>{status.bindingMethod ?? t("pages.dashboard.nodeCredentials.notAvailable")}</Table.Td>
									</Table.Tr>
									<Table.Tr>
										<Table.Th>{t("pages.dashboard.nodeCredentials.nodeName")}</Table.Th>
										<Table.Td>{status.lastKnownNodeName ?? t("pages.dashboard.nodeCredentials.notAvailable")}</Table.Td>
									</Table.Tr>
									<Table.Tr>
										<Table.Th>{t("pages.dashboard.nodeCredentials.tokenExpires")}</Table.Th>
										<Table.Td>
											<Group gap="xs" wrap="nowrap">
												<Text component="span">{tokenDisplay}</Text>
												{tokenExpired ? (
					...
					<SectionCard title={t("pages.dashboard.lastUpdate.title")}>
						<Text>
