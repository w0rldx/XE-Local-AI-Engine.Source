...
function tooltip({ isAuthoritative, maxTokens, modelLabel, nodeLabel, usedTokens }: ContextUsageModel): string {
	const usedOrigin = usedTokens === undefined
	...
	const maxOrigin = maxTokens === undefined ? "Max context unknown for this model on this node" : "Max context reported by selected node";
...
export function ContextUsageBadge(props: ContextUsageModel) {
	...
	const percent = usedTokens !== undefined && maxTokens !== undefined && maxTokens > 0 ? (usedTokens / maxTokens) * 100 : undefined;
	...
	const accessibleText = percent === undefined
	...
	return (
		<Tooltip label={tooltip(props)} multiline={true} withArrow={true}>
			...
			>
				...
				{percent !== undefined ? <Progress size={4} radius="xl" value={Math.min(percent, 100)} color={color} aria-hidden="true" style={{ flex: 1 }} /> : null}
				<span style={{ position: "absolute", width: 1, height: 1, overflow: "hidden", clip: "rect(0 0 0 0)" }}>{accessibleText}</span>
			</div>
		</Tooltip>
	);
}
