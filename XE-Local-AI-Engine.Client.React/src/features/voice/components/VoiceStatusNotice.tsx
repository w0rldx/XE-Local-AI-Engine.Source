...
import { IconHandClick, IconInfoCircle } from "@tabler/icons-react";
...
// Voice status banners (plan §7.3, §3.8, §10): the one-time model-download progress/error (dismissible), the
// "tap to enable audio" gesture hint while the AudioContext is still suspended, and a capability/fallback notice
// (e.g. "using browser speech — WebGPU unavailable"). Self-gates on the runtime context; renders nothing when voice
// is off. NOTE (M1): in the default delivery path the Kokoro worker downloads weights through Transformers.js' own
// cache, so the ModelCache-driven progress bar is the seam for the self-hosted/prefetch path and only shows when
// ModelCache actually fetches.
...
export function VoiceStatusNotice() {
	...
	const { enabled, capabilities, audioSuspended, downloadProgress, downloadError, dismissDownloadNotice } = useVoiceRuntime();
	...
	if (!showProgress && downloadError === undefined && !audioSuspended && !showFallbackNotice) {
		return null;
	}
	...
	return (
		<Stack gap={6} mb="xs" data-testid="voice-status-notice">
			{audioSuspended ? (
				<Alert color="blue" variant="light" icon={<IconHandClick size={16} />} p="xs">
					<Text size="xs">{t("voice.notice.tapToEnable")}</Text>
				</Alert>
			) : null}
			{showProgress ? (
