...
import { IconPlayerStop, IconReload, IconTrash } from "@tabler/icons-react";
...
export function SourceBuildCard() {
	...
	const status = useSourceBuildStatus();
	const runtime = useLlamaCppRuntimeStatus();
	const start = useStartSourceBuild();
	const cancel = useCancelSourceBuild();
	const remove = useRemoveSourceBuild();
	const hub = useSourceBuildHub();
	...
	// The build form opens itself exactly when it is the answer: a build to watch, or a failure to retry. Both come
	// off the status reads, which cost nothing — deciding this by probing would pay the very cost the disclosure
	// exists to defer. llama.cpp records no validity on its installed runtime, so it has no third condition.
	const buildForm = useSourceBuildFormDisclosure(isBuilding || liveError !== null);
	// The probe runs the toolchain, so it is asked for only while the form that needs its answer is on screen.
	const prerequisites = useSourceBuildPrerequisites(backend, buildForm.opened);
	...
	return (
		<Card withBorder={true} radius="md" p="lg" data-testid="source-build-card">
			<Stack gap="md">
				...
				{isBuilding || installed?.isSourceBuild ? (
					<Group>
						{isBuilding ? (
							<Button
								color="yellow"
				...
				<SourceBuildFormDisclosure opened={buildForm.opened} onToggle={buildForm.toggle} testId="source-build-form">
					...
					<SourceBuildPrerequisiteList
						...
						translationPrefix="pages.nodeSettings.llamaCpp.sourceBuild"
					/>
