...
import { getInvocationMonitor, type InvocationCurrentDto, type InvocationHistoryDto } from "@/features/invocations/api/InvocationsApi";
import { formatInvocationDuration, formatInvocationText, formatInvocationTimestamp, getInvocationStatusColor, isInvocationActive, sortInvocationHistory } from "@/features/invocations/models/InvocationMonitorModel";
    ...
    <Card withBorder={true} radius="md" p="lg">
      <Stack gap="md">
        <Group justify="space-between" align="flex-start">
          <Stack gap={4}>
            <Title order={3}>Current invocation</Title>
            <Text size="sm" c="dimmed" style={{ wordBreak: "break-all" }}>{current.invocationId}</Text>
          </Stack>
        ...
        {current.error ? <Alert color="red" icon={<IconAlertTriangle size={16} />}>{current.error}</Alert> : null}
      </Stack>
    </Card>
  );
}
...
function HistoryRows({ history }: { readonly history: InvocationHistoryDto[] }) {
  ...
  return history.map((entry) => (
    <Table.Tr key={entry.invocationId}>
      <Table.Td><Text size="sm" style={{ wordBreak: "break-all" }}>{entry.invocationId}</Text></Table.Td>
      <Table.Td><Badge color={getInvocationStatusColor(entry.status)}>{entry.status}</Badge></Table.Td>
      ...
      <Stack gap="lg">
        <Group justify="space-between" align="flex-start">
          ...
          <Group gap="sm">
            ...
            <Button variant="subtle" leftSection={<IconRefresh size={16} />} onClick={() => monitorQuery.refetch()} disabled={monitorQuery.isFetching}>
              Refresh
            </Button>
          </Group>
        </Group>
          ...
          <Stack gap="md">
            <Group justify="space-between">
              <Stack gap={2}>
                ...
                <Text size="sm" c="dimmed">Showing up to {monitor?.historyCapacity ?? 0} most recent terminal invocations.</Text>
              </Stack>
              <IconHistory size={22} />
            </Group>
