...
import type { InvocationMonitorResponseDto } from "@/features/invocations/api/InvocationsApi";

const { apiMock } = vi.hoisted(() => ({
  apiMock: {
    getInvocationMonitor: vi.fn(),
  },
}));

vi.mock("@/features/invocations/api/InvocationsApi", () => apiMock);
...
describe("Invocations", () => {
  ...
  beforeEach(() => {
    ...
    apiMock.getInvocationMonitor.mockResolvedValue(createMonitor());
  });

  it("renders current invocation and history", async () => {
  ...
  it("renders idle and empty history states", async () => {
    apiMock.getInvocationMonitor.mockResolvedValue({ current: null, history: [], historyCapacity: 50 });
  ...
  it("refreshes monitor data", async () => {
    renderWithProviders(<Invocations />);
    ...
    await waitFor(() => expect(apiMock.getInvocationMonitor).toHaveBeenCalledTimes(2));
  });
});

function createMonitor(): InvocationMonitorResponseDto {
  return {
    current: {
