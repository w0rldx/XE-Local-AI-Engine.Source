// Ambient stubs for the two Graph Workflows reads every `/chat` render makes (the Chat definitions for the workflow
// picker, and the selected conversation's bound runs). The Chat page tests stub the generated TanStack options rather
// than installing MSW, so without these the reads would hit the no-network guard and fail silently as query errors.
// Spread into a `vi.mock("@/core/api/generated/@tanstack/react-query.gen", …)` factory after `importOriginal()`.
//
// Both answer empty: no chat workflow exists and no run is bound, which is the normal-chat baseline those tests assert.
