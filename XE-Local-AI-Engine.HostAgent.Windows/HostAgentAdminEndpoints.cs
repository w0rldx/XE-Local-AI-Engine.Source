...
public static class HostAgentAdminEndpoints
{
    public static void UseLocalAdminRequestGuards(this WebApplication app)
    {
        ...
        {
            if (!IsLoopbackRequest(context) || !HasAllowedHost(context) || !HasAllowedOrigin(context))
            {
    ...
    public static void MapLocalAdminEndpoints(this WebApplication app)
    {
        app.MapGet("/status", async (HttpContext context,
            HostAgentSecretStore secretStore,
            HostAgentAdminService adminService,
            CancellationToken cancellationToken) =>
        {
            if (!await IsAuthorizedAsync(context, secretStore, cancellationToken).ConfigureAwait(false))
            {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            var lines = await adminService.ReadLogsAsync(tail ?? 200, cancellationToken).ConfigureAwait(false);
            return Results.Ok(new HostAgentLogTail(lines));
        });
    ...
    private static async Task<bool> IsAuthorizedAsync(HttpContext context,
        ...
        CancellationToken cancellationToken)
    {
        if (!TryGetBearerToken(context, out var token) || !OperatingSystem.IsWindows())
        {
            return false;
        }

        var expectedToken = await secretStore.GetOrCreateAdminTokenAsync(cancellationToken).ConfigureAwait(false);
    ...
    private static bool HasAllowedOrigin(HttpContext context)
    {
