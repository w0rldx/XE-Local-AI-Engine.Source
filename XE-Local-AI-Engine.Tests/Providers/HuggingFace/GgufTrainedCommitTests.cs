...
public sealed class GgufTrainedCommitTests
{
    ...
    public async Task Commit_MergedTrainedModel_CarriesOriginTrainedAndItsDerivedFromLineage()
    {
        ...
        {
            AbsolutePath = source
        }, MergedDestination(), progress: null, CancellationToken.None);
    ...
    public async Task Commit_TrainedAdapter_MirrorsItsOwnBytesIntoTheAdapterMembersAndNamesItsBaseModel()
    {
        ...
        {
            AbsolutePath = source
        }, AdapterDestination(), progress: null, CancellationToken.None);
    ...
    public async Task Prepare_TrainedDestinationWithoutLineage_IsRejected()
    {
        ...
        _ = await AssertEx.ThrowsAsync<ArgumentException>(() => importer.PrepareAsync(new GgufImportSource
            {
                AbsolutePath = source
            },
    ...
    public async Task Prepare_TrainedAdapterOnThePublicImportPath_IsStillRejected()
    {
        ...
        var failure = await AssertEx.ThrowsAsync<GgufImportException>(() => importer.PrepareAsync(new GgufImportSource
            {
                AbsolutePath = source
            },
            MergedDestination(),
    ...
    private static GgufImportDestination MergedDestination() =>
        new()
        {
            ...
            Lineage = new TrainedModelLineage
            {
                DerivedFromRepoId = "meta/base",
                DerivedFromRevision = "main",
                DerivedFromContentFingerprint = "v1:dataset"
            }
        };
