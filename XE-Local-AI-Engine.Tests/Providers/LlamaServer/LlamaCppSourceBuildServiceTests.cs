...
public sealed class LlamaCppSourceBuildServiceTests
{
    ...
    public async Task AppendLog_ConcurrentCallbacks_PersistAndPublishInIdenticalOrder()
    {
        ...
        using var service = new LlamaCppSourceBuildService(new AlwaysReadyProbe(), new CapturingBinaryManager(store, signal), store, signal,
            new LeaseOnlySupervisor(), new LlamaCppSourceBuildActivity(), publisher, NullLogger<LlamaCppSourceBuildService>.Instance, TimeProvider.System, temp.Path);
    ...
    public async Task Cancel_CustomCpu_RetainsTheActiveRuntimeAndAllowsAFreshStart()
    {
        ...
        var start = await service.StartAsync(new LlamaCppSourceBuildRequest
        {
        ...
        var repeated = await service.StartAsync(new LlamaCppSourceBuildRequest
        {
            Backend = LlamaCppSourceBackend.Cpu,
            Source = LlamaCppSourceSelection.Custom,
            Repository = "https://github.com/example/fork",
