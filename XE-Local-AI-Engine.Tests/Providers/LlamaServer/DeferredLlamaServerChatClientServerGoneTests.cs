...
public sealed class DeferredLlamaServerChatClientServerGoneTests
{
    ...
    public async Task BoundBenchmarkEndpoint_BypassesSupervisorAndExternalEndpointResolution()
    {
        ...
        var externalEndpoint = new LlamaServerEndpoint
        {
            ModelName = "model-a",
            Role = ModelRole.Chat,
            BaseAddress = new Uri("http://external.example/v1")
        };
        ...
        var pinnedEndpoint = new LlamaServerEndpoint
        {
            ModelName = "model-a",
            Role = ModelRole.Chat,
            BaseAddress = new Uri("http://127.0.0.1:19002/v1")
        };
    ...
    public async Task GetResponse_OnABoundBenchmarkEndpoint_TakesNoLeaseAndNeverReEnsures()
    {
        ...
        var binding = Substitute.For<ILlamaServerEndpointBinding>();
        var boundEndpoint = new LlamaServerEndpoint
        {
            ModelName = "model-a",
            Role = ModelRole.Chat,
            BaseAddress = ProfilingEndpoint
        };
