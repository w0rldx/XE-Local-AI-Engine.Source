...
public sealed class DatasetGenerationQueueTests
{
    ...
    public void EventBuffer_ReplaysAfterCursorAndDemandsResetOnceEvicted()
    {
        ...
        _ = buffer.Append(datasetId, DatasetGenerationEventKind.State, new DatasetGenerationPayload
        {
            State = "Generating"
        });
        var second = buffer.Append(datasetId, DatasetGenerationEventKind.SampleAdded, new DatasetGenerationPayload
        {
            Completed = 1,
            Total = 4
        });
    ...
    public void EventBuffer_TrimsToItsBound_AndThenDemandsReset()
    {
        ...
        {
            _ = buffer.Append(datasetId, DatasetGenerationEventKind.Progress, new DatasetGenerationPayload
            {
                Completed = index
            });
        }
