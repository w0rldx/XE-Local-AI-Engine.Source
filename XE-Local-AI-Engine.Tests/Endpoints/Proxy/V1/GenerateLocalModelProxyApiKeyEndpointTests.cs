...
public sealed class GenerateLocalModelProxyApiKeyEndpointTests
{
    ...
    public async Task Generate_WhenAnonymous_Returns401()
    {
        ...
        using var response = await LocalModelProxyApiKeyRequests.AnonymousAsync(client, HttpMethod.Post);
    ...
    public async Task Generate_WhenAuthenticatedButNotOperator_Returns403()
    {
        ...
        using var response = await LocalModelProxyApiKeyRequests.AsNonOperatorAsync(Factory, client, HttpMethod.Post);
    ...
    public async Task Generate_RevealsPlaintextExactlyOnceAndRotatesOnRegenerate()
    {
        ...
        var generated = AssertEx.NotNull(await minted.Content.ReadFromJsonAsync<GeneratedLocalModelProxyApiKeyResponse>());
        ...
        using var secondStatus = await LocalModelProxyApiKeyRequests.AsOperatorAsync(Factory, client, HttpMethod.Get);
        var secondBody = await secondStatus.Content.ReadAsStringAsync();
        ...
        using var rotated = await LocalModelProxyApiKeyRequests.AsOperatorAsync(Factory, client, HttpMethod.Post);
        AssertEx.Equal(HttpStatusCode.OK, rotated.StatusCode);
        var regenerated = AssertEx.NotNull(await rotated.Content.ReadFromJsonAsync<GeneratedLocalModelProxyApiKeyResponse>());
