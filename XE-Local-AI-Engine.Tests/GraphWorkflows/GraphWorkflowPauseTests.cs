...
public sealed class GraphWorkflowPauseTests
{
    ...
    public async Task ADispatchedPause_ParksTheRowAndTheRunAndAsksForADecision()
    {
        ...
        var nodeRun = await harness.ReadNodeRunAsync(runId, "review");
    ...
    public async Task Approve_FiresTheApproveEdgeAndTheRunReachesItsEnd()
    {
        ...
        var result = await harness.DecideAsync(runId, "review", Guid.NewGuid(), GraphWorkflowDecisionKind.Approve);
        ...
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Succeeded, (await harness.ReadNodeRunAsync(runId, "shipped")).Status);
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Skipped, (await harness.ReadNodeRunAsync(runId, "rejected")).Status);
    ...
    public async Task Reject_FiresTheRejectEdgeRatherThanFailingTheNode()
    {
        ...
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Succeeded, (await harness.ReadNodeRunAsync(runId, "rejected")).Status);
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Skipped, (await harness.ReadNodeRunAsync(runId, "shipped")).Status);
    }
    ...
    {
        await using var harness = new GraphWorkflowHarness(Host);
        var runId = await ParkedRunAsync(harness, GraphWorkflowGraphs.PauseStrandedRejection, "review");
        ...
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Skipped, (await harness.ReadNodeRunAsync(runId, "stranded")).Status);
    }
    ...
    public async Task TheSameOperationIdTwice_AnswersTheSameResultAndWritesTheDecisionOnce()
    {
        ...
        var first = await harness.DecideAsync(runId, "review", operationId, GraphWorkflowDecisionKind.Approve, "looks fine");
        var replay = await harness.DecideAsync(runId, "review", operationId, GraphWorkflowDecisionKind.Approve, "trimmed");

        AssertEx.Equal(first.Decision, replay.Decision);
        AssertEx.Equal(first.NodeRunStatus, replay.NodeRunStatus);
        var decided = (await harness.ReadEventsAsync(runId)).Count(entry => entry.EventType == GraphWorkflowEventTypes.GateDecided);
    ...
    public async Task TheSameOperationIdUnderADifferentSubject_IsRefusedWithTheStandingAnswer()
    {
        ...
        AssertEx.Equal("alice",
            (await harness.ReadNodeRunAsync(runId, "review")).DecidedBySubject,
    ...
    public async Task AnOperationIdReusedOnASecondPauseOfTheSameRun_IsRefusedRatherThanBreakingTheIndex()
    {
        ...
        var operationId = Guid.NewGuid();
        _ = await harness.DecideAsync(runId, "first", operationId, GraphWorkflowDecisionKind.Approve);
        ...
        AssertEx.Equal(GraphWorkflowNodeRunStatus.WaitingForApproval,
            (await harness.ReadNodeRunAsync(runId, "second")).Status,
    ...
    public async Task DecidingANodeRunThatIsNotWaiting_IsRefused()
    {
        ...
        await harness.TransitionNodeRunAsync(runId, "first", GraphWorkflowNodeRunStatus.Running);
    ...
    public async Task APayloadOverHalfTheOutputBudget_IsABadRequestAndOneJustUnderItComposes()
    {
        ...
        var stored = await harness.ReadNodeRunAsync(accepted, "review");
    ...
    public async Task TheStoredAnswerAndThePreFlightDocument_SpellTheDecisionTheSameWay(GraphWorkflowDecisionKind decision)
    {
        ...
        _ = await harness.DecideAsync(runId, "review", Guid.NewGuid(), decision, "why not");

        var stored = await harness.ReadNodeRunAsync(runId, "review");
    ...
    private static async Task<Guid> ParkedRunAsync(GraphWorkflowHarness harness, string graphJson, string nodeKey)
    {
        ...
        await AdvanceUntilWaitingAsync(harness, runId, nodeKey);
        return runId;
    }
