...
public sealed class FirstRunModelProvisioningServiceTests
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        return new FirstRunModelProvisioningService(configuration,
            ...
            coordinator,
            SubstitutedRuntimeOrchestration.Over(binaryManager: binaryManager,
                variantSelector: variantSelector ?? new FakeVariantSelector(),
                acquisitionStatus: acquisitionStatus ?? new RecordingAcquisitionStatusRegistry()),
            settingsStore,
    ...
    private sealed class RecordingBinaryManager : ILlamaCppBinaryManager
    {
        ...
        public Task<LlamaBinary> EnsureBinaryAsync(GpuVariant variant, CancellationToken ct)
        {
            ...
            AcquisitionStatus?.Report(new RuntimeAcquisitionUpdate
            {
            ...
            {
                ServerExecutablePath = "/fake/llama-server",
                Version = "b9692",
                Variant = variant,
        ...
        {
            return Task.FromResult(EnsureCalled
                ? new LlamaBinary
                {
                    ServerExecutablePath = "/fake/llama-server",
                    Version = "b9692",
    ...
    private sealed class FakeDownloadCoordinator : IGgufDownloadCoordinator
    {
        ...
        public GgufDownloadStatus? GetStatus(string modelName)
        {
            return new GgufDownloadStatus
            {
                ...
                CompletedBytes = null,
                TotalBytes = null,
                SanitizedError = _terminalPhase == GgufDownloadPhase.Failed ? "Download failed." : null
            };
        }
