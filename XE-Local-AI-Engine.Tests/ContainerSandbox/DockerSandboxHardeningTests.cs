...
public sealed class DockerSandboxHardeningTests
{
    ...
    {
        AssertEx.Contains(DockerSandboxHardening.FindViolations(Specification(), Conformant() with
            {
                PidMode = "host"
            }),
    ...
    public void BuildSpecification_MountsABoundedTmpfsAtTheToolchainTemporaryDirectory()
    {
        ...
        var specification = DockerSandboxHardening.BuildSpecification(Options() with
            {
                TempSizeMb = 64
            },
            new ResolvedContainerIdentity(UserId: 1000, GroupId: 1000),
    ...
    public void FindViolations_VerifiesTheRequestedNetworkModeRatherThanAssumingNone()
    {
        ...
        AssertEx.Empty(DockerSandboxHardening.FindViolations(bridge, Conformant() with
        {
            NetworkMode = "bridge"
        }));
        AssertEx.ContainsSingle(DockerSandboxHardening.FindViolations(bridge, Conformant() with
            {
                NetworkMode = "none"
            }),
    ...
    {
        var exception = await AssertEx.ThrowsAsync<SandboxCapabilityNotSupportedException>(() => Task.FromResult(DockerSandboxHardening.ResolveNetworkMode(SandboxNetworkPolicy.Restricted)));
