...
public sealed class SkillImportServiceTests
{
    ...
    public async Task Commit_WithoutAcknowledgementWritesNothing()
    {
        ...
        await AssertEx.ThrowsAsync<SkillImportException>(() =>
            harness.Service.CommitAsync(new SkillImportCommitRequest
            {
                Token = preview.Token,
    ...
    public async Task Commit_LandsTheSkillDisabledWithImportedProvenance()
    {
        ...
        var result = await harness.Service.CommitAsync(new SkillImportCommitRequest
        {
            Token = preview.Token,
            SkillNames = ["pdf-tools"],
            Acknowledged = true
    ...
    public async Task Commit_GitHubSourceRecordsTheRepositoryProvenance()
    {
        ...
        await harness.Service.CommitAsync(new SkillImportCommitRequest
        {
            Token = preview.Token,
            SkillNames = ["pdf-tools"],
            Acknowledged = true
    ...
    public async Task Commit_DefaultsToSkippingAConflictAndCanBeToldToReplace()
    {
        ...
        var skipped = await harness.Service.CommitAsync(new SkillImportCommitRequest
        {
            Token = preview.Token,
            SkillNames = ["pdf-tools"],
            Acknowledged = true
    ...
    public async Task Commit_RejectsASelectionThatIsNotInTheApprovedPreview()
    {
        ...
        await AssertEx.ThrowsAsync<SkillImportException>(() =>
            ...
            {
                Token = preview.Token,
                SkillNames = ["pdf-tools", "other-skill"],
                Acknowledged = true
            }));
