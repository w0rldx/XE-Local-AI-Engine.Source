...
public sealed class DevWorkflowEventPublisherTests
{
    [Test]
    [Arguments(DevWorkflowChangeKind.Run, "run")]
    [Arguments(DevWorkflowChangeKind.Node, "node")]
    [Arguments(DevWorkflowChangeKind.Gate, "gate")]
    [Arguments(DevWorkflowChangeKind.Artifact, "artifact")]
    public async Task PublishAsync_SendsTheChangeToTheRunGroupWithALowercaseKind(DevWorkflowChangeKind kind, string expected)
    {
        var runId = Guid.NewGuid();
        var proxy = Substitute.For<IClientProxy>();
        var clients = Substitute.For<IHubClients>();
        clients.Group($"dev-workflow-run-{runId:N}").Returns(proxy);
        var hubContext = Substitute.For<IHubContext<DevWorkflowRunHub>>();
        ...
        await proxy.Received(1)
                   .SendCoreAsync("devWorkflowChanged",
                       Arg.Is<object?[]>(arguments => arguments.Length == 1
                                                      && arguments[0] is DevWorkflowChanged
                                                      && ((DevWorkflowChanged)arguments[0]!).RunId == runId
                                                      && ((DevWorkflowChanged)arguments[0]!).Seq == 42
                                                      && ((DevWorkflowChanged)arguments[0]!).Kind == expected),
