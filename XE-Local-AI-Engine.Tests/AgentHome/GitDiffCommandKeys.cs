...
///     (<c>executable + " " + string.Join(" ", arguments)</c>), for the two <c>git diff</c> commands the Marker G
///     patch export issues. Deriving the keys from <see cref="AgentHomeGit" /> keeps the test in lockstep with the
///     production command shape — if the §9.1 flags change, both move together.
/// </summary>
