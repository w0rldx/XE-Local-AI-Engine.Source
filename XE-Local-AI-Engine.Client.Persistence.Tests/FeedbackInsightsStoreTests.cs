...
public sealed class FeedbackInsightsStoreTests : IDisposable
{
    ...
    private static async Task InsertMessageAsync(DbConnection connection, Guid messageId, Guid conversationId, long createdAtUtc, Guid? agentDefinitionId)
    {
        ...
        command.CommandText = """
                              ...
                              VALUES ($message_id, $conversation_id, (SELECT COALESCE(MAX(sequence), -1) + 1 FROM messages WHERE conversation_id = $conversation_id), 'assistant', 'answer', $created_at_utc, $created_at_utc, $agent_definition_id);
                              """;
