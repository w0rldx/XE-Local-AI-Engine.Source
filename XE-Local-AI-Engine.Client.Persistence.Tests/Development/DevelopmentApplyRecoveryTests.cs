...
public sealed class DevelopmentApplyRecoveryTests : IDisposable
{
    ...
    public async Task CrashAfterHostMutation_SameKeyFinalizesWithoutApplyingTwice()
    {
        ...
        await using var provider = await _fixture.BuildProviderAsync(port);
        ...
        await AssertEx.ThrowsAsync<InvalidOperationException>(() => coordinator.ApplyAsync(operationId, subject, repository));
        var completed = await coordinator.ApplyAsync(operationId, subject, repository);
    ...
    public async Task CompletedApply_ResponseReplayDoesNotInspectOrMutateHostAgain()
    {
        ...
        var repository = Repository(seed);
        var completed = await coordinator.ApplyAsync(operationId, subject, repository);
        var replay = await coordinator.ApplyAsync(operationId, subject, repository);
    ...
    public async Task AmbiguousHostState_BlocksIdempotentlyWithoutApplying()
    {
        ...
        var repository = Repository(seed);
        var blocked = await coordinator.ApplyAsync(operationId, subject, repository);
        var replay = await coordinator.ApplyAsync(operationId, subject, repository);
