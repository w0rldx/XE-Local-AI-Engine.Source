...
public sealed class BenchmarkPairwiseStoreTests : IDisposable
{
    ...
    public async Task EnsureComparisonsAsync_CreatesBothOrdersOfEverySlot_AndBumpsTheComparisonSetVersion()
    {
        ...
        var cohort = await store.GetPairwiseCohortAsync(project.Id);
        ...
        AssertEx.Equal(expected: 0, await store.EnsureComparisonsAsync(project.Id, Slots(runs), Runtime(), null));
        AssertEx.Equal(expected: 1, (await store.GetPairwiseCohortAsync(project.Id)).ComparisonSetVersion,
    ...
    public async Task EnsureComparisonsAsync_AfterAFailedComparison_ReEnqueuesThatSlotAtTheNextAttemptSequence()
    {
        ...
        await store.MarkComparisonFailedAsync(claimed.QueueSequence, claimed.Version, "the judge invocation failed");
    ...
    public async Task ComparisonSetVersion_MovesOnEveryTerminalization_SoAFitOverARebuiltSetIsStale()
    {
        ...
        await store.MarkComparisonSucceededAsync(new BenchmarkComparisonSuccessCommand
        {
            QueueSequence = first.QueueSequence,
            ExpectedWorkVersion = first.Version,
            Verdict = "a",
            ResultJson = null,
        ...
        var afterTerminalizations = (await store.GetPairwiseCohortAsync(project.Id)).ComparisonSetVersion;
    ...
    public async Task MarkComparisonSucceededAsync_ClaimsTheCohortForItsExecutionKey()
    {
        ...
        var promoted = AssertEx.NotNull(await store.GetJudgePolicyRevisionAsync(revision.Id));
    ...
    public async Task PublishPairwiseFitAsync_SwitchesThePointerInOneTransaction_AndRefusesADuplicateKey()
    {
        ...
        AssertEx.True(await store.PublishPairwiseFitAsync(Fit(project, revision, "v1:second", setVersion: 2, runs)));

        context.ChangeTracker.Clear();
        var stored = await context.BenchmarkPairwiseFits.AsNoTracking().Where(fit => fit.ProjectId == project.Id).ToListAsync();
        ...
        AssertEx.Equal("v1:second", AssertEx.NotNull(await store.GetActivePairwiseFitAsync(project.Id)).FitKey);

        // A racing second terminalization recomputes the SAME key from the same inputs. It must no-op, not mint a twin.
        AssertEx.False(await store.PublishPairwiseFitAsync(Fit(project, revision, "v1:second", setVersion: 2, runs)));
        AssertEx.Equal("v1:second", AssertEx.NotNull(await store.GetActivePairwiseFitAsync(project.Id)).FitKey,
    ...
    public async Task Ranking_PairwiseMode_ReadsScoresFromTheActiveFit_AndAnOperatorScoreStillWins()
    {
        ...
        AssertEx.True(await store.PublishPairwiseFitAsync(Fit(project, revision, "v1:ranked", setVersion, runs, [69, 50, 31])));
        ...
        var overridden = await store.SetUserScoreAsync(runs[2], 100, ranked.Items.Single(run => run.Id == runs[2]).Version);
        var afterOverride = await store.ListAllRunsAsync(project.Id);
    ...
    public async Task Ranking_ActiveFitWasFitOverAnOlderComparisonSet_IsNotRankedAndReadsPairwiseStale()
    {
        ...
        _ = await store.EnsureComparisonsAsync(project.Id, Slots(runs), Runtime(), null);
        var setVersion = (await store.GetPairwiseCohortAsync(project.Id)).ComparisonSetVersion;
        AssertEx.True(await store.PublishPairwiseFitAsync(Fit(project, revision, "v1:stale", setVersion, runs, [70, 30])));

        // One comparison terminalizes, so the set the fit covered is no longer the set on disk.
        var claimed = await ClaimComparisonAsync(store);
        await store.MarkComparisonFailedAsync(claimed.QueueSequence, claimed.Version, "interrupted");
    ...
    public async Task Ranking_RunAbsentFromTheActiveFit_ReadsPairwiseInsufficient()
    {
        ...
        _ = await store.EnsureComparisonsAsync(project.Id, Slots(runs), Runtime(), null);
        var setVersion = (await store.GetPairwiseCohortAsync(project.Id)).ComparisonSetVersion;
        AssertEx.True(await store.PublishPairwiseFitAsync(Fit(project, revision, "v1:partial", setVersion, [runs[0], runs[1]], [60, 40])));
    ...
    public async Task DeleteRunAsync_WhileAComparisonNamingItIsQueued_IsRefusedFromEitherSideOfThePair()
    {
        ...
        var refusedB = await AssertEx.ThrowsAsync<BenchmarkConflictException>(() => DeleteAsync(store, runB));
        var refusedA = await AssertEx.ThrowsAsync<BenchmarkConflictException>(() => DeleteAsync(store, runA));

        AssertEx.Equal("ActiveRun", refusedB.Code);
        AssertEx.Equal("ActiveRun", refusedA.Code);
        AssertEx.Equal(expected: 2, (await store.GetPairwiseCohortAsync(project.Id)).Comparisons.Count);
    }
    ...
    public async Task DeleteRunAsync_OfAFittedParticipant_RemovesItsComparisonsAndRetiresTheFit()
    {
        ...
        AssertEx.True(await store.PublishPairwiseFitAsync(Fit(project, revision, "v1:fitted", beforeVersion, runs, [70, 50, 30])));

        await DeleteAsync(store, runs[2]);
        ...
        AssertEx.Null(await store.GetActivePairwiseFitAsync(project.Id));
        context.ChangeTracker.Clear();
        ...
        AssertEx.Equal(expected: 0, await store.EnsureComparisonsAsync(project.Id, Slots([runs[0], runs[1]]), Runtime(), null));
    }
    ...
    public async Task DeleteRunAsync_OnAPointwiseProject_TouchesNoPairwiseState()
    {
        ...
        await DeleteAsync(store, runs[1]);

        // No comparison ever named this run, so nothing pairwise is invalidated by its removal.
        AssertEx.Equal("v1:untouched", AssertEx.NotNull(await store.GetActivePairwiseFitAsync(project.Id)).FitKey);
    }

    private static async Task DeleteAsync(BenchmarkStore store, Guid runId) =>
        await store.DeleteRunAsync(runId, AssertEx.NotNull(await store.GetRunAsync(runId)).Version);
    ...
    private static async Task<(BenchmarkProjectRecord Project, BenchmarkJudgePolicyRevisionRecord Revision, Guid[] Runs)> SeedCohortAsync(BenchmarkStore store,
        int runCount)
    {
        var created = await store.CreateProjectAsync(CreateProject());
        var activation = await store.ActivateJudgePolicyAsync(created.Id, created.Version, PolicyBytes, PolicyHash);
        var project = AssertEx.NotNull(await store.GetProjectAsync(created.Id));
        List<Guid> runs = [];
        ...
        var cohort = await store.GetPairwiseCohortAsync(project.Id);
        return (AssertEx.NotNull(await store.GetProjectAsync(project.Id)),
            activation.Revision,
    ...
    private static async Task<(long QueueSequence, long Version, Guid ComparisonId, int Order)> ClaimComparisonAsync(BenchmarkStore store)
    {
        ...
        var comparison = AssertEx.NotNull(await store.GetComparisonAsync(comparisonId));
    ...
    private static BenchmarkLaunchReceiptCommand Receipt() =>
        new()
        {
            ...
            EnvironmentFactsJson = "{\"facts\":true}",
            EnvironmentFactsHash = "facts-hash",
