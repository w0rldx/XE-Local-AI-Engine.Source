...
public sealed class AddBenchmarkJudgePoliciesMigrationTests
{
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies.sqlite");

        AssertEx.True(await probe.TableExistsAsync("benchmark_judge_policy_revisions"), "Judge policy revisions must exist.");
        AssertEx.True(await probe.TableExistsAsync("benchmark_judge_attempts"), "Judge attempts must exist.");
        AssertEx.True((await probe.ColumnsAsync("benchmark_judge_policy_revisions")).IsSupersetOf(new[]
            {
                "id",
                "project_id",
                "revision",
        ...
        AssertEx.True((await probe.ColumnsAsync("benchmark_judge_attempts")).IsSupersetOf(new[]
            {
                "run_id",
                "sequence",
                "policy_revision_id",
        ...
        AssertEx.True((await probe.ColumnsAsync("benchmark_work_items")).Contains("judge_attempt_id"),
            "Judge work must name the attempt it judges.");
        AssertEx.True((await probe.ColumnsAsync("benchmark_runs")).Contains("current_judge_attempt_id"),
            "A run must point at its current attempt.");
        AssertEx.True((await probe.ColumnsAsync("benchmark_projects")).Contains("current_judge_policy_revision_id"),
        ...
        AssertEx.True(await probe.IndexExistsAsync("benchmark_work_items", "ux_benchmark_work_items_primary_run", unique: true, "run_id"),
            "The primary work index must be filtered on kind, not composite with it.");
        AssertEx.True(await probe.IndexExistsAsync("benchmark_work_items", "ux_benchmark_work_items_judge_attempt", unique: true, "judge_attempt_id"),
            "Judge work must be unique per attempt.");
        AssertEx.False(await probe.IndexExistsAsync("benchmark_work_items", "ux_benchmark_work_items_run_kind", unique: true, "run_id", "kind"),
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-purge.sqlite", PreviousMigration);
        await SeedLegacyBenchmarkRowsAsync(probe);
        AssertEx.Equal(expected: 1L, await probe.ScalarAsync("SELECT COUNT(*) FROM benchmark_runs;"));

        await probe.MigrateToAsync(ThisMigration);

        // The feature is unused, so the migration drops the rows rather than mapping them.
        AssertEx.Equal(expected: 0L, await probe.ScalarAsync("SELECT COUNT(*) FROM benchmark_work_items;"));
        AssertEx.Equal(expected: 0L, await probe.ScalarAsync("SELECT COUNT(*) FROM benchmark_runs;"));
        AssertEx.Equal(expected: 0L, await probe.ScalarAsync("SELECT COUNT(*) FROM benchmark_projects;"));
    }

    [Test]
    public async Task Migrate_Down_RemovesTheJudgeTablesAndRestoresTheOldWorkIndex()
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-down.sqlite");

        await probe.MigrateToAsync(PreviousMigration);


        AssertEx.False(await probe.TableExistsAsync("benchmark_judge_attempts"), "Down must drop the attempts table.");
        AssertEx.False(await probe.TableExistsAsync("benchmark_judge_policy_revisions"), "Down must drop the revisions table.");
        AssertEx.False((await probe.ColumnsAsync("benchmark_work_items")).Contains("judge_attempt_id"), "Down must drop the attempt pointer.");
        AssertEx.True(await probe.IndexExistsAsync("benchmark_work_items", "ux_benchmark_work_items_run_kind", unique: true, "run_id", "kind"),
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-user-score.sqlite");
        await SeedProjectAndRunAsync(probe);

        await probe.ExecuteAsync("UPDATE benchmark_runs SET user_score = 0;");
        await probe.ExecuteAsync("UPDATE benchmark_runs SET user_score = 100;");
        await probe.ExecuteAsync("UPDATE benchmark_runs SET user_score = NULL;");
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-attempt-checks.sqlite");
        await SeedProjectAndRunAsync(probe);
        var revisionId = await SeedRevisionAsync(probe, "policy-hash-a");

        await InsertAttemptAsync(probe, Guid.NewGuid(), revisionId, sequence: 1, status: "Succeeded", score: 0);
        await InsertAttemptAsync(probe, Guid.NewGuid(), revisionId, sequence: 2, status: "Succeeded", score: 100);
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-work-checks.sqlite");
        await SeedProjectAndRunAsync(probe);
        var revisionId = await SeedRevisionAsync(probe, "policy-hash-a");
        var attemptId = Guid.NewGuid();
        await InsertAttemptAsync(probe, attemptId, revisionId, sequence: 1, status: "Queued", score: null);
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-filtered-indexes.sqlite");
        await SeedProjectAndRunAsync(probe);
        var revisionId = await SeedRevisionAsync(probe, "policy-hash-a");
        var firstAttemptId = Guid.NewGuid();
        var secondAttemptId = Guid.NewGuid();
        await InsertAttemptAsync(probe, firstAttemptId, revisionId, sequence: 1, status: "Succeeded", score: 60);
        await InsertAttemptAsync(probe, secondAttemptId, revisionId, sequence: 2, status: "Queued", score: null);

        await InsertWorkItemAsync(probe, "Primary", judgeAttemptId: null);
        ...
        await InsertWorkItemAsync(probe, "Judge", firstAttemptId);
        // The point of the filtered index: a second judge item is fine, a second item for the SAME attempt is not.
        await InsertWorkItemAsync(probe, "Judge", secondAttemptId);
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("benchmark-judge-policies-hash-unique.sqlite");
        await SeedProjectAndRunAsync(probe);
        _ = await SeedRevisionAsync(probe, "policy-hash-a");
        _ = await SeedRevisionAsync(probe, "policy-hash-b", revision: 2);
    ...
    {
        await SeedProjectAndRunAsync(probe, atHead: false);
        await probe.ExecuteAsync("""
                                 ...
                                 """,
            command => command.Parameters.AddWithValue("$run", RunId.ToString()));
    }
