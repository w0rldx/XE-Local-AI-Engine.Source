...
public sealed class AddMcpServerApiKeyScopeMigrationTests
{
    ...
    public async Task Migrate_ExistingKey_BackfillsDelegateScope()
    {
        ...
        AssertEx.Equal("0", await probe.ColumnDefaultAsync("mcp_server_api_keys", "scope"));
        AssertEx.Equal(expected: 1L,
            (await probe.LongsAsync("SELECT \"notnull\" FROM pragma_table_info('mcp_server_api_keys') WHERE name = 'scope';")).Single(),
            "Scope must be non-null so no key has ambiguous authority.");
        AssertEx.Equal(expected: 1L,
            (await probe.LongsAsync("SELECT \"notnull\" FROM pragma_table_info('mcp_server_api_keys') WHERE name = 'generation_id';")).Single(),
            "The conditional last-used write requires every row to carry a generation token.");
        AssertEx.Equal(expected: 0L,
            (await probe.LongsAsync("SELECT scope FROM mcp_server_api_keys;")).Single(),
            "Legacy credentials must remain delegate credentials after migration.");

        var tableSql = Convert.ToString(await probe.ScalarAsync("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'mcp_server_api_keys';"));
        ...
        _ = await AssertEx.ThrowsAsync<SqliteException>(() =>
            probe.ExecuteAsync("UPDATE mcp_server_api_keys SET scope = 2;"));
    }
}
