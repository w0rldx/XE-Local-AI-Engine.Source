...
public sealed class AddMcpAgenticRunAuthorityMigrationTests
{
    ...
    public async Task Migrate_AddsDelegateDefaultAndRejectsInconsistentAuthority()
    {
        ...
        var tableSql = AssertEx.NotNull(Convert.ToString(await probe.ScalarAsync(
            "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'mcp_agent_runs';").ConfigureAwait(false)));
        ...
        AssertEx.Null(await probe.ScalarAsync(
            "SELECT requesting_key_prefix FROM mcp_agent_runs WHERE request_id = '24fd2ec2-eab3-4dd4-b0d5-052414a61751';").ConfigureAwait(false));
