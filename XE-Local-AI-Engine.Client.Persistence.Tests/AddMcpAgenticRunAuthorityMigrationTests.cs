...
public sealed class AddMcpAgenticRunAuthorityMigrationTests
{
    ...
    public async Task Migrate_AddsDelegateDefaultAndRejectsInconsistentAuthority()
    {
        ...
        AssertEx.Equal("0", await probe.ColumnDefaultAsync("mcp_agent_runs", "is_agentic_auto_approve"));
        AssertEx.Equal(expected: 1L,
            (await probe.LongsAsync("SELECT \"notnull\" FROM pragma_table_info('mcp_agent_runs') WHERE name = 'is_agentic_auto_approve';")).Single());
        var tableSql = AssertEx.NotNull(Convert.ToString(await probe.ScalarAsync("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'mcp_agent_runs';")));
        AssertEx.True(tableSql.Contains("CK_mcp_agent_runs_agentic_authority", StringComparison.Ordinal));
        AssertEx.Equal(expected: 0L,
            (await probe.LongsAsync("SELECT is_agentic_auto_approve FROM mcp_agent_runs WHERE request_id = '24fd2ec2-eab3-4dd4-b0d5-052414a61751';")).Single(),
            "Durable runs accepted before scoped authority existed must backfill as delegate.");
        AssertEx.Equal(expected: 0L,
            (await probe.LongsAsync("SELECT status FROM mcp_agent_runs WHERE request_id = '24fd2ec2-eab3-4dd4-b0d5-052414a61751';")).Single(),
            "A migrated queued delegate must remain queued for dispatcher restart execution.");
        AssertEx.Null(await probe.ScalarAsync("SELECT requesting_key_prefix FROM mcp_agent_runs WHERE request_id = '24fd2ec2-eab3-4dd4-b0d5-052414a61751';"));
