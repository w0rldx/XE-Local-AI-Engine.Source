...
public sealed class ExternalAppInstanceStoreTests
{
    ...
    public async Task CreateAsync_WritesTheRowAndItsFirstEventAtSequenceOne()
    {
        ...
        var events = await store.ListEventsAsync(command.Id, afterSequence: 0, limit: 10);
    ...
    public async Task ListByApplicationAsync_ReturnsOnlyThatApplicationsInstances()
    {
        ...
        _ = await store.CreateAsync(odysseus);
        _ = await store.CreateAsync(other);

        var matches = await store.ListByApplicationAsync("odysseus");

        AssertEx.Equal(expected: 1, matches.Count);
        AssertEx.Equal(odysseus.Id, matches[0].Id);
        AssertEx.Empty(await store.ListByApplicationAsync("nothing-installed"));
        AssertEx.Equal(expected: 2, (await store.ListAsync()).Count, "and the unfiltered list still sees both.");
    }
    ...
    public async Task UpdateStatusAsync_WhenVersionIsStale_WritesNothingIncludingTheEvent()
    {
        ...
        AssertEx.Equal(expected: 1L, await fixture.RawTableCountAsync("external_app_instance_events"),
    ...
    public async Task UpdateStatusAsync_MintsMonotonicSequencesAcrossManyTransitions()
    {
        ...
        {
            var result = await store.UpdateStatusAsync(ExternalAppTestFixture.Transition(command.Id, version, next, status, kind));
        ...
        var events = await store.ListEventsAsync(command.Id, afterSequence: 0, limit: 100);
    ...
    public async Task UpdateStatusAsync_WhenTwoWritersRaceOnOneRow_ExactlyOneWins()
    {
        ...
        {
            var command = ExternalAppTestFixture.Create();
            _ = await new ExternalAppInstanceStore(seedContext).CreateAsync(command);
        ...
        var tracked = await slow.ExternalAppInstances.SingleAsync(row => row.Id == instanceId);
        ...
        var snapshot = AssertEx.NotNull(await new ExternalAppInstanceStore(reader).GetAsync(instanceId));
    ...
    public async Task UpdateStatusAsync_NullOptionalFields_LeaveTheStoredValuesAlone()
    {
        ...
        var seeded = await store.UpdateStatusAsync(new ExternalAppStatusUpdate
        {
            InstanceId = command.Id,
    ...
    public async Task UpdateStatusAsync_WithClearFailure_ErasesAStaleCategoryAndSummary()
    {
        ...
        var failed = await store.UpdateStatusAsync(new ExternalAppStatusUpdate
        {
            InstanceId = command.Id,
            ExpectedVersion = created.Version,
            ExpectedStatuses = new HashSet<ExternalAppInstanceStatus>
        ...
        var afterFailure = AssertEx.NotNull(await store.GetAsync(command.Id));
        AssertEx.Equal((ExternalAppFailureCategory?)ExternalAppFailureCategory.ImagePullFailed, afterFailure.FailureCategory);

        var recovered = await store.UpdateStatusAsync(new ExternalAppStatusUpdate
        {
            InstanceId = command.Id,
    ...
    public async Task UpdateStatusAsync_CanClearNeedsRecreate()
    {
        ...
        var version = AssertEx.NotNull(await store.GetAsync(command.Id)).Version;

        // The start that rebuilt the containers against the new environment is what pays the debt off.
        var started = await store.UpdateStatusAsync(new ExternalAppStatusUpdate
        {
            InstanceId = command.Id,
        ...
        AssertEx.False(AssertEx.NotNull(await store.GetAsync(command.Id)).NeedsRecreate);
    }
    ...
    public async Task CommitUpdateAsync_WithABridgeToken_SealsItOntoARowThatHadNone()
    {
        ...
        var created = await store.CreateAsync(command);
        AssertEx.Null(AssertEx.NotNull(await store.GetAsync(command.Id)).BridgeToken,
    ...
    public async Task CommitUpdateAsync_WithoutABridgeToken_LeavesTheRowsOwnTokenAlone()
    {
        ...
        AssertEx.Equal(ExternalAppTestFixture.BridgeTokenFor(command.Id),
            AssertEx.NotNull(await store.GetAsync(command.Id)).BridgeToken);
    }
    ...
    public async Task DeleteAsync_RemovesTheEventsAsWellAsTheRow()
    {
        ...
        AssertEx.False(await store.DeleteAsync(doomed.Id, expectedVersion: version + 5));
        AssertEx.Equal(expected: 2L, await fixture.RawTableCountAsync("external_app_instances"));

        AssertEx.True(await store.DeleteAsync(doomed.Id, version));
        ...
        AssertEx.NotNull(await store.GetAsync(survivor.Id));
        AssertEx.Equal(expected: 1L, await fixture.RawTableCountAsync("external_app_instance_events"), "and its own event survives.");
    }
    ...
    public async Task ListEventsAsync_PagesBySequenceAndRejectsANonPositiveLimit()
    {
        ...
        {
            var applied = await store.UpdateStatusAsync(ExternalAppTestFixture.Transition(command.Id, version, next, status));
            AssertEx.True(applied.Applied);
            version = applied.Version;
            status = next;
        }

        var firstPage = await store.ListEventsAsync(command.Id, afterSequence: 0, limit: 2);
        AssertEx.True(firstPage.Select(row => row.Sequence).SequenceEqual([1L, 2L]));

        var secondPage = await store.ListEventsAsync(command.Id, firstPage[^1].Sequence, limit: 2);
    ...
    public async Task AppendedEventDetail_OverFourKibibytes_IsRejected()
    {
        ...
        var failure = await AssertEx.ThrowsAsync<ArgumentException>(() => store.UpdateStatusAsync(new ExternalAppStatusUpdate
        {
            InstanceId = command.Id,
            ExpectedVersion = created.Version,
            ExpectedStatuses = new HashSet<ExternalAppInstanceStatus>
        ...
        _ = await AssertEx.ThrowsAsync<ArgumentException>(() => store.CreateAsync(ExternalAppTestFixture.Create(firstEventDetailJson: oversized)));

        // Exactly at the bound is legal: it is 4096 bytes, not 4096 minus one.
        var atTheBound = await store.UpdateStatusAsync(new ExternalAppStatusUpdate
        {
            InstanceId = command.Id,
            ExpectedVersion = created.Version,
            ExpectedStatuses = new HashSet<ExternalAppInstanceStatus>
