...
public sealed class IntegrationApiKeyStoreTests
{
    ...
    public async Task CreateAsync_RoundTripsTheDigestAndSupportsManyKeysPerPrincipal()
    {
        ...
        var listed = await readStore.ListAsync();
        ...
        var byPrefix = AssertEx.NotNull(await readStore.GetByPrefixAsync("xeint_aaaaaaaa"));
        ...
        AssertEx.Null(await readStore.GetByPrefixAsync("xeint_nosuchkey"));
    }
    ...
    public async Task TouchLastUsedAsync_StampsTheTimestampWithoutRewritingTheSealedDigest()
    {
        ...
        AssertEx.True(await store.TouchLastUsedAsync(keyId, atUtc: 7_000));
        AssertEx.False(await store.TouchLastUsedAsync(Guid.NewGuid(), atUtc: 7_000));

        var sealedAfter = await ReadSealedHashAsync(fixture, keyId);
        ...
        await using var readContext = fixture.CreateContext();
        var read = AssertEx.NotNull(await new IntegrationApiKeyStore(readContext, new FixedTimeProvider(FixedNow)).GetByPrefixAsync("xeint_aaaaaaaa"));
    ...
    public async Task RevokeAsync_IsASoftStampThatDeletesNothingAndDoesNotRepeat()
    {
        ...
        _ = await store.CreateAsync(new IntegrationApiKeyCreateCommand
        {
            ...
            PrincipalId = Guid.NewGuid(),
            KeyPrefix = "xeint_aaaaaaaa",
            KeyHash = SHA256.HashData("k"u8.ToArray()),
            Label = "Ingest",
            AllowedTriggerIdsJson = null
        });
        ...
        AssertEx.False(await store.RevokeAsync(Guid.NewGuid(), atUtc: 8_100));
