...
public sealed class TrainingDatasetStoreTests : IDisposable
{
    ...
    private static TrainingSampleInput Sample(Guid datasetId, string sourceHash) =>
        new()
        {
            ...
            ContentJson = Encoding.UTF8.GetBytes($$"""{"schemaVersion":1,"parts":[{"kind":"user","sequence":0,"content":"{{sourceHash}}"}]}"""),
            ValidationJson = Encoding.UTF8.GetBytes("""{"schemaVersion":1,"passed":true,"layers":[]}"""),
