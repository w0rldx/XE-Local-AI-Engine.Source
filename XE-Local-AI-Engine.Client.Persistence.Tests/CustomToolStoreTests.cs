...
public sealed class CustomToolStoreTests : IDisposable
{
    ...
    public async Task UpdateAsync_BumpsVersionOnConfigChange_NotOnEnabledOrAckToggle()
    {
        ...
        clock.Advance(10);
        var toggled = AssertEx.NotNull(await store.UpdateAsync(added.Id, new CustomToolInput
            {
                Name = Name,
                Description = Description,
                Kind = CustomToolKind.HttpFetch,
        ...
        var edited = AssertEx.NotNull(await store.UpdateAsync(added.Id, new CustomToolInput
            {
                Name = Name,
                Description = Description,
                Kind = CustomToolKind.HttpFetch,
        ...
        clock.Advance(10);
        var reMode = AssertEx.NotNull(await store.UpdateAsync(added.Id, new CustomToolInput
            {
                Name = Name,
                Description = Description,
                Kind = CustomToolKind.HttpFetch,
    ...
    public async Task Name_IsNocaseUnique()
    {
        ...
        _ = await store.CreateAsync(new CustomToolInput
        {
        ...
        var exception = AssertEx.Throws<DbUpdateException>(() => store.CreateAsync(new CustomToolInput
            {
                Name = "custom__WEATHER",
                Description = Description,
                Kind = CustomToolKind.HttpFetch,
