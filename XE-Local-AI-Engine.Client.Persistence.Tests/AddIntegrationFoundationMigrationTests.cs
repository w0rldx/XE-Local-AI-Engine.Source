...
public sealed class AddIntegrationFoundationMigrationTests
{
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("integration-foundation.sqlite");
        ...
        {
            AssertEx.True(await probe.TableExistsAsync(table), $"{table} must exist after the migration chain.");
        }

        AssertEx.Equal("'chat'", await probe.ColumnDefaultAsync("conversations", "kind"),
            "A conversation with no explicit kind must read as an ordinary chat.");
        AssertEx.Equal("0", await probe.ColumnDefaultAsync("integration_executions", "output_bytes"),
            "The plaintext output-byte tally starts at zero rather than NULL, because every write adds to it.");

        AssertEx.True((await probe.ColumnsAsync("integration_executions")).Contains("stop_requested_at_utc"),
            "The durable cancel marker must exist, or the cancel path cannot survive a restart.");
        AssertEx.Equal(expected: 0L, await NotNullAsync(probe, "integration_executions", "stop_requested_at_utc"),
        ...
        {
            AssertEx.True((await probe.ColumnsAsync(table)).Contains("principal_id"), $"{table} must carry the ownership column.");
            AssertEx.Equal(expected: 1L, await NotNullAsync(probe, table, "principal_id"),
                $"{table}.principal_id is NOT NULL — no row can predate the column, because every table here is new in this migration.");
        }

        AssertEx.True(await probe.IndexExistsAsync("integration_triggers", "ux_integration_triggers_name", unique: true, "name"));
        AssertEx.True(await probe.IndexExistsAsync("integration_triggers", "ix_integration_triggers_enabled", unique: false, "enabled"));
        AssertEx.True(await probe.IndexExistsAsync("integration_api_keys", "ux_integration_api_keys_prefix", unique: true, "key_prefix"));
        AssertEx.True(await probe.IndexExistsAsync("integration_sessions", "ux_integration_sessions_conversation_id", unique: true, "conversation_id"));
        AssertEx.True(await probe.IndexExistsAsync("integration_sessions", "ix_integration_sessions_trigger", unique: false, "trigger_id"));
        ...
        AssertEx.True(await probe.IndexExistsAsync("integration_executions", "ix_integration_executions_session", unique: false, "session_id"));
    ...
    {
        await using var probe = await MigrationSchemaProbe.FromChatTemplateAsync("integration-kind-backfill.sqlite", PreviousMigrationId);
        ...
        AssertEx.Equal("work-session", await KindAsync(probe, OwnedConversationId),
            "The transcript a work session owns must stop appearing in the chat list after the upgrade.");
        AssertEx.Equal("chat", await KindAsync(probe, PlainConversationId),
        ...
        await probe.ExecuteAsync(BackfillSql);
        AssertEx.Equal("work-session", await KindAsync(probe, OwnedConversationId));
        AssertEx.Equal("chat", await KindAsync(probe, PlainConversationId));
    }
    ...
    {
        var value = await probe.ScalarAsync($"SELECT \"notnull\" FROM pragma_table_info('{table}') WHERE name = '{column}';");
        return Convert.ToInt64(value, CultureInfo.InvariantCulture);
    }

    private static async Task<string?> KindAsync(MigrationSchemaProbe probe, string conversationId)
    {
        var value = await probe.ScalarAsync($"SELECT kind FROM conversations WHERE conversation_id = '{conversationId}';");
