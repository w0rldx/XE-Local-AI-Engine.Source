...
public sealed class GraphWorkflowRunStoreTests
{
    ...
    public async Task StartRun_WithARequestIdThatAlreadyWon_AnswersTheRunThatHoldsIt()
    {
        ...
        var first = await store.StartRunAsync(StartCommand(definition, requestId));
        var second = await store.StartRunAsync(StartCommand(definition, requestId));
    ...
    public async Task StartRun_CommitsTheRunItsNodeRunsAndItsFirstEventTogether()
    {
        ...
        var nodeRuns = await store.ListNodeRunsAsync(run.Id);
        ...
        var events = await store.ListEventsAsync(run.Id);
    ...
    public async Task StartRun_AgainstADefinitionDeletedInTheMeantime_AnswersNotFoundAndWritesNoRun()
    {
        ...
        _ = await AssertEx.ThrowsAsync<GraphWorkflowNotFoundException>(() => store.StartRunAsync(StartCommand(definition, Guid.NewGuid())));

        AssertEx.Equal(expected: 0, await fixture.RawTableCountAsync("graph_workflow_runs"));
    }
    ...
    public async Task StartRun_AgainstADefinitionEditedInTheMeantime_IsRefused()
    {
        ...
        var store = GraphWorkflowTestFixture.StoreFor(context);
        var definition = await GraphWorkflowTestFixture.SeedDefinitionAsync(store);
        _ = await store.UpdateDefinitionAsync(new UpdateGraphWorkflowDefinitionCommand
        {
    ...
    public async Task TransitionRun_WithAStaleExpectedVersion_IsRefused()
    {
        ...
        _ = await AssertEx.ThrowsAsync<GraphWorkflowInvalidTransitionException>(() =>
            store.TransitionRunAsync(new TransitionGraphWorkflowRunCommand
            {
                RunId = run.Id,
                ExpectedVersion = 99,
    ...
    public async Task ListInterruptedNodeRuns_TakesQueuedAndRunningOnly()
    {
        ...
        await using var context = await fixture.CreateSchemaAsync();
        var store = GraphWorkflowTestFixture.StoreFor(context);
        var run = await StartAsync(store, GraphWorkflowTestFixture.FourNodeGraph, ["queued", "running", "waiting", "pending"]);
        await MoveAsync(store, run.Id, "queued", GraphWorkflowNodeRunStatus.Queued);
        await MoveAsync(store, run.Id, "running", GraphWorkflowNodeRunStatus.Running);
        await MoveAsync(store, run.Id, "waiting", GraphWorkflowNodeRunStatus.Running);
        await MoveAsync(store, run.Id, "waiting", GraphWorkflowNodeRunStatus.WaitingForApproval);
    ...
    public async Task ReconcileNonTerminalNodeRuns_AppliesTheCollapseAndItsVerdictsInOneCommitAndIsIdempotent()
    {
        ...
        var queued = await store.GetNodeRunAsync(run.Id, "queued");
        var running = await store.GetNodeRunAsync(run.Id, "running");
        ...
        [
            ...
            {
                NodeRunId = queued.Id,
                ObservedStatus = GraphWorkflowNodeRunStatus.Queued,
                ObservedAttempt = 1,
                Repairs = []
            },
        ...
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Pending, (await store.GetNodeRunAsync(run.Id, "queued")).Status);

        var failed = await store.GetNodeRunAsync(run.Id, "running");
        ...
        AssertEx.Equal(GraphWorkflowNodeRunStatus.WaitingForApproval,
            (await store.GetNodeRunAsync(run.Id, "waiting")).Status,
            "a pause is a durable human wait and survives a restart untouched.");

        AssertEx.Empty(await store.ReconcileNonTerminalNodeRunsAsync("the host restarted", []),
    ...
    public async Task ReconcileNonTerminalNodeRuns_WithASettlement_FailsWhateverNoVerdictMatched()
    {
        ...
        var store = GraphWorkflowTestFixture.StoreFor(context);
        var run = await StartAsync(store);
        await MoveAsync(store, run.Id, "start", GraphWorkflowNodeRunStatus.Running);
        ...
        _ = await store.ReconcileNonTerminalNodeRunsAsync("the host restarted",
            ...
            {
                FailureClass = GraphWorkflowFailureClass.Interrupted,
                SanitizedReason = "nobody could judge this row"
            });
    ...
    public async Task ReconcileNonTerminalNodeRuns_WithNoVerdictAndNoSettlement_LeavesTheRowAlone()
    {
        ...
        AssertEx.Empty(await store.ReconcileNonTerminalNodeRunsAsync("the host restarted", []));
        AssertEx.Equal(GraphWorkflowNodeRunStatus.Running, (await store.GetNodeRunAsync(run.Id, "start")).Status);
    }
    ...
    public async Task CountActiveRuns_CountsTheExecutingRunsNoFurtherThanItsProbeLimit()
    {
        ...
        AssertEx.Equal(expected: 2, await store.CountActiveRunsAsync(probeLimit: 2), "counting past the cap is work nobody reads.");
    }
    ...
    public async Task StartRun_WithARequestIdAlreadyHeldByAnotherDefinitionsRun_StillAnswersTheRunThatWon()
    {
        ...
        await using var context = await fixture.CreateSchemaAsync();
        var store = GraphWorkflowTestFixture.StoreFor(context);
        var first = await GraphWorkflowTestFixture.SeedDefinitionAsync(store, "First");
        var second = await GraphWorkflowTestFixture.SeedDefinitionAsync(store, "Second");
        var requestId = Guid.NewGuid();

        var won = await store.StartRunAsync(StartCommand(first, requestId));
        var lost = await store.StartRunAsync(StartCommand(second, requestId));
    ...
    public async Task ListEvents_ForARunThatDoesNotExist_AnswersNotFoundRatherThanAnEmptyPage()
    {
        ...
        _ = await AssertEx.ThrowsAsync<GraphWorkflowNotFoundException>(() => store.ListEventsAsync(Guid.NewGuid()));
    }
    ...
    public async Task TransitionRun_ToCompleted_WritesTheOutputAndTheCompletionInstant()
    {
        ...
        AssertEx.True((await store.ListEventsAsync(run.Id)).Any(static entry => entry.EventType == "run.completed"),
    ...
    public async Task DecideNodeRun_OnARowThatIsNotOpenToOne_WritesNothingAndAnswersNull(GraphWorkflowNodeRunStatus staged)
    {
        ...
        if (staged == GraphWorkflowNodeRunStatus.WaitingForApproval)
        {
            ...
            await MoveAsync(store, run.Id, "start", GraphWorkflowNodeRunStatus.WaitingForApproval);
            _ = await DecideAsync(store, run.Id, Guid.NewGuid());
        }

        var before = await store.GetNodeRunAsync(run.Id, "start");
        var eventsBefore = (await store.ListEventsAsync(run.Id)).Count;

        var result = await DecideAsync(store, run.Id, Guid.NewGuid());

        AssertEx.Null(result, "the conditional write matched no row, and says so rather than raising.");
        var after = await store.GetNodeRunAsync(run.Id, "start");
        AssertEx.Equal(before.Status, after.Status);
        AssertEx.Equal<Guid?>(before.DecisionOperationId, after.DecisionOperationId);
        AssertEx.Equal(eventsBefore, (await store.ListEventsAsync(run.Id)).Count, "a declined mutation appends nothing.");
    }
    ...
    public async Task DecideNodeRun_OnceTheRunStoppedBeingLive_WritesNothingAndAnswersNull(GraphWorkflowRunStatus stopped)
    {
        ...
        AssertEx.Null(await DecideAsync(store, run.Id, Guid.NewGuid()), "a run that stopped being live declines the answer.");

        var untouched = await store.GetNodeRunAsync(run.Id, "start");
        ...
        AssertEx.Equal(eventsBefore, (await store.ListEventsAsync(run.Id)).Count);
    }
    ...
    public async Task FindNodeRunByDecisionOperation_AnswersTheRowThatOperationDecidedAndNothingForAnother()
    {
        ...
        await ParkAsync(store, run.Id, "start");
        var operationId = Guid.NewGuid();
        _ = await DecideAsync(store, run.Id, operationId);

        var found = AssertEx.NotNull(await store.FindNodeRunByDecisionOperationAsync(run.Id, operationId));

        AssertEx.Equal("start", found.NodeKey);
        AssertEx.Null(await store.FindNodeRunByDecisionOperationAsync(run.Id, Guid.NewGuid()), "an id nobody used names no row.");
        AssertEx.Null(await store.FindNodeRunByDecisionOperationAsync(Guid.NewGuid(), operationId),
    ...
    public async Task DecideNodeRun_WithAnOperationIdAnotherNodeRunOfTheRunHolds_IsRefused()
    {
        ...
        {
            await ParkAsync(store, run.Id, nodeKey);
        }

        _ = await DecideAsync(store, run.Id, operationId);

        _ = await AssertEx.ThrowsAsync<GraphWorkflowInvalidTransitionException>(() => DecideAsync(store, run.Id, operationId, "done"));

        AssertEx.Equal(GraphWorkflowNodeRunStatus.WaitingForApproval,
            (await store.GetNodeRunAsync(run.Id, "done")).Status,
    ...
    {
        await StartRunningAsync(store, runId);
        await MoveAsync(store, runId, nodeKey, GraphWorkflowNodeRunStatus.Running);
        await MoveAsync(store, runId, nodeKey, GraphWorkflowNodeRunStatus.WaitingForApproval);
    }
    ...
    {
        var definition = await GraphWorkflowTestFixture.SeedDefinitionAsync(store, graphJson: graphJson);
        return await store.StartRunAsync(StartCommand(definition, Guid.NewGuid(), nodeKeys));
    }
    ...
    private static StartGraphWorkflowRunCommand StartCommand(GraphWorkflowDefinitionSnapshot definition, Guid requestId, IReadOnlyList<string>? nodeKeys = null) =>
        new()
        {
            ...
            NodeRuns =
            [
                .. (nodeKeys ?? ["start", "done"]).Select(static key => new GraphWorkflowNodeRunSeed
                {
