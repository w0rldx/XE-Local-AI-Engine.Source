...
///     Windows host can reach. That is not tidying: the defect this exists to close was that the decoration happened
///     on the non-Windows branch only, which is exactly the class of mistake an inline
...
{
    /// <summary>
    ///     The decorator appropriate to the at-rest scheme that host uses: DPAPI on Windows, the operator-secret
    ///     AES-GCM wrapper everywhere else.
    /// </summary>
    /// <remarks>
    ///     They differ only in which failure counts as a ring failure and in what the operator can do about it — never
    ///     in whether the ring is allowed to regenerate itself silently.
    /// </remarks>
    ...
    ///     registering the resolver by implementation type, since there would then be no inner instance to construct.
    ///     A fail-closed change on a startup path must never be able to brick a correct install, and this is the guard
