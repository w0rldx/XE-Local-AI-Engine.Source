...
/// <remarks>
///     The two name/description caps are this surface's own; the rest are shared with the skill draft route. Refusing
///     here rather than in the handler keeps an oversized or hostile request from ever reaching the slot.
/// </remarks>
public sealed class DraftAgentDefinitionRequestValidator : Validator<DraftAgentDefinitionRequest>
{
    private const int MaxExistingDescriptionLength = 2000;
    private const int MaxExistingNameLength = 120;

    public DraftAgentDefinitionRequestValidator()
    {
