...
{
    public static SlashCommandInput ToInput(this CreateSlashCommandRequest request) =>
        ToInput(request.Name, request.Description, request.Action);

    public static SlashCommandInput ToInput(this UpdateSlashCommandRequest request) =>
        ToInput(request.Name, request.Description, request.Action);

    public static SlashCommandResponse ToResponse(this SlashCommandCatalogItem item) =>
        new()
        {
            Id = item.Id,
            Name = item.Name,
            ...
            Source = item.Source,
            Action = new SlashCommandActionDto
            {
                Type = SlashCommandActionTypeDto.SendPrompt,
                Prompt = item.Prompt
            }
        };
