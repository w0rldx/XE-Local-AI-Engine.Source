...
/// <summary>
///     Authoring-time validation of a candidate command executable for the ProgramLaunch selector. Desktop-only, since
///     a headless host has no operator picking a local binary, and Operator-gated.
/// </summary>
/// <remarks>
///     Runs the same <c>HostExecutableGuard</c> checks the executor runs at launch — absolute path, not a shell,
///     interpreter or script, a real regular file (O_NOFOLLOW, no symlink) — and reports ok/reason. Not a filesystem
///     browser: it validates the one path the UI supplies.
/// </remarks>
