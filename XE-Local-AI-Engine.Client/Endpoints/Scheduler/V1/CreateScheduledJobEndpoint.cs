...
public sealed class CreateScheduledJobEndpoint : Endpoint<CreateScheduledJobRequest, ScheduledJobResponse>
{
    private readonly IScheduledJobManagementService _scheduledJobManagementService;

    public CreateScheduledJobEndpoint(IScheduledJobManagementService scheduledJobManagementService)
    {
    ...
    {
        var record = await _scheduledJobManagementService.CreateJobAsync(req.ToInput(), ct);
