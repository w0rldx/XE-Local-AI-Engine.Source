...
/// <summary>
///     FastEndpoints handler for disabling a scheduled job (POST scheduler/jobs/{scheduledJobId}/disable).
/// </summary>
    ...
    : Endpoint<ScheduledJobActionRequest, ScheduledJobResponse>
{
    ...
    {
        var record = await _scheduledJobManagementService.SetEnabledAsync(req.ScheduledJobId, false, ct).ConfigureAwait(false);
        if (record is null)
        {
