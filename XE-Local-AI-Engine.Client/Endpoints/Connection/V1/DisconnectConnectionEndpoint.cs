...
public sealed class DisconnectConnectionEndpoint : EndpointWithoutRequest<ConnectionStatusResponse>
{
    private readonly IConnectionControlService _connectionControlService;

    public DisconnectConnectionEndpoint(IConnectionControlService connectionControlService)
    {
    ...
    {
        var status = await _connectionControlService.DisconnectAsync(ct);
