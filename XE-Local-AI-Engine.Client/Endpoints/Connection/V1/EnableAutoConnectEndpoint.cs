...
public sealed class EnableAutoConnectEndpoint : EndpointWithoutRequest<ConnectionStatusResponse>
{
    private readonly IConnectionControlService _connectionControlService;

    public EnableAutoConnectEndpoint(IConnectionControlService connectionControlService)
    {
    ...
    {
        var status = await _connectionControlService.SetAutoConnectAsync(enabled: true, ct);
