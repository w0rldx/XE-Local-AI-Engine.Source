...
/// <summary>
///     Returns the inbound-MCP credential (including its plaintext key) plus the endpoint URL to point a client at.
///     Answers 200 with <c>configured=false</c> rather than 404 when no key exists, so the settings page can render the
///     empty state from one call.
/// </summary>
