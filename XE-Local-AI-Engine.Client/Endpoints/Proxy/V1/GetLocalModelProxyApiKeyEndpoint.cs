...
/// <summary>
///     Returns the inbound model-proxy credential's non-secret metadata (prefix, timestamps) plus the OpenAI base URL to
///     point a tool at. It cannot return the key itself — the node keeps only a one-way digest, and the response type has
