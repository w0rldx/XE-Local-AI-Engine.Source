...
///     runtime cannot be changed right now. A deliberate typed DTO rather than the shared <c>ConflictProblemDetails</c>
///     envelope, because most of these refusals are a value the service returns — an install outcome, a prerequisite
///     checklist — and a body no exception ever reaches cannot be an exception handler's to write (ADR 0009). Keeping
///     the envelope here means the install and remove routes cannot drift apart, and the reason code they share exists
///     once.
/// </summary>
