...
- **External Apps** — curated containerised applications a user installs onto the node from an XE-authored catalog. The shipped catalog is empty until the XE-owned catalog repository exists, so a node installs nothing until a catalog is published to it; everything below describes the machinery that runs an application once one is. Every image is pinned by digest and every application is described by an XE-owned manifest whose schema is the allow-list; the engine never reads a user-supplied Compose file. Containers run under an engine-owned policy — `cap_drop ALL` with additions bounded by Docker's own default set, `no-new-privileges`, seccomp, a private per-instance network, an explicit mount set and **ports published on loopback only** — and the daemon's own read-back is verified against that policy before and after start, so a disagreement stops the instance instead of warning about it. Each instance keeps its data in a directory the engine owns, and reset and uninstall delete it through a short-lived engine-owned helper container, which is what makes deletion work for data an application wrote as its own non-root user. **A running Docker daemon is required for this feature and for nothing else**: Docker stays off the inference path, and a node without a daemon simply installs nothing. It ships **enabled by default** (`ExternalApps:Enabled`); setting that to `false` and restarting returns 404 from every route and the hub, but it does **not** stop containers that are already running and does **not** hide the navigation entry, which is compile-time and stays until the next build — so the order for disabling a node is stop or uninstall every instance, then set the flag (`Services/ExternalApps`, `Services/Containers`, `Endpoints/ExternalApps/V1`, `catalog/external-apps`, `src/features/externalApps`). See [docs/wiki/23-external-apps.md](docs/wiki/23-external-apps.md) and [ADR 0010](docs/adr/0010-external-apps-container-execution.md), with the maintained build record in [External Apps implementation status](docs/roadmaps/external-apps-status.md).
