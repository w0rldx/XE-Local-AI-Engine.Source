...
///     tree-kill + restart it on demand.
/// </summary>
/// <remarks>
///     Mirrors <c>ILlamaServerProcessSupervisor</c> for the image runtime and also exposes the operator eject boundary
///     used before managed-runtime mutation.
/// </remarks>
...
public interface IImageServerSupervisor
{
    ...
    /// </summary>
    /// <remarks>
    ///     Reuses a live daemon; otherwise resolves the model file-set, selects the GPU backend, acquires the binary,
    ///     launches, and waits for readiness (poll <c>/sdcpp/v1/capabilities</c>).
    /// </remarks>
    ...
    ///     reaper and cap-admission LRU evictor keep it alive while a generation is in flight.
    /// </summary>
    /// <remarks>
    ///     Returns <see langword="null" /> when no live daemon backs the model — the caller then proceeds leaseless and
    ///     relies on the just-completed <see cref="EnsureRunningAsync" /> plus self-heal. Call immediately after
