...
///     CUDA/dxcore and <c>sd-server --backend vulkan0</c> hard-fails with "backend 'vulkan0' was not found". The
///     <see cref="ISdGpuBackendSelector" /> consults this before choosing Vulkan so it can fall back to CPU, which always works. The
///     probe is intentionally conservative (fail-safe), so any uncertainty resolves to "absent". Implementations must be cheap and
///     thread-safe; the default computes its verdict once and caches it for the process lifetime.
/// </remarks>
