...
- **build-and-test** — Release build, `scripts/openapi-live-check.sh`, then the solution's test
  ...
  `scripts/run-tests-memory-safe.sh` (`GROUPS=$(nproc)` processes, each covering a packed set of
