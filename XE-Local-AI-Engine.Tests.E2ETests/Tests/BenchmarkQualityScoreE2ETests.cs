...
public sealed class BenchmarkQualityScoreE2ETests : XESerialE2ETestBase
{
    ...
    {
        var judgeScoreA = await SeedProjectWithTwoJudgedRunsAsync();
        ...
        await Expect(Page.GetByTestId($"benchmark-quality-source-{_runA}")).ToHaveTextAsync("judge");
        ...
        await Page.GetByTestId($"benchmark-run-select-{_runA}").CheckAsync();
        ...
        await paneA.GetByTestId("benchmark-score-save").ClickAsync();
        await Expect(Page.GetByTestId($"benchmark-quality-source-{_runA}")).ToHaveTextAsync("operator", Polled);
        ...
        await Expect(Page.GetByTestId($"benchmark-quality-source-{_runA}")).ToHaveTextAsync("judge", Polled);
        await Expect(QualityScore(_runA)).ToHaveTextAsync(judgeScoreA.ToString(provider: null));
        ...
        await Expect(Page.GetByTestId("benchmark-rubric-editor")).ToBeVisibleAsync();
        ...
        await Expect(confirm).ToContainTextAsync("All 2 succeeded runs will be re-judged");
        await Page.GetByTestId("benchmark-rejudge-confirm-accept").ClickAsync();

        // 6. The activation reset the cohort and enqueued one fresh attempt per succeeded run under revision 2.
        await Expect(paneA.GetByTestId("benchmark-judge-state")).ToHaveTextAsync("Judge queued", Polled);
        await Expect(paneA.GetByTestId("benchmark-judge-policy")).ToHaveTextAsync("policy r2", Polled);
    ...
    public async Task RemoveSeededProjectAsync()
    {
        ...
        while (await store.ClaimNextAsync() is { } work)
        {
            if (work.Kind == BenchmarkWorkKind.Judge)
            {
                _ = await store.MarkJudgeCancelledAsync(work.RunId, work.Version);
                continue;
            }

            _ = await store.MarkPrimaryCancelledAsync(work.RunId, work.Version);
        }
        ...
        foreach (var run in (await store.ListRunsAsync(_projectId, skip: 0, take: 100)).Items)
        {
            await store.DeleteRunAsync(run.Id, run.Version);
        }

        if (await store.GetProjectAsync(_projectId) is { } project)
        {
            await store.DeleteProjectAsync(_projectId, project.Version);
        }
    ...
    {
        await using var scope = Factory.Services.CreateAsyncScope();
        var scores = await CompleteQueuedJudgeWorkAsync(scope.ServiceProvider.GetRequiredService<IBenchmarkStore>());
        return scores[_runA];
    }
    ...
    private async Task<Dictionary<Guid, int>> CompleteQueuedJudgeWorkAsync(IBenchmarkStore store)
    {
        ...
        while (await store.ClaimNextAsync() is { Kind: BenchmarkWorkKind.Judge } work)
        {
            var attemptId = work.JudgeAttemptId
                            ?? throw new InvalidOperationException("Judge work carried no attempt id.");
            var attempt = await store.GetJudgeAttemptAsync(attemptId)
                          ?? throw new InvalidOperationException("The claimed judge attempt disappeared.");
            var revision = await store.GetJudgePolicyRevisionAsync(attempt.PolicyRevisionId)
